...
function MemberHome(p){return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">내 몸이 어떻게 변하고 있는지 확인해요</p><div className="hero-card"><b>오늘 상태</b><span>{p.checkins?.[0]?`${p.checkins[0].condition||"보통"} · 근육통 ${p.checkins[0].soreness||"없음"}`:"건강관리 탭에서 오늘 상태를 기록해보세요"}</span></div><div className="grid2"><Metric t="체중 변화" v={`${p.startW} → ${p.curW}kg`}/><Metric t="오늘 목표" v={p.onboarding.goal||"꾸준한 변화"}/></div><ReviewRoutine {...p}/><MCard title="대표 피드백"><p>{p.latest?.trainerComment||"공개 수업일지의 대표 코멘트가 여기에 표시됩니다."}</p></MCard></>}
...
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); const inbody=p.body?.inbody||[]; const volume=p.sessions.reduce((a,s)=>a+(s.totalVolume||0),0); const partData=buildPartVolumeChange(p.sessions); const msg=partData.find(x=>x.delta>0); const colors=["#2F73F6","#16C784","#ff9f43","#8B5CF6","#ff6b6b","#5EEAD4"]; return <><h1>변화분석</h1><p className="sub">운동량보다 몸의 변화와 수행능력 향상을 먼저 보여드려요</p><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={190}><LineChart data={weightData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" name="체중" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><MCard title="부위별 볼륨 변화 그래프"><ResponsiveContainer width="100%" height={260}><BarChart data={buildPartVolumeChart(p.sessions)}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name"/><YAxis/><Tooltip/><Legend/>{["가슴","등","어깨","하체","팔","코어"].map((k,i)=><Bar key={k} dataKey={k} fill={colors[i]} radius={[6,6,0,0]}/>)}</BarChart></ResponsiveContainer><p className="notice soft">2주 전 · 지난주 · 이번주를 비교해 어떤 부위의 운동량이 늘었는지 확인하세요.</p></MCard><MCard title="부위별 평균 볼륨 변화">{partData.map(x=><div className="volume-card" key={x.part}><b>{x.part} 평균 볼륨</b><span>초기 {Math.round(x.first).toLocaleString()}kg → 최근 {Math.round(x.recent).toLocaleString()}kg</span></div>)}<p className="notice soft">{msg?`${msg.part} 운동 볼륨이 꾸준히 증가하고 있습니다.`:'기록이 쌓일수록 변화가 더 선명하게 보입니다.'}</p></MCard><div className="grid2 secondary-metrics"><Metric t="골격근량" v={(inbody.at(-1)?.muscleMass)||'-'}/><Metric t="체지방" v={(inbody.at(-1)?.bodyFat)||'-'}/></div><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard><MCard title="참고 정보"><Info l="누적 운동량" v={`${Math.round(volume).toLocaleString()}kg`}/></MCard></>}
