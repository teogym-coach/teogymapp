...
function ReviewRoutine({profile,sessions,onboarding,checkins}){const nextPart=profile?.nextPtPart&&profile.nextPtPart!=="미정"?profile.nextPtPart:""; const initial=MEMBER_WORKOUT_PARTS.find(x=>x!==nextPart)||"등"; const [showAll,setShowAll]=useState(false); const [part,setPart]=useState(initial); const parts=showAll?MEMBER_WORKOUT_PARTS:MEMBER_WORKOUT_PARTS.filter(x=>x!==nextPart); const rec=buildReviewRoutine(sessions,onboarding,checkins,part); return <MCard title="대표님 추천 개인운동"><p className="notice">대표님과 함께한 수업 기록을 기준으로 만든 개인운동 추천입니다.<br/>무리하지 말고 가능한 중량으로 진행해주세요.</p>{nextPart&&part!==nextPart&&<p className="notice soft">{nextPart}는 다음 PT에서 진행 예정입니다.<br/>오늘 개인운동은 다른 부위를 추천드릴게요.</p>}<div className="part-pills">{parts.map(x=><button key={x} className={part===x?"active":""} onClick={()=>setPart(x)}>{x}</button>)}</div>{nextPart&&<button className="linkbtn" onClick={()=>setShowAll(v=>!v)}>{showAll?"추천 부위만 보기":"전체 부위 보기"}</button>}<h3>{part} 추천</h3>{!rec.hasData&&<p className="notice soft">아직 기록이 많지 않아 기본 루틴을 추천드립니다.</p>}<RecommendationGroup title="자극이 좋았던 운동" items={rec.goodStim} desc="최근 자극도 높음 / 불편감 기록 없음"/><RecommendationGroup title="불편감 없이 수행한 운동" items={rec.painFree} desc="최근 기록에서 통증 메모 없음"/><RecommendationGroup title="연습이 필요한 운동" items={rec.practice} desc="최근 수행 횟수 적음 / 자세 연습 필요"/><h3>오늘 추천 루틴</h3>{rec.routine.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><span>{x.sets}세트 · {x.reps}회 · {x.weight}</span><em>{rec.hasData?"최근 수업 기준":"기본 안전 루틴"}</em></div>)}<p className="danger">컨디션이 좋지 않다면 중량을 낮춰 진행해주세요.</p><p>대표 코멘트: {rec.comment}</p></MCard>}
