...
// 상태 정의
const BODY_STATUS = {
  tight: { label:"타이트 / 과긴장", color:"#ef4444", dot:"#ef4444", emoji:"🔴" },
  weak:  { label:"약화 / 기능저하", color:"#3b82f6", dot:"#3b82f6", emoji:"🔵" },
};

// ── 전면 근육 SVG 패스 (viewBox 0 0 200 480) ──
const FRONT_MUSCLES = [
  // 상부승모근 L/R (어깨~목)
  { id:"utrap_l", label:"상부승모근L", side:"L",
    d:"M52 72 Q44 80 42 95 Q48 100 58 98 Q66 90 68 80 Q62 72 52 72 Z" },
  { id:"utrap_r", label:"상부승모근R", side:"R",
    d:"M148 72 Q156 80 158 95 Q152 100 142 98 Q134 90 132 80 Q138 72 148 72 Z" },
  // 전면 어깨 (삼각근 전면) L/R
  { id:"ant_delt_l", label:"전면어깨L", side:"L",
    d:"M38 95 Q28 105 26 125 Q28 140 38 148 Q48 142 52 128 Q54 112 48 100 Z" },
  { id:"ant_delt_r", label:"전면어깨R", side:"R",
    d:"M162 95 Q172 105 174 125 Q172 140 162 148 Q152 142 148 128 Q146 112 152 100 Z" },
  // 흉근 L/R
  { id:"pec_l", label:"흉근L", side:"L",
    d:"M68 80 Q58 85 48 100 Q52 128 60 138 Q72 145 86 142 L90 115 Q90 88 80 80 Z" },
  { id:"pec_r", label:"흉근R", side:"R",
    d:"M132 80 Q142 85 152 100 Q148 128 140 138 Q128 145 114 142 L110 115 Q110 88 120 80 Z" },
  // 복직근
  { id:"rectus_ab", label:"복직근", side:"C",
    d:"M86 142 Q82 155 82 200 Q86 215 100 218 Q114 215 118 200 Q118 155 114 142 Z" },
  // 외복사근 L/R
  { id:"ext_obl_l", label:"외복사근L", side:"L",
    d:"M60 138 Q52 155 52 185 Q56 205 70 210 Q80 205 82 188 Q82 165 74 148 Z" },
  { id:"ext_obl_r", label:"외복사근R", side:"R",
    d:"M140 138 Q148 155 148 185 Q144 205 130 210 Q120 205 118 188 Q118 165 126 148 Z" },
  // 고관절 굴곡근 L/R (장요근)
  { id:"hip_flex_l", label:"고관절굴곡근L", side:"L",
    d:"M82 218 Q74 228 72 245 Q76 255 86 255 Q94 250 96 238 Q96 225 90 218 Z" },
  { id:"hip_flex_r", label:"고관절굴곡근R", side:"R",
    d:"M118 218 Q126 228 128 245 Q124 255 114 255 Q106 250 104 238 Q104 225 110 218 Z" },
  // 대퇴사두 L/R
  { id:"quad_l", label:"대퇴사두L", side:"L",
    d:"M60 255 Q52 275 50 320 Q52 350 62 360 Q76 362 84 352 Q90 338 88 295 Q86 268 78 255 Z" },
  { id:"quad_r", label:"대퇴사두R", side:"R",
    d:"M140 255 Q148 275 150 320 Q148 350 138 360 Q124 362 116 352 Q110 338 112 295 Q114 268 122 255 Z" },
  // 내전근 L/R
  { id:"adduct_l", label:"내전근L", side:"L",
    d:"M88 255 Q84 275 84 320 Q86 348 90 358 Q98 362 104 355 Q108 338 106 295 Q104 268 96 255 Z" },
  { id:"adduct_r", label:"내전근R", side:"R",
    d:"M112 255 Q116 275 116 320 Q114 348 110 358 Q102 362 96 355 Q92 338 94 295 Q96 268 104 255 Z" },
];

// ── 후면 근육 SVG 패스 ──
const BACK_MUSCLES = [
  // 상부승모근 L/R
  { id:"utrap_b_l", label:"상부승모근L", side:"L",
    d:"M52 72 Q44 80 42 96 Q50 102 62 99 Q70 90 68 78 Q62 72 52 72 Z" },
  { id:"utrap_b_r", label:"상부승모근R", side:"R",
    d:"M148 72 Q156 80 158 96 Q150 102 138 99 Q130 90 132 78 Q138 72 148 72 Z" },
  // 중하부 승모근 (마름근 포함)
  { id:"mid_trap", label:"중하부승모근", side:"C",
    d:"M68 98 Q62 110 60 135 Q68 145 100 150 Q132 145 140 135 Q138 110 132 98 Q118 105 100 106 Q82 105 68 98 Z" },
  // 후면 어깨 (삼각근 후면) L/R
  { id:"post_delt_l", label:"후면어깨L", side:"L",
    d:"M38 96 Q28 108 26 128 Q28 142 40 150 Q50 142 54 128 Q56 110 48 100 Z" },
  { id:"post_delt_r", label:"후면어깨R", side:"R",
    d:"M162 96 Q172 108 174 128 Q172 142 160 150 Q150 142 146 128 Q144 110 152 100 Z" },
  // 광배근 L/R
  { id:"lat_l", label:"광배L", side:"L",
    d:"M40 148 Q34 168 36 205 Q42 220 60 225 Q78 220 82 205 Q80 170 70 148 Z" },
  { id:"lat_r", label:"광배R", side:"R",
    d:"M160 148 Q166 168 164 205 Q158 220 140 225 Q122 220 118 205 Q120 170 130 148 Z" },
  // 척추기립근 L/R
  { id:"erect_l", label:"척추기립근L", side:"L",
    d:"M84 108 Q80 130 80 180 Q82 210 88 225 Q96 228 100 225 Q100 195 98 158 Q96 128 92 108 Z" },
  { id:"erect_r", label:"척추기립근R", side:"R",
    d:"M116 108 Q120 130 120 180 Q118 210 112 225 Q104 228 100 225 Q100 195 102 158 Q104 128 108 108 Z" },
  // 둔근 L/R
  { id:"glute_l", label:"둔근L", side:"L",
    d:"M58 238 Q50 255 50 280 Q54 298 68 302 Q84 302 92 290 Q96 272 90 252 Q82 238 68 238 Z" },
  { id:"glute_r", label:"둔근R", side:"R",
    d:"M142 238 Q150 255 150 280 Q146 298 132 302 Q116 302 108 290 Q104 272 110 252 Q118 238 132 238 Z" },
  // 햄스트링 L/R
  { id:"ham_l", label:"햄스트링L", side:"L",
    d:"M52 298 Q46 320 46 358 Q50 374 62 376 Q76 374 82 360 Q86 340 82 308 Q74 298 62 298 Z" },
  { id:"ham_r", label:"햄스트링R", side:"R",
    d:"M148 298 Q154 320 154 358 Q150 374 138 376 Q124 374 118 360 Q114 340 118 308 Q126 298 138 298 Z" },
  // 비복근 L/R
  { id:"gastro_l", label:"비복근L", side:"L",
    d:"M50 378 Q44 400 46 428 Q52 440 64 440 Q74 436 78 420 Q80 400 76 380 Q66 374 56 378 Z" },
  { id:"gastro_r", label:"비복근R", side:"R",
    d:"M150 378 Q156 400 154 428 Q148 440 136 440 Q126 436 122 420 Q120 400 124 380 Q134 374 144 378 Z" },
  // 가자미근 L/R
  { id:"soleus_l", label:"가자미근L", side:"L",
    d:"M48 430 Q44 448 46 462 Q52 468 64 467 Q74 464 76 450 Q78 436 72 430 Z" },
  { id:"soleus_r", label:"가자미근R", side:"R",
    d:"M152 430 Q156 448 154 462 Q148 468 136 467 Q126 464 124 450 Q122 436 128 430 Z" },
];
...
// bodyMap: { muscleId: "tight" | "weak" }
function getMuscleStatus(bodyMap, id) { return bodyMap[id] || null; }

function generateAutoSummary(bodyMap, posture, mobility) {
  const getName = id => ALL_MUSCLES.find(m=>m.id===id)?.label || id;
  const tightList = Object.entries(bodyMap).filter(([,v])=>v==="tight").map(([k])=>getName(k));
  const weakList  = Object.entries(bodyMap).filter(([,v])=>v==="weak").map(([k])=>getName(k));
  const issues = [];
  ...
  const mobIssues = MOBILITY_ITEMS.filter(item=>{const v=mobility[item.key];return v&&(v==="제한"||v==="제한 있음"||v==="약함"||v==="있음");}).map(item=>item.label);
  if (mobIssues.length) issues.push("기능 제한: "+mobIssues.join(", "));
  if (!issues.length) return "";
  const recs = [];
  if (tightList.some(l=>l.includes("흉근")||l.includes("전면어깨"))) recs.push("흉근 이완 + 후면 어깨 활성화");
  if (weakList.some(l=>l.includes("둔근"))) recs.push("둔근 활성화 운동");
  if (weakList.some(l=>l.includes("척추기립근")||l.includes("중하부승모근"))) recs.push("코어 안정화 + 중부 승모근 강화");
...
// ── 근육 형태 바디맵 SVG ──────────────────────
function MuscleBodySVG({ muscles, bodyMap, mode, onClickMuscle, view }) {
  // 인체 실루엣 경로 (전/후 공통 외곽)
  const SILHOUETTE_FRONT = "M100 8 C88 8 78 18 78 30 C78 44 86 54 96 58 L92 68 C80 70 68 75 60 82 C48 90 38 100 34 115 L26 145 C22 160 24 175 30 185 L36 210 C38 218 36 225 34 232 L30 258 C62 260 76 255 86 258 L84 310 C82 338 80 360 78 382 L74 415 C72 428 72 442 74 455 L76 468 L90 468 L90 455 C90 442 90 428 90 415 L92 382 C94 360 96 338 96 312 L98 268 L102 268 L104 312 C104 338 106 360 108 382 L110 415 C110 428 110 442 110 455 L110 468 L124 468 L126 455 C128 442 128 428 126 415 L122 382 C120 360 118 338 116 310 L114 258 C124 255 138 260 170 258 L166 232 C164 225 162 218 164 210 L170 185 C176 175 178 160 174 145 L166 115 C162 100 152 90 140 82 C132 75 120 70 108 68 L104 58 C114 54 122 44 122 30 C122 18 112 8 100 8 Z";
  const SILHOUETTE_BACK  = SILHOUETTE_FRONT; // 동일 외곽 사용

  return (
    <svg viewBox="0 0 200 480" style={{width:"100%",maxWidth:220,display:"block",margin:"0 auto"}}>
      <defs>
        <filter id="mGlow" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur"/>
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
        <filter id="mGlowStrong" x="-30%" y="-30%" width="160%" height="160%">
      ...
      {/* 실루엣 베이스 */}
      <path d={view==="front"?SILHOUETTE_FRONT:SILHOUETTE_BACK}
        fill="#13131f" stroke="#2a2a40" strokeWidth="1.5" strokeLinejoin="round"/>

      {/* 근육 패스 */}
      {muscles.map(m => {
        const status = getMuscleStatus(bodyMap, m.id);
        const color  = status ? BODY_STATUS[status].color : null;
        const hovered= false;
        return (
          <path key={m.id} d={m.d}
            fill={status ? color+"cc" : "#1e1e32"}
            stroke={status ? color : "#2a2a42"}
            strokeWidth={status ? 1.5 : 0.8}
            filter={status ? "url(#mGlow)" : "none"}
      ...
      {/* 라벨 오버레이 — 상태 있는 것만 */}
      {muscles.filter(m=>bodyMap[m.id]).map(m => {
        const status= bodyMap[m.id];
        const color = BODY_STATUS[status]?.color;
        // 패스 중심점 대략 계산 (bbox 없으므로 label 위치는 부위 x,y로)
        const pts   = m.d.match(/[\d.]+\s+[\d.]+/g)||[];
        const xs    = pts.map(p=>parseFloat(p.split(" ")[0]));
        const ys    = pts.map(p=>parseFloat(p.split(" ")[1]));
        const cx    = (Math.min(...xs)+Math.max(...xs))/2;
        const cy    = (Math.min(...ys)+Math.max(...ys))/2;
        return (
          <text key={m.id+"_lbl"} x={cx} y={cy} textAnchor="middle" dominantBaseline="middle"
            style={{fontSize:5.5,fill:color,fontWeight:"700",pointerEvents:"none",
              userSelect:"none",fontFamily:"sans-serif",filter:"url(#mGlowStrong)"}}>
            {m.label.replace("L","").replace("R","").substring(0,5)}
          </text>
        );
      })}

      {/* 뷰 레이블 */}
      <text x="100" y="476" textAnchor="middle"
        style={{fontSize:7,fill:"#2a2a40",fontFamily:"monospace",letterSpacing:".12em"}}>
...
function AssessmentScreen({ member, onBack, showToast }) {
  ...
  async function generateAIRoutine() {
    ...
    const tightMuscles = Object.entries(bodyMap).filter(([,v])=>v==="tight")
      .map(([k])=>ALL_MUSCLES.find(m=>m.id===k)?.label||k);
...
각 단계별로 구체적인 운동 이름과 세트/횟수를 포함해 간결하게 작성해주세요. 한국어로 답변하세요.`;
  ...
  if (viewRec) {
    ...
    const getLabel = id => ALL_MUSCLES.find(m=>m.id===id)?.label||id;
    return (
      <div>
            ...
            {tList.length>0 && (
              <div style={{marginBottom:8}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {tList.map(([id])=><span key={id} style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"2px 8px",borderRadius:4,background:"rgba(239,68,68,.15)",color:"#ef4444"}}>{getLabel(id)}</span>)}
                </div>
              </div>
            )}
            {wList.length>0 && (
              <div>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {wList.map(([id])=><span key={id} style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"2px 8px",borderRadius:4,background:"rgba(59,130,246,.15)",color:"#3b82f6"}}>{getLabel(id)}</span>)}
                </div>
              </div>
            )}
          </Card>
    ...
    <div>
      ...
      {tab==="바디맵" && (
        <div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:12}}>
            <button onClick={()=>setMode("tight")}
              style={{flex:1,padding:"10px",borderRadius:8,border:"2px solid",cursor:"pointer",
                borderColor:mode==="tight"?"#ef4444":"#1a1a24",
                background:mode==="tight"?"rgba(239,68,68,.15)":"transparent",
                color:mode==="tight"?"#ef4444":"#54546a",fontWeight:800,fontSize:13}}>
              🔴 타이트 / 과긴장
            </button>
            <button onClick={()=>setMode("weak")}
              style={{flex:1,padding:"10px",borderRadius:8,border:"2px solid",cursor:"pointer",
                borderColor:mode==="weak"?"#3b82f6":"#1a1a24",
                background:mode==="weak"?"rgba(59,130,246,.15)":"transparent",
                color:mode==="weak"?"#3b82f6":"#54546a",fontWeight:800,fontSize:13}}>
              🔵 약화 / 기능저하
            </button>
          </div>
          ...
          {/* 카운터 */}
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            <div style={{flex:1,padding:"6px 10px",borderRadius:7,background:"rgba(239,68,68,.1)",border:"1px solid rgba(239,68,68,.2)"}}>
              <Mo c="#ef4444" s={9}>🔴 과긴장 {tightCount}곳</Mo>
            </div>
            <div style={{flex:1,padding:"6px 10px",borderRadius:7,background:"rgba(59,130,246,.1)",border:"1px solid rgba(59,130,246,.2)"}}>
              <Mo c="#3b82f6" s={9}>🔵 기능저하 {weakCount}곳</Mo>
            </div>
          </div>

          {/* 바디맵 */}
          <Card style={{marginBottom:10,background:"#0d0d18",border:"1px solid #1a1a2e"}}>
            <MuscleBodySVG
            ...
            <Card style={{marginBottom:10}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {Object.entries(bodyMap).map(([id,status])=>{
                  ...
                  const label = ALL_MUSCLES.find(m=>m.id===id)?.label||id;
                  return (
                    <button key={id}
                      ...
                      style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"3px 8px",borderRadius:4,
                        background:col+"22",color:col,border:"1px solid "+col+"44",cursor:"pointer"}}>
                      {label} ✕
                    </button>
                  );
                })}
                  ...
                  <div key={r.id} style={{background:"#111116",border:"1px solid #1a1a24",borderRadius:10,padding:"12px 14px",cursor:"pointer"}}
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                      ...
                      <div style={{display:"flex",gap:5}}>
                        {tc>0&&<span style={{fontFamily:"'DM Mono',monospace",fontSize:8,padding:"2px 6px",borderRadius:4,background:"rgba(239,68,68,.15)",color:"#ef4444"}}>🔴 {tc}</span>}
                        {wc>0&&<span style={{fontFamily:"'DM Mono',monospace",fontSize:8,padding:"2px 6px",borderRadius:4,background:"rgba(59,130,246,.15)",color:"#3b82f6"}}>🔵 {wc}</span>}
