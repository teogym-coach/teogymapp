...
function MemberApp({ onLogout }) {
  ...
  const effectiveBody=applyHistoryMode(body,effectiveOnboarding.weightHistoryMode,effectiveOnboarding.weightHistoryModeStartedAt,"records"); const effectiveNutrition=applyHistoryMode(nutrition,effectiveOnboarding.calorieHistoryMode,effectiveOnboarding.calorieHistoryModeStartedAt,"logs"); const weights=getBodyWeightRecords(effectiveBody), curW=profile.currentWeight||weights.at(-1)?.weight||form.weight||profile.weight||"-", startW=profile.startWeight||weights[0]?.weight||curW; const totalReg=parseInt(String(profile.totalSessions||"").replace(/[^0-9]/g,""))||0; const remaining=totalReg?Math.max(0,totalReg-sessions.length):"-"; const latest=sessions.at(-1); const recentKcal=(effectiveNutrition?.logs||[]).find(l=>l.date===today)?.kcal||(effectiveNutrition?.logs||[]).at(-1)?.kcal||"-"; const steps=(checkins.find(c=>c.date===today||c.id===today)?.steps)||checkins.find(c=>c.steps)?.steps||"-";
...
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); return <><h1>변화분석</h1><p className="sub">체중 변화와 같은 부위의 최근 운동량 변화를 확인해요</p><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={190}><LineChart data={weightData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" name="체중" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><CalorieTrendCard {...p}/><MemberCalorieAnalysisCard {...p}/><PartVolumeCard sessions={p.sessions}/><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard></>}
...
function applyHistoryMode(data,mode,startDate,field){if(mode!=="fresh"||!startDate||!data)return data; const key=String(startDate).slice(0,10); const rows=(data[field]||[]).filter(r=>String(r.date||r.id||"").slice(0,10)>=key); return {...data,[field]:rows};}
...
function DataImportToggle({label,value,onChange,disabled}){return <div style={{padding:10,borderRadius:8,background:"#020617",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#e2e8f0" s={10} style={{display:"block",marginBottom:7}}>{label}</Mo><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>{[["import","불러오기"],["fresh","새로 시작"]].map(([k,t])=><Btn key={k} ghost sm onClick={()=>onChange(k)} disabled={disabled} style={{marginTop:0,color:value===k?"#0f172a":"#cbd5e1",background:value===k?"#5EEAD4":"#111827",borderColor:value===k?"#5EEAD4":"rgba(255,255,255,.12)"}}>{t}</Btn>)}</div></div>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const saveImportMode=async(key,value)=>{setBusy(true); setMsg(""); try{const now=new Date().toISOString(); const patch={...(ob||{}),[key]:value,[`${key}StartedAt`]:value==="fresh"?now:(ob?.[`${key}StartedAt`]||""),updatedAt:now}; await saveMemberOnboarding(memberId,patch); setOb(patch); setMsg(`${key==="weightHistoryMode"?"체중 기록":"섭취칼로리 기록"} 표시 기준 저장 완료 · ${value==="import"?"불러오기":"새로 시작"}`);}catch(e){setMsg(`표시 기준 저장 실패 · ${e?.message||String(e)}`);}finally{setBusy(false);}};
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:8,alignItems:"stretch"}}>
      ...
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.length?ci.map(c=>{const pain=c.painRecord||{}; const soreParts=c.sorenessBodyParts?.length?c.sorenessBodyParts.join(", "):(c.sorenessPart||c.soreness||pain.part||"없음"); const soreLevel=c.sorenessLevel||c.painLevel||c.sorenessIntensity||pain.level||"-"; const rpe=c.rpe??c.sessionRpe??"-"; return <div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5,lineHeight:1.5}}><b style={{color:"#e5e7eb"}}>{c.date||c.id}</b><br/>체중 {c.weight||"-"}kg · 컨디션 {c.condition||"-"}<br/>근육통: {soreParts} · 강도: {soreLevel} · RPE: {rpe}</div>}):<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 체크인 기록이 없습니다</div>}</div>
    </div>
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"grid",gap:8}}><Mo c="#38bdf8" s={9}>기존 기록 표시 기준</Mo><DataImportToggle label="체중 기록" value={ob?.weightHistoryMode||"import"} onChange={v=>saveImportMode("weightHistoryMode",v)} disabled={busy}/><DataImportToggle label="섭취칼로리 기록" value={ob?.calorieHistoryMode||"import"} onChange={v=>saveImportMode("calorieHistoryMode",v)} disabled={busy}/><div style={{fontSize:10,color:"#94a3b8"}}>새로 시작을 선택해도 기존 데이터는 삭제하지 않고 회원앱 표시 기준만 저장합니다.</div></div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원앱 온보딩</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{ob?.completedAt?`${ob.gender||"-"} · ${formatAgeFromBirthYear(ob.birthYear)} · ${ob.heightCm||"-"}cm · ${ob.startingWeightKg||"-"}kg · ${ob.goal||"-"} · ${ob.weeklyWorkoutCount||"-"}`:"아직 완료 전"}</div>{ob?.completedAt&&ob?.birthYear&&<div style={{fontSize:10,color:"#64748b",marginTop:3}}>상세: {ob.birthYear}년생</div>}{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>집중 부위: {ob.focusAreas.join(" · ")}</div>}</div>
