...
function MemberApp({ onLogout }) {
  ...
  if(!onboardingReadFailed&&!onboarding?.completedAt) return <><MemberDebugPanel profile={profile} logs={accessLogs} errors={accessErrors}/><MemberOnboarding profile={profile} body={body} existing={onboarding} onDone={load}/></>;
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,curW,startW,remaining,latest,recentKcal,steps,form,setForm,saveCheck,onLogout,setTab,accessErrors};
  return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><main className="member-page"><MemberDebugPanel profile={profile} logs={accessLogs} errors={accessErrors}/>{tab==="home"&&<MemberHome {...common}/>} {tab==="workout"&&<MemberWorkout {...common}/>} {tab==="health"&&<MemberHealth {...common}/>} {tab==="analysis"&&<MemberAnalysis {...common}/>} {tab==="profile"&&<MemberProfile {...common}/>}</main><nav className="member-nav">{[["home","⌂","홈"],["workout","＋","운동기록"],["health","♥","건강관리"],["analysis","▥","변화분석"],["profile","●","프로필"]].map(([k,i,l])=><button key={k} onClick={()=>setTab(k)} className={tab===k?"active":""}><span>{i}</span>{l}</button>)}</nav></div>;
}

function formatDiagValue(v){return v?.toDate?.()?.toISOString?.()||v||"-";}
function MemberMatchList({title,items}){return <div className="notice" style={{marginTop:10}}><b>{title}</b>{(items||[]).length===0?<div>조회 결과 없음</div>:(items||[]).map(m=><div key={m.id} style={{marginTop:8,paddingTop:8,borderTop:"1px solid rgba(47,115,246,.18)"}}><b>memberId:</b> {m.id}<br/><b>name:</b> {m.name||"-"}<br/><b>email:</b> {m.email||"-"}<br/><b>memberUid:</b> {m.memberUid||"-"}<br/><b>trainerUid:</b> {m.trainerUid||"-"}<br/><b>createdAt:</b> {formatDiagValue(m.createdAt)}</div>)}</div>}
...
function MemberDiagnostics({profile,details}){const d=profile?._diagnostics||details||{}; const matched=profile||{}; const queryErrors=d.queryErrors||{}; return <><div className="notice"><b>auth.uid:</b> {d.authUid||auth.currentUser?.uid||"-"}<CopyValueButton value={d.authUid||auth.currentUser?.uid||""}/><br/><b>auth.email:</b> {d.authEmail||auth.currentUser?.email||"-"}<br/><b>memberUid 쿼리 결과 개수:</b> {(d.memberUidMatches||[]).length}<br/><b>email 쿼리 결과 개수:</b> {(d.emailMatches||[]).length}<br/><b>최종 매칭된 memberId:</b> {d.matchedMemberId||matched.id||"-"}<br/><b>최종 매칭 기준:</b> {d.matchedBy||matched._matchedBy||"none"}<br/><b>최종 매칭 name:</b> {matched.name||"-"}<br/><b>최종 매칭 email:</b> {matched.email||"-"}<br/><b>최종 매칭 memberUid:</b> {matched.memberUid||"-"}<br/><b>최종 매칭 trainerUid:</b> {matched.trainerUid||"-"}</div>{Object.keys(queryErrors).length>0&&<div className="danger">진단 쿼리 실패: {Object.entries(queryErrors).map(([k,e])=>`${k} · ${e.path} (${e.code||"unknown"})`).join(" · ")}</div>}<MemberMatchList title="email == auth.email 조회 문서" items={d.emailMatches}/><MemberMatchList title="memberUid == auth.uid 조회 문서" items={d.memberUidMatches}/></>;}
...
function MemberOnboarding({profile,body,existing,onDone}){const today=new Date().toISOString().slice(0,10); const [step,setStep]=useState(0); const [d,setD]=useState({gender:existing?.gender||profile.gender||"",birthYear:existing?.birthYear||profile.birthYear||profile.birth||1988,heightCm:existing?.heightCm||profile.height||"",startingWeightKg:existing?.startingWeightKg||getLatestBodyWeight(body)?.weight||profile.weight||"",goal:existing?.goal||"",focusAreas:existing?.focusAreas||[],weeklyWorkoutCount:existing?.weeklyWorkoutCount||""}); const qs=["성별","출생연도","키","현재 체중","운동 목표","집중 관리 부위","주 운동 가능 횟수"]; const next=async()=>{if(step<6){setStep(step+1);return} await saveMemberOnboarding(profile.id,{...d,completedAt:new Date().toISOString()}); if(d.startingWeightKg){await saveBodyCheck(profile.id,{...(body||{}),records:upsertBodyRecord(body?.records||[],{id:"onboarding_start",date:today,weight:d.startingWeightKg,note:"온보딩 초기 체중"})});} onDone();}; const pick=(k,v)=>setD({...d,[k]:v}); return <div className="member-shell onboard"><style>{CSS+MEMBER_CSS}</style><div className="onbar"><i style={{width:`${(step+1)/7*100}%`}}/></div><section className="onbody"><p>TEO GYM 회원 온보딩</p><h1>{qs[step]}</h1>{step===0&&<div className="choice2">{["남성","여성"].map(x=><button className={d.gender===x?"sel":""} onClick={()=>pick("gender",x)}>{x}</button>)}</div>}{step===1&&<select className="bigselect" value={d.birthYear} onChange={e=>pick("birthYear",e.target.value)}>{Array.from({length:80},(_,i)=>new Date().getFullYear()-10-i).map(y=><option key={y}>{y}</option>)}</select>}{step===2&&<input className="bignum" type="number" value={d.heightCm} onChange={e=>pick("heightCm",e.target.value)} placeholder="cm"/>}{step===3&&<input className="bignum" type="number" value={d.startingWeightKg} onChange={e=>pick("startingWeightKg",e.target.value)} placeholder="kg"/>}{step===4&&<Choices vals={ONBOARDING_GOALS} cur={d.goal} onPick={v=>pick("goal",v)}/>} {step===5&&<Choices vals={ONBOARDING_FOCUS} cur={d.focusAreas} multi onPick={v=>pick("focusAreas",d.focusAreas.includes(v)?d.focusAreas.filter(x=>x!==v):[...d.focusAreas,v])}/>} {step===6&&<Choices vals={[1,2,3,4,5,6,7].map(n=>`주 ${n}회`)} cur={d.weeklyWorkoutCount} onPick={v=>pick("weeklyWorkoutCount",v)}/>}</section><button className="onnext" onClick={next}>다음</button></div>}
...
function MemberHome(p){return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">오늘의 상태를 한눈에 확인해요</p><div className="hero-card"><b>오늘 수업</b><span>{p.latest?`${p.latest.date} 공개 수업일지 도착`:'예정된 공개 수업을 기다리는 중'}</span></div><div className="grid2"><Metric t="남은 PT" v={p.remaining}/><Metric t="최근 체중 변화" v={`${p.startW} → ${p.curW}kg`}/><Metric t="최근 섭취 칼로리" v={p.recentKcal==='-'?'-':`${p.recentKcal}kcal`}/><Metric t="최근 걸음수" v={p.steps==='-'?'-':`${p.steps}보`}/></div><ReviewRoutine {...p}/><MCard title="다음 목표"><p>{p.onboarding.goal} · {p.onboarding.weeklyWorkoutCount}</p></MCard><MCard title="대표 피드백"><p>{p.latest?.trainerComment||"공개 수업일지의 대표 코멘트가 여기에 표시됩니다."}</p></MCard><MCard title="최근 운동 기록">{p.latest?<SessionMini s={p.latest}/>:<p>아직 공개 기록이 없습니다.</p>}</MCard></>}
...
function MemberProfile(p){return <><h1>프로필</h1><div className="profile-head"><div className="avatar">{p.profile.name?.[0]||'T'}</div><div><h2>{p.profile.name}</h2><p>{p.onboarding.goal}</p></div></div><MCard title="회원 정보"><Info l="운동 빈도" v={p.onboarding.weeklyWorkoutCount}/><Info l="가입일" v={String(p.profile.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||'-')}/><Info l="남은 PT" v={p.remaining}/><Info l="키/체중" v={`${p.onboarding.heightCm}cm · ${p.curW}kg`}/></MCard><button className="primary">대표에게 연락하기</button><button className="ghost" onClick={p.onLogout}>로그아웃</button></>}
function ReviewRoutine({sessions,onboarding,checkins}){const rec=buildReviewRoutine(sessions,onboarding,checkins); return <MCard title="오늘 혼자 운동한다면"><p className="notice">대표님과 함께 했던 운동을 기준으로 만든 복습 루틴입니다. 무리하지 말고 가능한 중량으로 진행해주세요.</p><h3>{rec.split}</h3>{rec.items.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><span>{x.sets}세트 · {x.reps}회 · {x.weight}</span><em>미완료</em></div>)}<p className="danger">통증이 있으면 즉시 중단하고 대표에게 알려주세요.</p><p>대표 코멘트: {rec.comment}</p></MCard>}
function buildReviewRoutine(sessions,onboarding,checkins){const lastCheck=checkins?.[0]||{}; const lower=lastCheck.soreness==='심함'||String(lastCheck.condition).includes('피곤'); const map=new Map(); sessions.slice(-8).forEach(s=>(s.exercises||[]).forEach(e=>{if(!e.name)return; const sets=e.sets||[]; const last=sets.findLast?.(x=>x.weight||x.reps)||sets[sets.length-1]||{}; map.set(e.name,{...e,last,count:(map.get(e.name)?.count||0)+1});})); const items=[...map.values()].sort((a,b)=>b.count-a.count).slice(0,5).map(e=>({name:e.name,sets:Math.max(1,Math.min(4,(e.sets||[]).length-(lower?1:0))),reps:e.last?.reps||'8~12',weight:e.last?.weight?`${Math.round(Number(e.last.weight)*(lower ? 0.8 : 0.9))}~${e.last.weight}kg`:'가능한 중량'})); return {split:onboarding?.focusAreas?.[0]||'최근 배운 운동 복습',items:items.length?items:[{name:'공개 수업일지 운동을 확인해주세요',sets:2,reps:'8~12',weight:'가볍게'}],comment:lower?'오늘은 컨디션을 고려해 세트 수와 강도를 낮췄어요.':'최근 수행한 중량의 80~100% 범위에서 안전하게 진행해요.'}}
...
function AdminMemberAppPanel({member,onAccountCreated}){
  ...
  const saveManualMemberUid=async()=>{
    ...
    try{
      ...
      onAccountCreated?.(patch);
      setManualUid("");
      setMsg(`회원 UID 수동 연결 완료 · 회원앱은 members.memberUid == auth.uid 조건으로 이 UID를 조회합니다: ${uid}`);
