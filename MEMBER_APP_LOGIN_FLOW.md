...
## 1. 회원 문서 준비
관리자 앱에서 회원을 등록할 때 `members/{memberId}` 문서에 회원 이메일을 저장합니다. 회원앱은 로그인한 Firebase Authentication 사용자의 UID와 `members.memberUid`가 일치하는 회원 문서만 조회합니다.
...
2. UID 매칭 문서가 없으면 회원앱 접근 권한이 없는 상태로 처리합니다. 이메일 일치만으로는 접근을 허용하지 않습니다.
