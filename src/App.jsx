...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const saveImportMode=async(key,value)=>{setBusy(true); setMsg(""); try{const now=new Date().toISOString(); const patch={...(ob||{}),[key]:value,[`${key}StartedAt`]:value==="fresh"?now:(ob?.[`${key}StartedAt`]||""),updatedAt:now}; await saveMemberOnboarding(memberId,patch); setOb(patch); setMsg(`${key==="weightHistoryMode"?"체중 기록":"섭취칼로리 기록"} ${value==="import"?"회원앱 전송 완료":"새로 시작 저장 완료"}`);}catch(e){setMsg(`표시 기준 저장 실패 · ${e?.message||String(e)}`);}finally{setBusy(false);}};
