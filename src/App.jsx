...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, saveSessionSoreness, saveMemberHealthInputs, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics,
} from "./db";
...
function MemberApp({ onLogout }) {
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,saveSoreness,onLogout,setTab,accessErrors};
...
function MemberWorkout({sessions}){return <><h1>수업일지</h1><p className="sub">대표님이 공개한 수업 결과 리포트입니다.</p>{sessions.slice().reverse().map(s=><MCard key={s.id} title={`${s.date} · ${formatTypes(s.selectedTypes||s.type)||'운동'}`}><SessionMini s={s}/></MCard>)}</>}
...
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); const inbody=p.body?.inbody||[]; const volume=p.sessions.reduce((a,s)=>a+(s.totalVolume||0),0); const trend=buildPartVolumeTrend(p.sessions); const partData=buildPartVolumeChange(p.sessions); const msg=partData.find(x=>x.delta>0); return <><h1>변화분석</h1><p className="sub">체중 변화 · 부위별 성장 추이 · 수행능력 변화를 중심으로 확인해요</p><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={190}><LineChart data={weightData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" name="체중" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><MCard title="부위별 볼륨 성장 추이"><p className="notice soft">부위끼리 비교하지 않고 같은 부위의 최근 3회 운동량 변화를 보여드려요.</p>{trend.map(t=><div className="part-trend" key={t.part}><b>{t.part}</b><ResponsiveContainer width="100%" height={120}><BarChart data={t.data}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name"/><YAxis hide/><Tooltip/><Bar dataKey="volume" name={`${t.part} 볼륨`} fill={mColor(t.part)} radius={[8,8,0,0]}/></BarChart></ResponsiveContainer></div>)}</MCard><MCard title="수행능력 변화">{buildPerformanceChanges(p.sessions).map(x=><div className="volume-card" key={x.name}><b>{x.name}</b><span>{x.before} → {x.after}</span></div>)}<p className="notice soft">{msg?`${msg.part} 운동량이 최근 기록에서 증가하고 있습니다.`:'기록이 쌓일수록 변화가 더 선명해집니다.'}</p></MCard><div className="grid2 secondary-metrics"><Metric t="골격근량" v={(inbody.at(-1)?.muscleMass)||'-'}/><Metric t="체지방" v={(inbody.at(-1)?.bodyFat)||'-'}/></div><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard><MCard title="참고 정보"><Info l="누적 운동량" v={`${Math.round(volume).toLocaleString()}kg`}/></MCard></>}
