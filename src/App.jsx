...
function MemberFeedbackForm({s,onSave}){const existing=s.memberFeedback||{}; const [d,setD]=useState({sorenessLevel:existing.sorenessLevel||"없음",sorenessBodyPart:existing.sorenessBodyPart||"등",rpe:Number.isFinite(Number(existing.rpe))?Number(existing.rpe):5,memo:existing.memo||""}); useEffect(()=>setD({sorenessLevel:existing.sorenessLevel||"없음",sorenessBodyPart:existing.sorenessBodyPart||"등",rpe:Number.isFinite(Number(existing.rpe))?Number(existing.rpe):5,memo:existing.memo||""}),[s.id,existing.sorenessLevel,existing.sorenessBodyPart,existing.rpe,existing.memo]); return <div className="member-feedback-form"><h3>수업 후 상태 기록</h3><SelectLine label="근육통" value={d.sorenessLevel} opts={SORENESS_LEVELS} onChange={v=>setD({...d,sorenessLevel:v})}/><SelectLine label="근육통 부위" value={d.sorenessBodyPart} opts={SORENESS_BODY_PARTS} onChange={v=>setD({...d,sorenessBodyPart:v})}/><div className="form-line"><label>RPE <small>RPE = Rate of Perceived Exertion</small><small>운동이 얼마나 힘들게 느껴졌는지 0~10점으로 표시해주세요.</small><small>0점 = 전혀 힘들지 않음 · 5점 = 보통 · 10점 = 더 이상 반복하기 어려움</small></label><div className="vas-buttons">{Array.from({length:11},(_,i)=><button type="button" key={i} className={Number(d.rpe)===i?"active":""} onClick={()=>setD({...d,rpe:i})}>{i}</button>)}</div></div><InputLine label="메모" value={d.memo} onChange={v=>setD({...d,memo:v})}/><button className="primary compact" onClick={()=>onSave?.(s.id,d)}>수업 후 상태 저장</button></div>}
              ...
              <div key={s.id||i}
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {s.memberFeedback && (
                      <Mo c="#22E0C2" s={9} style={{display:"block",marginBottom:4}}>회원 수업 후 상태: {s.memberFeedback.sorenessLevel||"-"} · {s.memberFeedback.sorenessBodyPart||"-"} · RPE {s.memberFeedback.rpe ?? "-"}{s.memberFeedback.memo?` · ${s.memberFeedback.memo}`:""}</Mo>
                    )}
