...
function MemberApp({ onLogout }) {
  const C=MEMBER_COLORS; const today=new Date().toISOString().slice(0,10); const [tab,setTab]=useState("home"); const [profile,setProfile]=useState(null); const [sessions,setSessions]=useState([]); const [body,setBody]=useState(null); const [nutrition,setNutrition]=useState(null); const [checkins,setCheckins]=useState([]); const [messages,setMessages]=useState([]); const [onboarding,setOnboarding]=useState(null); const [loading,setLoading]=useState(true); const [memberError,setMemberError]=useState(""); const [form,setForm]=useState({date:today,weight:"",condition:"보통",soreness:"없음"}); const [memberErrorDetails,setMemberErrorDetails]=useState(null);
  const withTimeout=(promise,ms,msg)=>Promise.race([promise,new Promise((_,reject)=>setTimeout(()=>reject(new Error(msg)),ms))]);
  const load=useCallback(async()=>{setLoading(true); setMemberError(""); setMemberErrorDetails(null); try{ console.group("[MemberApp] 최초 진입 읽기 순서"); console.log("0) Firebase Auth", {uid:auth.currentUser?.uid||null,email:auth.currentUser?.email||null}); const p=await withTimeout(getMemberAppProfile(),5000,"로그인 후 회원 정보를 불러오지 못했습니다. 잠시 후 다시 시도하거나 대표에게 문의해주세요."); setProfile(p); if(!p){ setMemberError("회원 정보를 찾을 수 없습니다. 대표에게 문의해주세요."); setMemberErrorDetails({code:"member/not-found",path:"members where memberUid == auth.uid",authUid:auth.currentUser?.uid||null,authEmail:auth.currentUser?.email||null}); return; } console.log("1) members 프로필", {path:`members/${p.id}`, authUid:auth.currentUser?.uid||null, authEmail:auth.currentUser?.email||null, memberUid:p.memberUid||null, memberEmail:p.email||null, match:p.memberUid===auth.currentUser?.uid}); const readStep=async(label,collectionName,path,fn)=>{ console.log(`${label}) 읽기 시작`, {collection:collectionName,path}); try{ const data=await withTimeout(fn(),5000,`${path} 읽기 시간이 초과됐습니다.`); console.log(`${label}) 읽기 성공`, {collection:collectionName,path,count:Array.isArray(data)?data.length:(data?1:0)}); return data; }catch(e){ const details={collection:collectionName,path,code:e?.code||"unknown",message:e?.message||String(e),authUid:auth.currentUser?.uid||null,authEmail:auth.currentUser?.email||null,memberUid:p.memberUid||null,memberEmail:p.email||null}; console.error(`${label}) permission-denied/읽기 실패`, details); e.memberAppDetails=details; throw e; } }; const ss=await readStep("2","sessions",`members/${p.id}/sessions (isPublished == true)`,()=>getPublishedSessions(p.id)); const bd=await readStep("3","bodyCheck",`members/${p.id}/bodyCheck/main`,()=>getBodyCheck(p.id)); const nt=await readStep("4","nutrition",`members/${p.id}/nutrition`,()=>getNutrition(p.id)); const ci=await readStep("5","memberCheckins",`members/${p.id}/memberCheckins`,()=>getMemberCheckins(p.id)); const ms=await readStep("6","memberMessages",`members/${p.id}/memberMessages`,()=>getMemberMessages(p.id)); const ob=await readStep("7","memberOnboarding",`members/${p.id}/memberOnboarding/main`,()=>getMemberOnboarding(p.id)); setSessions(ss); setBody(bd); setNutrition(nt); setCheckins(ci); setMessages(ms); setOnboarding(ob); const w=getLatestBodyWeight(bd)?.weight||p.weight||p.startWeight||""; setForm(f=>({...f,weight:w})); }catch(e){ console.error("[MemberApp] load failed:",e); setMemberError(e.message||"회원앱 정보를 불러오지 못했습니다."); setMemberErrorDetails(e.memberAppDetails||{code:e?.code||"unknown",message:e?.message||String(e),authUid:auth.currentUser?.uid||null,authEmail:auth.currentUser?.email||null}); }finally{ console.groupEnd?.(); setLoading(false); }},[]); useEffect(()=>{load();},[load]);
  if(loading) return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><Spin/></div>;
  if(memberError||!profile) return <MemberAppError message={memberError||"회원 정보를 찾을 수 없습니다. 대표에게 문의해주세요."} details={memberErrorDetails} onRetry={load} onLogout={onLogout}/>;
  if(!onboarding?.completedAt) return <MemberOnboarding profile={profile} body={body} existing={onboarding} onDone={load}/>;
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding,curW,startW,remaining,latest,recentKcal,steps,form,setForm,saveCheck,onLogout,setTab};
  return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><main className="member-page">{tab==="home"&&<MemberHome {...common}/>} {tab==="workout"&&<MemberWorkout {...common}/>} {tab==="health"&&<MemberHealth {...common}/>} {tab==="analysis"&&<MemberAnalysis {...common}/>} {tab==="profile"&&<MemberProfile {...common}/>}</main><nav className="member-nav">{[["home","⌂","홈"],["workout","＋","운동기록"],["health","♥","건강관리"],["analysis","▥","변화분석"],["profile","●","프로필"]].map(([k,i,l])=><button key={k} onClick={()=>setTab(k)} className={tab===k?"active":""}><span>{i}</span>{l}</button>)}</nav></div>;
}

function MemberAppError({message,details,onRetry,onLogout}){const isAuthMissing=details?.code==="auth/user-not-found"; return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><div className="member-page"><MCard title="회원앱을 열 수 없습니다"><p className="danger">{message}</p>{details&&<div className="notice"><b>오류 코드:</b> {details.code||"unknown"}{details.path&&<><br/><b>실패 경로:</b> {details.path}</>}{details.authUid&&<><br/><b>Auth UID:</b> {details.authUid}</>}{details.authEmail&&<><br/><b>Auth email:</b> {details.authEmail}</>}{details.memberUid&&<><br/><b>members.memberUid:</b> {details.memberUid}</>}{details.memberEmail&&<><br/><b>members.email:</b> {details.memberEmail}</>}</div>}<p className="notice">{isAuthMissing?"회원 이메일은 등록되어 있지만 Firebase Authentication 계정이 없습니다. 대표에게 [회원앱 초대 보내기]를 요청해주세요.":"Firebase Authentication 로그인은 완료됐지만 Firestore 권한 또는 데이터 조회 단계에서 실패했습니다. 위 오류 코드와 실패 경로를 확인해주세요."}</p><button className="primary" onClick={onRetry}>다시 시도</button><button className="ghost" onClick={onLogout}>로그아웃</button></MCard></div></div>}
