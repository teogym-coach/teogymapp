...
function CalorieTrendChart({rows,chartRows,admin=false}){const tooltipStyle=admin?{background:"#111827",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,fontFamily:"'DM Mono',monospace",fontSize:11,color:"#e2e8f0"}:{background:"#FFFFFF",border:"1px solid #E8ECF1",borderRadius:12,boxShadow:"0 10px 28px rgba(32,36,42,.08)",fontSize:12,color:"#20242A"}; const axisTick=admin?{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}:{fontSize:11,fill:"#8B949E",fontWeight:800}; if(!rows.length)return <div style={{height:admin?190:180,display:"grid",placeItems:"center",border:`1px dashed ${admin?"rgba(255,255,255,0.12)":"#D9E7FF"}`,borderRadius:admin?8:18,background:admin?"#0B1120":"#F6F7F9"}}><Mo c={admin?"#54546a":"#8B949E"} s={10}>섭취 칼로리 기록 수집 중</Mo></div>; return <ResponsiveContainer width="100%" height={admin?190:190}><ComposedChart data={chartRows} margin={{top:8,right:admin?14:8,left:admin?-18:-14,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke={admin?"rgba(255,255,255,0.08)":"#EEF1F4"}/><XAxis dataKey="date" tick={axisTick} tickLine={false} axisLine={{stroke:admin?"rgba(255,255,255,0.08)":"#E8ECF1"}}/><YAxis tick={axisTick} tickLine={false} axisLine={{stroke:admin?"rgba(255,255,255,0.08)":"#E8ECF1"}} unit="kcal"/><Tooltip contentStyle={tooltipStyle} formatter={(v,n)=>[formatKcal(v),n]}/><Legend wrapperStyle={{fontFamily:admin?"'DM Mono',monospace":undefined,fontSize:admin?8:11,color:admin?"#94a3b8":"#66717C",fontWeight:800}}/><Bar dataKey="kcal" name="일별 섭취" fill={admin?"#5EEAD4":"#93C5FD"} radius={[5,5,0,0]} opacity={admin?0.35:0.28}/><Line type="monotone" dataKey="avg7" name="7일 평균" stroke={admin?"#5EEAD4":"#2F73F6"} strokeWidth={admin?2.5:4} dot={false}/><Line type="monotone" dataKey="avg30" name="30일 평균" stroke={admin?"#7c6fff":"#14B8A6"} strokeWidth={admin?2:3} dot={false}/><Line type="monotone" dataKey="target" name="추천 기준" stroke={admin?"#ff6b6b":"#94A3B8"} strokeWidth={admin?1.5:2} strokeDasharray="4 4" dot={false}/></ComposedChart></ResponsiveContainer>}
function CalorieTrendCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const rows=getKcalLogs(nutrition).slice(-30).map(x=>({date:x.date.slice(5),kcal:x.kcal,target:target.value})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; const m7=mean(avg7),m30=mean(avg30); const chartRows=rows.map(r=>({...r,avg7:m7,avg30:m30})); const items=[{label:"최근 7일 평균",value:formatKcal(m7),color:admin?"#54a0ff":"#2F73F6"},{label:"최근 30일 평균",value:formatKcal(m30),color:admin?"#5EEAD4":"#14B8A6"},{label:"유지칼로리",value:formatKcal(a.maintenance),color:admin?"#ffd166":"#20242A"},{label:"권장섭취",value:formatKcal(target.value),sub:target.shortLabel||target.label,color:admin?"#22c55e":"#16A34A"}]; const content=<><CalorieTrendChart rows={rows} chartRows={chartRows} admin={admin}/><CalorieKpiGrid items={items} admin={admin}/></>; return admin?<Card title="📈 섭취 칼로리 변화 그래프" style={{marginBottom:11}}>{content}</Card>:<MCard title="섭취 칼로리 변화 그래프">{content}</MCard>}
