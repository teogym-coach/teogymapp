...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, saveSessionSoreness, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics,
} from "./db";
...
const MEMBER_WORKOUT_PARTS = ["등","가슴","어깨","팔","하체","코어","유산소","교정"];
...
function MemberApp({ onLogout }) {
  ...
  const weights=getBodyWeightRecords(body), curW=weights.at(-1)?.weight||form.weight||"-", startW=weights[0]?.weight||curW; const totalReg=parseInt(String(profile.totalSessions||"").replace(/[^0-9]/g,""))||0; const remaining=totalReg?Math.max(0,totalReg-sessions.length):"-"; const latest=sessions.at(-1); const recentKcal=(nutrition?.logs||[]).at(-1)?.kcal||"-"; const steps=checkins.find(c=>c.steps)?.steps||"-";
  const saveCheck=async()=>{await saveMemberCheckin(profile.id,today,form); await load(); alert("오늘 상태가 저장됐어요");};
...
function SelectLine({label,value,opts,onChange}){return <div style={{margin:"12px 0"}}><label>{label}</label><select value={value} onChange={e=>onChange(e.target.value)}>{opts.map(o=><option key={o}>{o}</option>)}</select></div>}
function MemberHome(p){const total=Number(p.totalReg)||0; const done=p.sessions.length; const percent=total?Math.min(100,Math.round(done/total*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">오늘의 상태를 한눈에 확인해요</p><div className="hero-card"><b>오늘 수업</b><span>{p.latest?`${p.latest.date} 공개 수업일지 도착`:'예정된 공개 수업을 기다리는 중'}</span></div><div className="grid2"><div className="metric pt-progress"><span>PT 진행 현황</span><b>{total?`${done} / ${total}회 진행`:`${done}회 진행`}</b><em>{total?`남은 ${p.remaining}회`:'대표님과 함께한 공개 수업 기준'}</em>{total>0&&<i><small style={{width:`${percent}%`}}/></i>}</div><Metric t="최근 체중 변화" v={`${p.startW} → ${p.curW}kg`}/><Metric t="최근 섭취 칼로리" v={p.recentKcal==='-'?'아직 기록 전':`${p.recentKcal}kcal`}/><Metric t="최근 걸음수" v={p.steps==='-'?'기록하면 반영돼요':`${p.steps}보`}/></div><ReviewRoutine {...p}/><MCard title="다음 목표"><p>{p.onboarding.goal} · {p.onboarding.weeklyWorkoutCount}</p></MCard><MCard title="대표 피드백"><p>{p.latest?.trainerComment||"공개 수업일지의 대표 코멘트가 여기에 표시됩니다."}</p></MCard><MCard title="최근 운동 기록">{p.latest?<SessionMini s={p.latest}/>:<p>아직 공개 기록이 없습니다.</p>}</MCard></>}
...
function MemberHealth(p){return <><h1>건강관리</h1><div className="grid2"><Metric t="체중" v={`${p.curW}kg`}/><Metric t="칼로리" v={p.recentKcal}/><Metric t="걸음수" v={p.steps}/><Metric t="목표" v={p.onboarding.goal}/></div><MCard title="오늘 상태 체크"><SelectLine label="컨디션" value={p.form.condition} opts={["좋음","보통","피곤","매우 피곤"]} onChange={v=>p.setForm({...p.form,condition:v})}/><SelectLine label="통증/근육통" value={p.form.soreness} opts={["없음","약간","심함"]} onChange={v=>p.setForm({...p.form,soreness:v})}/><button className="primary" onClick={p.saveCheck}>저장</button></MCard>{["영양제","인바디","캘린더","설문"].map(x=><MCard key={x} title={x}><p>관리자 건강관리 허브 데이터와 함께 확인합니다.</p></MCard>)}</>}
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); const inbody=p.body?.inbody||[]; const volume=p.sessions.reduce((a,s)=>a+(s.totalVolume||0),0); const partData=buildPartVolumeChange(p.sessions); const msg=partData.find(x=>x.delta>0); return <><h1>변화분석</h1><p className="sub">운동 수행능력 변화를 중심으로 확인해요</p><MCard title="부위별 평균 볼륨 변화">{partData.map(x=><div className="volume-card" key={x.part}><b>{x.part} 평균 볼륨</b><span>1주차 {Math.round(x.first).toLocaleString()}kg → 최근 {Math.round(x.recent).toLocaleString()}kg</span></div>)}<p className="notice soft">{msg?`${msg.part} 운동 볼륨이 꾸준히 증가하고 있습니다.`:'기록이 쌓일수록 변화가 더 선명하게 보입니다.'}</p></MCard><MCard title="부위별 볼륨 변화 그래프"><ResponsiveContainer width="100%" height={220}><LineChart data={buildPartVolumeChart(p.sessions)}><XAxis dataKey="name"/><YAxis/><Tooltip/><Legend/>{['가슴','등','하체','어깨','팔','코어'].map((k,i)=><Line key={k} dataKey={k} stroke={['#2F73F6','#16C784','#8B5CF6','#ff9f43','#ff6b6b','#5EEAD4'][i]} strokeWidth={3} dot={{r:3}}/> )}</LineChart></ResponsiveContainer></MCard><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={180}><LineChart data={weightData}><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><div className="grid2 secondary-metrics"><Metric t="골격근량" v={(inbody.at(-1)?.muscleMass)||'-'}/><Metric t="체지방" v={(inbody.at(-1)?.bodyFat)||'-'}/><Metric t="출석률" v={`${p.remaining==='-'?0:Math.round(p.sessions.length/(p.sessions.length+p.remaining||1)*100)}%`}/><Metric t="운동 횟수" v={`${p.sessions.length}회`}/><Metric t="누적 운동량" v={`${Math.round(volume).toLocaleString()}kg`}/></div><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard></>}
function MemberProfile(p){return <><h1>프로필</h1><div className="profile-head"><div className="avatar">{p.profile.name?.[0]||'T'}</div><div><h2>{p.profile.name}</h2><p>{p.onboarding.goal}</p></div></div><MCard title="회원 정보"><Info l="운동 빈도" v={p.onboarding.weeklyWorkoutCount}/><Info l="가입일" v={String(p.profile.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||'-')}/><Info l="PT 진행 현황" v={p.totalReg?`${p.sessions.length}/${p.totalReg}회 · 남은 ${p.remaining}회`:`${p.sessions.length}회 진행`}/><Info l="키/체중" v={`${p.onboarding.heightCm}cm · ${p.curW}kg`}/></MCard><button className="primary" onClick={()=>alert("대표에게 문의해주세요.")}>대표에게 연락하기</button><button className="ghost" onClick={p.onLogout}>로그아웃</button></>}
...
function formatExerciseDose(e,lower){const sets=e.sets||[]; const last=sets.findLast?.(x=>x.weight||x.reps)||sets[sets.length-1]||{}; const weights=sets.map(x=>Number(x.weight)).filter(Number.isFinite); const minW=Math.min(...weights), maxW=Math.max(...weights); return {sets:Math.max(1,Math.min(4,sets.length-(lower?1:0)||3)),reps:last.reps||"8~12",weight:weights.length?`${minW===maxW?minW:`${minW}~${maxW}`}kg`:"가능한 중량"};}
function buildReviewRoutine(sessions,onboarding,checkins,selectedPart){const lastCheck=checkins?.[0]||{}; const lower=lastCheck.soreness==='심함'||String(lastCheck.condition).includes('피곤'); const cutoff=new Date(Date.now()-14*86400000).toISOString().slice(0,10); const map=new Map(); sessions.forEach(s=>(s.exercises||[]).forEach(e=>{if(!e.name||!exerciseMatchesPart(e,selectedPart))return; const prev=map.get(e.name)||{...e,count:0,recent:false,stim:0,painFree:0,latestDate:""}; const feedback=String(e.feedback||""); const stim=/자극|좋|잘|우수|높/.test(feedback)?1:0; const pain=/통증|불편|아픔|저림/.test(feedback)?1:0; map.set(e.name,{...prev,...e,count:prev.count+1,recent:prev.recent||(s.date||"")>=cutoff,stim:prev.stim+stim,painFree:prev.painFree+(pain?0:1),latestDate:String(s.date||prev.latestDate)});})); const list=[...map.values()]; const sorted=[...list].sort((a,b)=>(b.stim-a.stim)||(b.painFree-a.painFree)||(a.count-b.count)||((a.recent?1:0)-(b.recent?1:0))||(b.count-a.count)); const fallback={등:["스트레이트 암풀다운","랫풀다운","시티드 케이블로우","페이스풀"],가슴:["체스트프레스","인클라인 덤벨프레스","케이블 플라이","푸시업"],어깨:["숄더프레스","사이드 레터럴 레이즈","리어델트 플라이","페이스풀"],팔:["케이블 푸시다운","덤벨 컬","해머 컬","오버헤드 익스텐션"],하체:["레그프레스","레그컬","힙브릿지","카프레이즈"],코어:["데드버그","플랭크","버드독","팔로프프레스"],유산소:["트레드밀 걷기","실내자전거","가벼운 인터벌"],교정:["흉추 회전","밴드 풀어파트","고관절 스트레칭","호흡 패턴 연습"]}; const base=(fallback[selectedPart]||fallback.등).map(name=>({name,sets:3,reps:"10~12",weight:"가능한 중량"})); const itemFor=e=>({name:e.name,...formatExerciseDose(e,lower)}); return {selectedPart,hasData:list.length>0,goodStim:sorted.filter(e=>e.stim>0).slice(0,2),painFree:sorted.filter(e=>e.painFree>0).slice(0,2),practice:sorted.filter(e=>e.count<3||!e.recent).slice(0,2),routine:(sorted.length?sorted.slice(0,4).map(itemFor):base),comment:lower?'오늘은 컨디션을 고려해 세트 수와 강도를 낮췄어요.':'컨디션이 좋지 않다면 중량을 낮춰 진행해주세요.'};}
function ReviewRoutine({profile,sessions,onboarding,checkins}){const nextPart=profile?.nextPtPart&&profile.nextPtPart!=="미정"?profile.nextPtPart:""; const [showAll,setShowAll]=useState(false); const [part,setPart]=useState(""); const parts=showAll?MEMBER_WORKOUT_PARTS:MEMBER_WORKOUT_PARTS.filter(x=>x!==nextPart); const rec=part?buildReviewRoutine(sessions,onboarding,checkins,part):null; return <MCard title="대표님 추천 개인운동"><p className="notice">대표님과 함께한 수업 기록을 기준으로 만든 개인운동 추천입니다.<br/>오늘 운동할 부위를 선택해주세요.</p>{nextPart&&<p className="notice soft">다음 PT 예정 부위: {nextPart}<br/>{nextPart}는 다음 PT에서 진행 예정입니다.<br/>오늘은 다른 부위를 선택해보세요.</p>}<div className="part-pills">{parts.map(x=><button key={x} className={part===x?"active":""} onClick={()=>setPart(x)}>{x}</button>)}</div>{nextPart&&<button className="linkbtn" onClick={()=>setShowAll(v=>!v)}>{showAll?"추천 부위만 보기":"전체 부위 보기"}</button>}{rec&&<><h3>{part} 운동 추천</h3><p className="notice soft">대표님과 함께했던 {part} 운동 기록을 기준으로 추천합니다.</p>{!rec.hasData&&<p className="notice soft">아직 이 부위 기록이 많지 않아 기본 루틴을 추천드립니다.</p>}<RecommendationGroup title="자극이 좋았던 운동" items={rec.goodStim} desc="최근 자극도 높음 / 불편감 기록 없음"/><RecommendationGroup title="불편감 없이 수행한 운동" items={rec.painFree} desc="최근 통증 메모 없음"/><RecommendationGroup title="연습이 필요한 운동" items={rec.practice} desc="최근 수행 횟수 적음 / 자세 연습 필요"/><h3>오늘 추천 루틴</h3>{rec.routine.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><span>{x.sets}세트 · {x.reps}회 · {x.weight}</span><em>{rec.hasData?"최근 수업 기준":"기본 안전 루틴"}</em></div>)}<p className="danger">통증이 있으면 즉시 중단하고 대표에게 알려주세요.<br/>컨디션이 좋지 않다면 중량을 낮춰 진행해주세요.</p><p>대표 코멘트: {rec.comment}</p></>}</MCard>}
function RecommendationGroup({title,items,desc}){if(!items?.length)return null; return <div className="rec-group"><h3>{title}</h3>{items.map(x=><p key={x.name}><b>{x.name}</b><span>{desc}</span></p>)}</div>}
...
function buildPartVolumeChart(sessions=[]){return [...sessions].sort((a,b)=>(a.date||'').localeCompare(b.date||'')).map((s,i)=>{const row={name:s.sessionNo?`${s.sessionNo}회`:`${i+1}회`}; ['가슴','등','하체','어깨','팔','코어'].forEach(part=>{row[part]=(s.exercises||[]).filter(e=>normalizeWorkoutPart(e.muscleTop)===part).reduce((x,e)=>x+(e.sets||[]).reduce((v,st)=>v+(Number(st.volume)||Number(st.weight||0)*Number(st.reps||0)),0),0);}); return row;}).slice(-8);}
