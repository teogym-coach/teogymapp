  ...
  if (showCard) {
    ...
    async function saveCardAsImage() {
      ...
      try {
        ...
        const name = member.name + "_" + sessionNo + "회차_" + date + ".png";
        link.download = name;
        link.href = canvas.toDataURL("image/png");
        link.click();
        showToast("이미지 저장 완료 ✓");
      } catch(e) {
        // iOS Safari는 download 속성 미지원 → 새 탭에서 열기
        try {
          const html2canvas = (await import("html2canvas")).default;
          const canvas = await html2canvas(el, { backgroundColor:"#0d0d10", scale:2, useCORS:true, logging:false });
          const img = canvas.toDataURL("image/png");
          const w = window.open();
          if (w) {
            w.document.write('<img src="'+img+'" style="max-width:100%;"/>');
            w.document.title = member.name+"_수업요약";
            showToast("새 탭에서 이미지를 길게 눌러 저장하세요");
          }
        } catch(e2) {
          showToast("저장 실패: 스크린샷을 이용해주세요","err");
        }
      }
    }
    return (
      <div>
        <SH title="📸 수업 요약 카드" sub="이미지로 저장 후 회원에게 전송"
        ...
        <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
          <button onClick={saveCardAsImage}
            style={{width:"100%",padding:"14px",borderRadius:10,border:"none",
              background:"linear-gradient(135deg,#00e5a0,#00b37e)",
              color:"#09090c",fontFamily:"'Syne',sans-serif",fontWeight:800,
          ...
          <div style={{padding:"10px 14px",background:"#111116",borderRadius:8,
            border:"1px dashed #1a1a24",fontSize:11,color:"#54546a",textAlign:"center",lineHeight:1.8}}>
            💡 <strong style={{color:"#ddddf0"}}>아이폰/아이패드</strong>에서는 이미지가 새 탭에서 열립니다.<br/>
            열린 이미지를 <strong style={{color:"#ddddf0"}}>길게 눌러</strong> 사진 앱에 저장하세요.
          </div>
        </div>
      </div>
    );
