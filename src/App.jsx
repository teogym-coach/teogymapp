...
function MemberApp({ onLogout }) {
  ...
  const saveCheck=async()=>{await saveMemberCheckin(profile.id,today,form); await saveMemberHealthInputs(profile.id,today,{weight:form.weight,kcal:form.kcal,steps:form.steps}); await load(); alert("건강관리 기록이 저장됐어요");};
...
function formatExerciseDose(e,lower){const sets=getFilledSets(e); const last=sets.findLast?.(x=>x.weight||x.reps)||sets[sets.length-1]||{}; const weights=sets.map(x=>Number(x.weight)).filter(n=>Number.isFinite(n)&&n>0); const minW=Math.min(...weights), maxW=Math.max(...weights); return {sets:Math.max(3,Math.min(4,sets.length-(lower?1:0)||3)),reps:last.reps||"10~12",weight:weights.length?`${minW===maxW?minW:`${minW}~${maxW}`}kg`:"가능한 중량"};}
function isTrainerMarkedExercise(e){return !!(e.isFavorite||e.favorite||e.isRecommended||e.recommended||e.memberAppRecommended);}
function buildReviewRoutine(sessions,onboarding,checkins,selectedPart){const lastCheck=checkins?.[0]||{}; const lower=['강함','매우 강함','심함'].includes(lastCheck.soreness)||String(lastCheck.condition).includes('피곤'); const cutoff=new Date(Date.now()-14*86400000).toISOString().slice(0,10); const map=new Map(); sessions.forEach(s=>(s.exercises||[]).forEach(e=>{const filled=getFilledSets(e); if(!e.name||!exerciseMatchesPart(e,selectedPart)||(!isTrainerMarkedExercise(e)&&filled.length<3))return; const prev=map.get(e.name)||{...e,count:0,recent:false,stim:0,painFree:0,latestDate:""}; const feedback=String(e.feedback||""); const stim=/자극|좋|잘|우수|높/.test(feedback)?1:0; const pain=/통증|불편|아픔|저림/.test(feedback)?1:0; map.set(e.name,{...prev,...e,count:prev.count+1,recent:prev.recent||(s.date||"")>=cutoff,stim:prev.stim+stim,painFree:prev.painFree+(pain?0:1),marked:prev.marked||isTrainerMarkedExercise(e),latestDate:String(s.date||prev.latestDate)});})); const list=[...map.values()]; const sorted=[...list].sort((a,b)=>(b.marked-a.marked)||(b.stim-a.stim)||(b.painFree-a.painFree)||((a.recent?1:0)-(b.recent?1:0))||(b.count-a.count)); const fallback={등:["스트레이트 암풀다운","랫풀다운","시티드 케이블로우","페이스풀"],가슴:["체스트프레스","인클라인 덤벨프레스","케이블 플라이","푸시업"],어깨:["숄더프레스","사이드 레터럴 레이즈","리어델트 플라이","페이스풀"],팔:["케이블 푸시다운","덤벨 컬","해머 컬","오버헤드 익스텐션"],하체:["레그프레스","레그컬","힙브릿지","카프레이즈"]}; const base=(fallback[selectedPart]||fallback.등).map(name=>({name,sets:3,reps:"10~12",weight:"가능한 중량"})); const itemFor=e=>({name:e.name,...formatExerciseDose(e,lower),reason:e.stim>0?'자극이 잘 왔던 운동':e.painFree>0?'통증 없이 진행했던 운동':'연습이 필요한 운동'}); return {selectedPart,hasData:list.length>0,goodStim:sorted.filter(e=>e.stim>0).slice(0,2),painFree:sorted.filter(e=>e.painFree>0).slice(0,2),practice:sorted.filter(e=>e.count<3||!e.recent).slice(0,2),routine:(sorted.length?sorted.slice(0,4).map(itemFor):base),comment:lower?'오늘은 컨디션을 고려해 세트 수와 강도를 낮췄어요.':'컨디션이 좋지 않다면 중량을 낮춰 진행해주세요.'};}
function ReviewRoutine({profile,sessions,onboarding,checkins}){const nextPart=profile?.nextPtPart&&profile.nextPtPart!=="미정"?normalizeWorkoutPart(profile.nextPtPart):"하체";  const recommendedDefault=nextPart==="하체"?"상체":nextPart?"상체/반대 부위":"상체"; const [selected,setSelected]=useState(nextPart==="하체"?"가슴":"하체"); const rec=buildReviewRoutine(sessions,onboarding,checkins,selected); return <MCard title="오늘 추천 루틴 · 부위 선택형"><div className="next-part"><span>다음 PT 예정 부위</span><b>{nextPart}</b><em>다음 수업 전까지는 {recommendedDefault} 운동을 추천합니다.</em></div><div className="part-pills">{["가슴","등","하체","어깨","팔"].map(x=><button key={x} className={selected===x?"active":""} onClick={()=>setSelected(x)}>{x}</button>)}</div><h3>{selected} 운동 추천</h3><p className="notice soft">기록 기반 우선순위: 1) 자극이 잘 왔던 운동 2) 통증이 없었던 운동 3) 연습이 필요한 운동. 기본은 3세트 이상 수행 운동만 추천하고, 대표가 표시한 운동은 예외로 포함합니다.</p>{!rec.hasData&&<p className="notice soft">아직 이 부위 3세트 이상 기록이 많지 않아 기본 루틴을 추천드립니다.</p>}{rec.routine.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><span>{x.sets}세트 × {x.reps}회</span><em>{x.reason||"기본 추천"} · 추천중량 {x.weight}</em></div>)}<p className="danger">통증이 있으면 즉시 중단하고 대표에게 알려주세요.<br/>컨디션이 좋지 않다면 중량을 낮춰 진행해주세요.</p></MCard>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated}){
  ...
  const sendInvite=async()=>{
    ...
    if(!email){setMsg("회원 이메일을 먼저 저장해주세요."); setInviteLog([{ok:false,text:"회원 문서에 이메일이 없어 중단했습니다.",at:new Date().toLocaleTimeString()}]); return;}
    setBusy(true); setMsg(""); setInviteLog([]); let prepared=null;
    try{
      addLog(true,`이메일 확인 완료: ${email}`);
      prepared=await createMemberAuthAccountIfNeeded(email);
      addLog(true,prepared.created?`Firebase Auth 사용자 생성 완료 · UID: ${prepared.uid}`:"Firebase Auth 사용자 이미 존재");
      await sendPasswordResetEmail(auth,email);
      addLog(true,"비밀번호 설정/재설정 메일 발송 완료");
      const resolvedUid=prepared.uid||existingMemberUid||"";
      const now=new Date().toISOString();
      const patch={
        memberAppAccountEmail:email,
        memberAppInviteSentAt:now,
        memberAppPasswordResetSentAt:now,
        memberAppAccountStatus:resolvedUid?"available":"auth-exists-missing-uid",
        memberAppLastInviteLog:{ok:true,code:prepared.code,uid:resolvedUid||null,at:now}
      };
      if(resolvedUid) patch.memberUid=resolvedUid;
      await updateMember(memberId,patch);
      if(prepared.uid){
        addLog(true,`members.memberUid 저장 완료: ${prepared.uid}`);
        setMsg("회원앱 초대를 발송했습니다. Firebase Auth 생성, 비밀번호 설정 메일 발송, memberUid 저장이 완료되었습니다.");
      }else if(existingMemberUid){
...
export default function App() {
  ...
  async function handleNormalizeEmails() {
    if (!window.confirm("email이 있는 기존 회원 문서를 trim().toLowerCase()로 정규화합니다. 계속하시겠습니까?")) return;
    setMigrating(true);
    try {
      const result = await migrateNormalizeMemberEmails();
      showToast(result.count > 0 ? `${result.count}명 이메일 정규화 완료 ✓` : `정규화할 이메일 없음 (${result.skipped}명 확인)`);
      setMembers(await getMembers());
      if (member?.id) setMember(prev => prev ? ({...prev, email:(prev.email||"").trim().toLowerCase()}) : prev);
    } catch(e) {
      showToast("실패: " + e.message, "err");
      console.error("[EMAIL MIGRATION] 실패:", e);
    }
    setMigrating(false);
  }
    ...
    <div style={{minHeight:"100vh",background:"#0B1120"}}>
      ...
      <nav className="noprint" style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={handleNormalizeEmails} disabled={migrating}
            title="회원 이메일 trim().toLowerCase() 정규화"
            style={{background:"none",border:"1px solid rgba(94,234,212,.22)",borderRadius:6,color:"#5EEAD4",fontSize:10,fontWeight:700,padding:"5px 9px"}}>
            이메일 정규화
          </button>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            ...
            <div>
              ...
              <Mo c="#94a3b8" s={10} style={{display:"block",marginTop:2}}>memberId: {member.id||"-"}</Mo>
              <Mo c="#64748b" s={10} style={{display:"block",marginTop:2}}>email: {member.email||"-"}</Mo>
              <Mo c="#64748b" s={10} style={{display:"block",marginTop:2}}>memberUid: {member.memberUid||"-"}</Mo>
              <Mo c="#64748b" s={10} style={{display:"block",marginTop:2}}>trainerUid: {member.trainerUid||"-"}</Mo>
...
function HealthRecordTab({ member, bodyData, nutritionData, onSaveBodyData, onSaveNutrition, showToast }) {
  const today = new Date().toISOString().split("T")[0];
