...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, saveSessionSoreness, saveSessionMemberFeedback, saveMemberHealthInputs, saveMemberProfileFields, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics,
} from "./db";
...
function MemberApp({ onLogout }) {
  ...
  const saveCheck=async()=>{const weightValue=String(form.weight??"").trim(); const parsedWeight=weightValue===""?null:Number(weightValue); if(weightValue!==""&&(!Number.isFinite(parsedWeight)||parsedWeight<=0)){alert("체중은 0보다 큰 숫자로 입력해주세요.");return;} const painRecord={part:form.painPart,side:form.painSide,vas:Number(form.painVas)||0,memo:form.painMemo||""}; await saveMemberCheckin(profile.id,today,{...form,painRecord}); await saveMemberHealthInputs(profile.id,today,{weight:weightValue,kcal:form.kcal,steps:form.steps}); if(String(form.memberMessage||"").trim()){await addMemberMessage(profile.id,{date:today,message:String(form.memberMessage).trim(),memberMessage:String(form.memberMessage).trim(),source:"memberApp"});} if(parsedWeight){setBody(prev=>({...(prev||{}),records:upsertBodyRecord(prev?.records||[],{id:`member_${today}`,date:today,weight:parsedWeight,note:"회원앱 직접 입력"})})); setForm(f=>({...f,weight:String(parsedWeight)}));} await load(); alert("건강관리 기록이 저장됐어요");};
  ...
  const saveFeedback=async(sessionId,feedback)=>{await saveSessionMemberFeedback(profile.id,sessionId,feedback); await load(); alert("수업 후 상태가 저장되었습니다.");};
  const saveProfileInfo=async(data)=>{await saveMemberProfileFields(profile.id,data); await load();};
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,saveSoreness,saveFeedback,saveProfileInfo,onLogout,setTab,accessErrors};
...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">대표님이 관리하는 변화 리포트입니다 · 오늘 상태도 건강관리 탭에서 기록하세요</p><div className="hero-card pt-progress"><b>대표님과 함께한 수업</b><strong>{done}회</strong>{p.totalReg?<><em>PT 진행률 {pct}% · 남은 PT {p.remaining}회</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 수업 기록이 쌓이고 있어요</em>}</div><div className="grid2 top-metrics"><ChangeReportMetric startW={p.startW} curW={p.curW}/><GoalMetric goal={p.onboarding.goal||"꾸준한 변화"}/><NextWorkoutMetric profile={p.profile}/></div><ReviewRoutine {...p}/><MCard title="대표 피드백"><p>{p.latest?.trainerComment||"공개 수업일지의 대표 코멘트가 여기에 표시됩니다."}</p></MCard></>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원앱 온보딩</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{ob?.completedAt?`${ob.gender||"-"} · ${ob.birthYear||"-"}년생 · ${ob.heightCm||"-"}cm · ${ob.startingWeightKg||"-"}kg · ${ob.goal||"-"} · ${ob.weeklyWorkoutCount||"-"}`:"아직 완료 전"}</div>{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>집중 부위: {ob.focusAreas.join(" · ")}</div>}</div>
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원 메모 / 요청사항</div>{ms.length?ms.slice(0,1).map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:7,lineHeight:1.55}}><b style={{color:"#e5e7eb"}}>{m.date||m.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||"최근 메모"}</b><br/>{m.memberMessage||m.message}</div>):<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 회원 메모가 없습니다</div>}</div>
  </div>
}
