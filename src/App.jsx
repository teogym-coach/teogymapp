...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, touchMemberAppLastLogin, saveSessionSoreness, saveSessionMemberFeedback, saveMemberHealthInputs, saveMemberProfileFields, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics, getRoutineRecommendations, saveRoutineRecommendation, deleteRoutineRecommendation, getDailyConditioning, saveDailyConditioning, deleteDailyConditioning,
} from "./db";
...
function MemberApp({ onLogout }) {
  ...
  const saveCheck=async()=>{const weightValue=String(form.weight??"").trim(); const parsedWeight=weightValue===""?null:Number(weightValue); if(weightValue!==""&&(!Number.isFinite(parsedWeight)||parsedWeight<=0)){alert("체중은 0보다 큰 숫자로 입력해주세요.");return;} const noPain=form.painPart==="없음"; const painRecord={part:noPain?"없음":form.painPart,side:noPain?"해당 없음":form.painSide,vas:noPain?0:(Number(form.painVas)||0),memo:form.painMemo||""}; await saveMemberCheckin(profile.id,today,{...form,goalNote:"",memberMessage:"",painRecord}); await saveMemberHealthInputs(profile.id,today,{weight:weightValue,kcal:form.kcal,steps:form.steps}); if(parsedWeight){setBody(prev=>({...(prev||{}),records:upsertBodyRecord(prev?.records||[],{id:`member_${today}`,date:today,weight:parsedWeight,note:"회원앱 직접 입력"})})); setForm(f=>({...f,weight:String(parsedWeight)}));} await load(); alert("건강관리 기록이 저장됐어요");};
  ...
  const saveProfileInfo=async(data)=>{await saveMemberProfileFields(profile.id,data); await load();};
  const common={profile,sessions,body:effectiveBody,nutrition:effectiveNutrition,checkins,onboarding:effectiveOnboarding,routineRecommendations,dailyConditioning,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,saveSoreness,saveFeedback,saveProfileInfo,onLogout,setTab:goMemberTab,resetMemberScroll,accessErrors};
...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">대표님이 관리하는 변화 리포트입니다 · 오늘 상태도 건강관리 탭에서 기록하세요</p><div className="hero-card pt-progress"><b>대표님과 함께한 수업</b><strong>{done}회</strong>{p.totalReg?<><em>PT 진행률 {pct}% · 남은 PT {p.remaining}회</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 수업 기록이 쌓이고 있어요</em>}</div><div className="grid2 top-metrics"><ChangeReportMetric startW={p.startW} curW={p.curW}/><GoalMetric goal={p.onboarding.goal||"꾸준한 변화"}/><RecommendedCalorieMetric {...p}/><NextWorkoutMetric profile={p.profile}/></div><ReviewRoutine {...p}/><DailyConditioningCard items={p.dailyConditioning}/></>}
...
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); return <><h1>변화분석</h1><p className="sub">섭취칼로리, 운동량, 체중 변화를 한눈에 확인해요</p><CalorieTrendCard {...p}/><MemberCalorieAnalysisCard {...p}/><PartVolumeCard sessions={p.sessions}/><MCard title="체중 변화 분석"><ResponsiveContainer width="100%" height={190}><LineChart data={weightData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" name="체중" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard></>}
...
function MemberProfile(p){const links=[{title:"교정 영상을 참고하려면",label:"인스타그램",url:"https://www.instagram.com/teogym_pt"},{title:"운동 영상을 참고하려면",label:"유튜브",url:"https://youtube.com/@gymteo"},{title:"운동 정보를 참고하려면",label:"블로그",url:"https://m.blog.naver.com/teogym"},{title:"테오짐 새소식을 보려면",label:"네이버 플레이스",url:"https://naver.me/5ZJFzvnv"}]; const weights=getBodyWeightRecords(p.body); const startRec=weights[0]||null; const curRec=weights.at(-1)||null; const loginEmail=(auth.currentUser?.email||p.profile.memberAppAccountEmail||p.profile.email||"-").trim(); const [editing,setEditing]=useState(false),[saving,setSaving]=useState(false),[msg,setMsg]=useState(""); const [d,setD]=useState({workoutFrequency:p.profile.workoutFrequency||p.onboarding.weeklyWorkoutCount||"",height:p.profile.height||p.onboarding.heightCm||"",startWeight:p.profile.startWeight||p.startW||"",currentWeight:p.profile.currentWeight||p.curW||"",targetWeightKg:p.onboarding.targetWeightKg||p.profile.targetWeight||"",goal:p.onboarding.goal||""}); const save=async()=>{setSaving(true);setMsg("");try{const result=await p.saveProfileInfo(d); await saveMemberOnboarding(p.profile.id,{...p.onboarding,targetWeightKg:d.targetWeightKg,goal:d.goal,updatedAt:new Date().toISOString()});setEditing(false);setMsg(result?.messages?.join("\n")||"회원 정보가 저장되었습니다.");}catch(e){const paths=[`members/${p.profile.id}`,`members/${p.profile.id}/memberOnboarding/main`,`members/${p.profile.id}/bodyCheck/main`]; console.error("[MemberProfile] profile save failed",{paths,code:e?.code,message:e?.message,results:e?.profileSaveResults,failures:e?.profileSaveFailures}); setMsg(e?.message||`저장 실패 경로: ${paths.join(" · ")}`);}finally{setSaving(false);}}; return <><h1>프로필</h1><div className="profile-head"><div className="avatar">{p.profile.name?.[0]||'T'}</div><div><h2>{p.profile.name}</h2><p>{p.onboarding.goal}</p></div></div><MCard title="회원 정보"><Info l="로그인 이메일" v={loginEmail}/><Info l="운동 빈도" v={p.profile.workoutFrequency||p.onboarding.weeklyWorkoutCount}/><Info l="가입일" v={String(p.profile.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||'-')}/><Info l="PT 진행 현황" v={p.totalReg?`${p.sessions.length}/${p.totalReg}회 · 남은 ${p.remaining}회`:`${p.sessions.length}회 진행`}/><Info l="키" v={`${p.profile.height||p.onboarding.heightCm||'-'}cm`}/><div className="profile-weight-grid"><ProfileWeightBlock label="시작 체중" weight={p.startW} dateText={getRecordDateText(startRec)} datePrefix="시작"/><ProfileWeightBlock label="현재 체중" weight={p.curW} dateText={getRecordDateText(curRec)} datePrefix="최근 수정"/></div><button className="primary compact" onClick={()=>{setEditing(true); p.resetMemberScroll?.();}}>정보 수정</button>{msg&&<div className="member-error" style={{marginTop:8}}>{msg}</div>}</MCard>{editing&&<MCard title="정보 수정"><SelectLine label="운동 빈도" value={d.workoutFrequency} opts={[1,2,3,4,5,6,7].map(n=>`주 ${n}회`)} onChange={v=>setD({...d,workoutFrequency:v})}/><InputLine label="키(cm)" type="number" value={d.height} onChange={v=>setD({...d,height:v})}/><InputLine label="시작 체중(kg)" type="number" value={d.startWeight} onChange={v=>setD({...d,startWeight:v})}/><InputLine label="현재 체중(kg)" type="number" value={d.currentWeight} onChange={v=>setD({...d,currentWeight:v})}/><InputLine label="목표 체중(kg)" type="number" value={d.targetWeightKg} onChange={v=>setD({...d,targetWeightKg:v})}/><SelectLine label="목표" value={d.goal} opts={AI_GOAL_OPTIONS} onChange={v=>setD({...d,goal:v})}/><div style={{display:"flex",gap:8}}><button className="primary" onClick={save} disabled={saving}>{saving?"저장 중...":"저장"}</button><button className="ghost" onClick={()=>{setEditing(false); p.resetMemberScroll?.();}} disabled={saving}>취소</button></div></MCard>}<MCard title="바로가기">{links.map(x=><a className="quick-card" key={x.url} href={x.url} target="_blank" rel="noreferrer"><span>{x.title}</span><b>{x.label}</b></a>)}</MCard><button className="ghost" onClick={p.onLogout}>로그아웃</button></>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const BriefTile=({label,value,sub,color="#5EEAD4",emoji})=><div style={{padding:10,borderRadius:9,background:color+"10",border:`1px solid ${color}55`,minHeight:66}}><Mo c={color} s={9} style={{display:"block",fontWeight:800,marginBottom:6}}>{label}</Mo><div style={{fontSize:15,color:"#e5e7eb",fontWeight:800,lineHeight:1.25}}>{emoji&&<span style={{marginRight:5}}>{emoji}</span>}{value}</div>{sub&&<div style={{fontSize:12,color:color,marginTop:3,fontWeight:800}}>{sub}</div>}</div>;
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:8,alignItems:"stretch"}}>
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>다음 PT 예정 부위</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원앱 개인운동 추천에서 우선 제외할 다음 수업 예정 부위입니다.</div><select value={member?.nextWorkoutPart||member?.nextPtPart||"미정"} onChange={e=>saveNextPtPart(e.target.value)} disabled={busy} style={{width:"100%",marginTop:8,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:12}}>{NEXT_PT_PART_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></div>
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>다음 PT 날짜</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원앱 다음 수업 운동 카드의 날짜와 D-Day에 즉시 반영됩니다.</div><input type="date" value={member?.nextWorkoutDate||""} onChange={e=>saveNextWorkoutDate(e.target.value)} disabled={busy} style={{width:"100%",marginTop:8,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:12}}/></div>
    </div>
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원 온보딩</Mo><div style={{fontSize:12,color:"#e5e7eb",marginTop:6,lineHeight:1.55,fontWeight:700}}>{ob?.completedAt?`${ob.gender||"-"} · ${formatAgeFromBirthYear(ob.birthYear)} · ${ob.heightCm||"-"}cm · ${ob.currentWeightKg||ob.startingWeightKg||"-"}kg · ${ob.weeklyWorkoutCount||"-"} · ${ob.goal||"-"}`:"아직 완료 전"}</div>{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:7}}>집중 목표<br/><b style={{color:"#e5e7eb"}}>{ob.focusAreas.join(" · ")}</b></div>}<div style={{fontSize:10,color:"#64748b",marginTop:6}}>상세 정보는 펼치기 또는 수정 화면에서 확인</div></div>
    <div style={{padding:12,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"center",marginBottom:9}}><Mo c="#5EEAD4" s={10} style={{fontWeight:900}}>회원 상태 브리핑</Mo><span style={{fontSize:10,color:"#64748b",background:"rgba(255,255,255,.04)",borderRadius:999,padding:"3px 7px"}}>{briefingDate} 기준</span></div>{ci.length?<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:8}}><BriefTile label="통증" value={painText} sub={`VAS ${painVas}`} color="#ff6b6b" emoji="📍"/><BriefTile label="근육통" value={sorenessText} color="#f97316" emoji="💪"/><BriefTile label="컨디션" value={conditionText} color="#5EEAD4" emoji="🙂"/><BriefTile label="최근 RPE" value={rpeText} color="#818cf8"/></div><div style={{marginTop:8,padding:10,borderRadius:8,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)"}}><Mo c="#94a3b8" s={9} style={{display:"block",marginBottom:4}}>대표 코멘트</Mo><div style={{fontSize:13,color:"#e5e7eb",fontWeight:700,lineHeight:1.45}}>{trainerComment}</div></div></>:<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 건강관리 기록이 없습니다</div>}</div>
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원 메모 / 요청사항</div>{ms.length?ms.slice(0,1).map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:7,lineHeight:1.55}}><b style={{color:"#e5e7eb"}}>{m.sessionTitle||m.date||m.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||"최근 메모"}</b><br/>{m.memberMessage||m.message}</div>):<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 회원 메모가 없습니다</div>}</div>
  </div>
}
