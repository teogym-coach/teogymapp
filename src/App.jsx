...
function MemberCalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const goal=onboarding?.goal||profile?.goal||nutrition?.goal; const target=getGoalCalorieRecommendation(a,goal); const logs=getKcalLogs(nutrition).slice(-30); const mean=arr=>arr.length?Math.round(arr.reduce((sum,x)=>sum+x.kcal,0)/arr.length):null; const m7=mean(logs.slice(-7)); return <MCard title="섭취 칼로리 분석"><CalorieSummaryCard avg7={m7} target={target.value} goal={goal}/></MCard>}
...
function MemberNoticeList({notices=[],onOpen}){return <MCard title="공지사항">{notices.length?notices.map(n=><button key={n.id} className={`member-notice-row ${n.isRead?"read":"unread"}`} onClick={()=>onOpen?.(n)}><span>{n.isImportant?"📌 중요":"공지"}</span><b>{n.title}</b><em>{formatCompactDate(n.createdAt)}</em><i>{n.isRead?"읽음":"새 공지"}</i></button>):<p className="notice soft">현재 확인할 공지사항이 없습니다.</p>}</MCard>}
...
function getMemberExerciseSection(ex){const text=[ex.section,ex.category,ex.type,ex.exerciseType,ex.movementPurpose,ex.funcCategory,ex.muscleTop,ex.name].map(v=>String(v||"").toLowerCase()).join(" "); if(/warm|워밍|준비운동|오픈북|월 슬라이드|호흡 리셋/.test(text))return "움직임 준비"; if(/conditioning|컨디셔닝/.test(text))return "컨디셔닝"; if(/correct|교정|움직임교정|조직이완|가동성|호흡|밸런스/.test(text))return "교정운동"; if(isFuncEx(ex))return "움직임 준비"; return "웨이트 트레이닝";}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const onboardingBaseLine=ob?.completed!==false&&ob?.completedAt?`${ob.gender||"-"} · ${formatAgeFromBirthYear(ob.birthYear)} · ${ob.heightCm||member?.height||"-"}cm`:"아직 완료 전";
