...
function MemberApp({ onLogout }) {
  const C=MEMBER_COLORS; const today=new Date().toISOString().slice(0,10); const [tab,setTab]=useState("home"); const [profile,setProfile]=useState(null); const [sessions,setSessions]=useState([]); const [body,setBody]=useState(null); const [nutrition,setNutrition]=useState(null); const [checkins,setCheckins]=useState([]); const [messages,setMessages]=useState([]); const [onboarding,setOnboarding]=useState(null); const [loading,setLoading]=useState(true); const [form,setForm]=useState({date:today,weight:"",condition:"보통",soreness:"없음"});
  const load=useCallback(async()=>{setLoading(true); const p=await getMemberAppProfile(); setProfile(p); if(p){ const [ss,bd,nt,ci,ms,ob]=await Promise.all([getPublishedSessions(p.id),getBodyCheck(p.id),getNutrition(p.id),getMemberCheckins(p.id),getMemberMessages(p.id),getMemberOnboarding(p.id)]); setSessions(ss); setBody(bd); setNutrition(nt); setCheckins(ci); setMessages(ms); setOnboarding(ob); const w=getLatestBodyWeight(bd)?.weight||p.weight||p.startWeight||""; setForm(f=>({...f,weight:w})); } setLoading(false);},[]); useEffect(()=>{load();},[load]);
  if(loading) return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><Spin/></div>;
  if(!profile) return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><div className="member-page"><h2>회원 정보를 찾을 수 없습니다</h2><button className="primary" onClick={onLogout}>로그아웃</button></div></div>;
