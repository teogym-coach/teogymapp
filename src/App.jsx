        ...
        <div>
          ...
          {error && (
            <div style={{padding:"12px 14px",background:"rgba(255,107,107,.08)",borderRadius:8,
              ...
              <div>{error}</div>
              {(error.includes("API 키") || error.includes("401") || error.includes("유효하지")) && (
                <div style={{marginTop:8,padding:"8px 10px",background:"rgba(255,209,102,.08)",
                  borderRadius:6,border:"1px solid rgba(255,209,102,.25)",color:"#ffd166",fontSize:11,lineHeight:1.8}}>
                  📌 <b>Vercel 환경변수 설정 방법</b><br/>
                  1. Vercel 대시보드 → 프로젝트 선택<br/>
                  2. Settings → Environment Variables<br/>
                  3. Name: <b>ANTHROPIC_API_KEY</b><br/>
                  4. Value: <b>sk-ant-api03-...</b> (Anthropic Console에서 복사)<br/>
                  5. Save 후 <b>Redeploy</b> 필수
                </div>
              )}
            </div>
          )}
