...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, saveSessionSoreness, saveSessionMemberFeedback, saveMemberHealthInputs, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics,
} from "./db";
...
function MemberApp({ onLogout }) {
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,saveSoreness,saveFeedback,onLogout,setTab,accessErrors};
...
function MemberProfile(p){const links=[{title:"교정 영상을 참고하려면",label:"인스타그램",url:"https://www.instagram.com/teogym_pt"},{title:"운동 영상을 참고하려면",label:"유튜브",url:"https://youtube.com/@gymteo"},{title:"운동 정보를 참고하려면",label:"블로그",url:"https://m.blog.naver.com/teogym"},{title:"테오짐 새소식을 보려면",label:"네이버 플레이스",url:"https://naver.me/5ZJFzvnv"}]; return <><h1>프로필</h1><div className="profile-head"><div className="avatar">{p.profile.name?.[0]||'T'}</div><div><h2>{p.profile.name}</h2><p>{p.onboarding.goal}</p></div></div><MCard title="회원 정보"><Info l="운동 빈도" v={p.onboarding.weeklyWorkoutCount}/><Info l="가입일" v={String(p.profile.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||'-')}/><Info l="PT 진행 현황" v={p.totalReg?`${p.sessions.length}/${p.totalReg}회 · 남은 ${p.remaining}회`:`${p.sessions.length}회 진행`}/><Info l="키/체중" v={`${p.onboarding.heightCm}cm · ${p.curW}kg`}/></MCard><MCard title="바로가기">{links.map(x=><a className="quick-card" key={x.url} href={x.url} target="_blank" rel="noreferrer"><span>{x.title}</span><b>{x.label}</b></a>)}</MCard><button className="ghost" onClick={p.onLogout}>로그아웃</button></>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:8,alignItems:"stretch"}}>
      ...
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.length?ci.map(c=><div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{c.date||c.id} · {c.weight||"-"}kg · {c.condition||"-"} · 근육통 {c.soreness||"-"}</div>):<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 체크인 기록이 없습니다</div>}</div>
    </div>
