...
function MemberApp({ onLogout }) {
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,onLogout,setTab,accessErrors};
...
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); const inbody=p.body?.inbody||[]; const volume=p.sessions.reduce((a,s)=>a+(s.totalVolume||0),0); return <><h1>변화분석</h1><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={180}><LineChart data={weightData}><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><div className="grid2"><Metric t="골격근량" v={(inbody.at(-1)?.muscleMass)||'-'}/><Metric t="체지방" v={(inbody.at(-1)?.bodyFat)||'-'}/><Metric t="출석률" v={`${p.remaining==='-'?0:Math.round(p.sessions.length/(p.sessions.length+p.remaining||1)*100)}%`}/><Metric t="운동 횟수" v={`${p.sessions.length}회`}/><Metric t="누적 운동량" v={`${Math.round(volume).toLocaleString()}kg`}/><Metric t="목표 달성률" v={`${p.remaining==='-'?0:Math.min(100,p.sessions.length*10)}%`}/></div><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"꾸준히 기록이 쌓이면 변화가 더 선명하게 보입니다."}</p></MCard></>}
function MemberProfile(p){return <><h1>프로필</h1><div className="profile-head"><div className="avatar">{p.profile.name?.[0]||'T'}</div><div><h2>{p.profile.name}</h2><p>{p.onboarding.goal}</p></div></div><MCard title="회원 정보"><Info l="운동 빈도" v={p.onboarding.weeklyWorkoutCount}/><Info l="가입일" v={String(p.profile.createdAt?.toDate?.()?.toISOString?.().slice(0,10)||'-')}/><Info l="PT 진행 현황" v={p.totalReg?`${p.sessions.length}/${p.totalReg}회 · 남은 ${p.remaining}회`:`${p.sessions.length}회 진행`}/><Info l="키/체중" v={`${p.onboarding.heightCm}cm · ${p.curW}kg`}/></MCard><button className="primary">대표에게 연락하기</button><button className="ghost" onClick={p.onLogout}>로그아웃</button></>}
