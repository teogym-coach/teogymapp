...
async function createMemberAuthAccountIfNeeded(email){
  ...
  if(res.ok) return {created:true,uid:data.localId};
  const code=data?.error?.message||"AUTH_CREATE_FAILED";
  if(code==="EMAIL_EXISTS") return {created:false,alreadyExists:true};
  throw new Error(`Firebase Auth 계정 준비 실패: ${code}`);
}
function getMemberAppInviteStatus(member){
  if(member?.memberAppAccountStatus==="available"||member?.memberUid) return {label:"회원앱 사용 가능",color:"#86efac"};
  if(member?.memberAppInviteSentAt) return {label:"초대 발송 완료",color:"#93c5fd"};
  return {label:"초대 전",color:"#fbbf24"};
}
function AdminMemberAppPanel({member,onAccountCreated}){const memberId=member?.id; const [ci,setCi]=useState([]),[ms,setMs]=useState([]),[ob,setOb]=useState(null); const [busy,setBusy]=useState(false),[msg,setMsg]=useState(""); useEffect(()=>{if(memberId)Promise.all([getMemberCheckins(memberId,5),getMemberMessages(memberId,5),getMemberOnboarding(memberId)]).then(([a,b,c])=>{setCi(a);setMs(b);setOb(c);}).catch(()=>{});},[memberId]); const inviteStatus=getMemberAppInviteStatus(member); const sendInvite=async()=>{const email=(member?.email||"").trim().toLowerCase(); if(!email){setMsg("회원 이메일을 먼저 저장해주세요.");return;} setBusy(true); setMsg(""); try{const prepared=await createMemberAuthAccountIfNeeded(email); await sendPasswordResetEmail(auth,email); const now=new Date().toISOString(); const patch={memberAppAccountEmail:email,memberAppInviteSentAt:now,memberAppPasswordResetSentAt:now,memberAppAccountStatus:prepared.alreadyExists||member?.memberUid?"available":"invited"}; if(prepared.uid) patch.memberUid=prepared.uid; await updateMember(memberId,patch); setMsg(prepared.alreadyExists?"이미 Firebase Auth 계정이 있어 비밀번호 재설정 메일만 발송했습니다.":"회원앱 초대를 발송했습니다. 회원이 이메일에서 직접 비밀번호를 설정합니다."); onAccountCreated?.(patch);}catch(e){setMsg(e.message);}finally{setBusy(false);}}; return <div style={{marginTop:10,display:"grid",gap:8}}><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#60a5fa" s={9}>회원앱 초대</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>이메일: {member?.email||"회원 이메일 없음"}</div><div style={{fontSize:11,color:inviteStatus.color,marginTop:4}}>상태: {inviteStatus.label}</div><div style={{fontSize:11,color:"#94a3b8",marginTop:6}}>흐름: 회원 이메일 확인 → Auth 계정이 없으면 자동 준비 → 비밀번호 설정/재설정 메일 발송 → 회원이 직접 비밀번호 설정</div><Btn sm onClick={sendInvite} disabled={busy||!member?.email} style={{marginTop:8}}>{busy?"발송 중...":"회원앱 초대 보내기"}</Btn>{msg&&<div style={{fontSize:11,color:msg.includes("발송")?"#86efac":"#ff9f43",marginTop:6}}>{msg}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원앱 온보딩</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{ob?.completedAt?`${ob.gender||"-"} · ${ob.birthYear||"-"}년생 · ${ob.heightCm||"-"}cm · ${ob.startingWeightKg||"-"}kg · ${ob.goal||"-"} · ${ob.weeklyWorkoutCount||"-"}`:"아직 완료 전"}</div>{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>집중 부위: {ob.focusAreas.join(" · ")}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.map(c=><div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{c.date||c.id} · {c.weight||"-"}kg · {c.condition||"-"} · 근육통 {c.soreness||"-"}</div>)}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo>{ms.map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{m.message}</div>)}</div></div>}
