...
function generateTempPassword(){return `Teo!${Math.random().toString(36).slice(2,8)}${Math.floor(1000+Math.random()*9000)}`;}
async function createMemberAuthAccount(email,password){
  const res=await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${firebaseConfig.apiKey}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password,returnSecureToken:false})});
  const data=await res.json();
  if(!res.ok){const code=data?.error?.message||"AUTH_CREATE_FAILED"; if(code==="EMAIL_EXISTS") throw new Error("이미 Firebase Authentication 계정이 있습니다. 중복 생성하지 않았습니다."); throw new Error(`Firebase Auth 계정 생성 실패: ${code}`);}
  return data;
}
function AdminMemberAppPanel({member,onAccountCreated}){const memberId=member?.id; const [ci,setCi]=useState([]),[ms,setMs]=useState([]),[ob,setOb]=useState(null); const [busy,setBusy]=useState(false),[msg,setMsg]=useState(""); useEffect(()=>{if(memberId)Promise.all([getMemberCheckins(memberId,5),getMemberMessages(memberId,5),getMemberOnboarding(memberId)]).then(([a,b,c])=>{setCi(a);setMs(b);setOb(c);}).catch(()=>{});},[memberId]); const createAccount=async()=>{const email=(member?.email||"").trim().toLowerCase(); if(!email){setMsg("회원 이메일을 먼저 저장해주세요.");return;} if(member?.memberUid||member?.memberAppAccountCreatedAt){setMsg("이미 회원앱 계정 생성 기록이 있어 중복 생성하지 않았습니다.");return;} setBusy(true); setMsg(""); try{const tempPassword=generateTempPassword(); const created=await createMemberAuthAccount(email,tempPassword); await sendPasswordResetEmail(auth,email); await updateMember(memberId,{memberUid:created.localId,memberAppAccountEmail:email,memberAppAccountCreatedAt:new Date().toISOString(),memberAppPasswordResetSentAt:new Date().toISOString()}); setMsg("회원앱 계정을 생성했고 비밀번호 재설정 메일을 발송했습니다."); onAccountCreated?.({memberUid:created.localId,memberAppAccountEmail:email,memberAppAccountCreatedAt:new Date().toISOString(),memberAppPasswordResetSentAt:new Date().toISOString()});}catch(e){setMsg(e.message);}finally{setBusy(false);}}; return <div style={{marginTop:10,display:"grid",gap:8}}><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#60a5fa" s={9}>회원앱 계정 생성</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>이메일: {member?.email||"회원 이메일 없음"}</div><div style={{fontSize:11,color:(member?.memberUid||member?.memberAppAccountCreatedAt)?"#86efac":"#fbbf24",marginTop:4}}>생성 여부: {(member?.memberUid||member?.memberAppAccountCreatedAt)?"생성됨":"미생성"}</div><div style={{fontSize:11,color:"#94a3b8",marginTop:6}}>흐름: 이메일 기준 Firebase Auth 계정 생성 → 임시 비밀번호 발급 → 비밀번호 재설정 메일 발송 → 회원이 직접 비밀번호 설정</div><Btn sm onClick={createAccount} disabled={busy||!member?.email||member?.memberUid||member?.memberAppAccountCreatedAt} style={{marginTop:8}}>{busy?"생성 중...":"회원앱 계정 생성"}</Btn>{msg&&<div style={{fontSize:11,color:msg.includes("생성했고")?"#86efac":"#ff9f43",marginTop:6}}>{msg}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원앱 온보딩</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{ob?.completedAt?`${ob.gender||"-"} · ${ob.birthYear||"-"}년생 · ${ob.heightCm||"-"}cm · ${ob.startingWeightKg||"-"}kg · ${ob.goal||"-"} · ${ob.weeklyWorkoutCount||"-"}`:"아직 완료 전"}</div>{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>집중 부위: {ob.focusAreas.join(" · ")}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.map(c=><div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{c.date||c.id} · {c.weight||"-"}kg · {c.condition||"-"} · 근육통 {c.soreness||"-"}</div>)}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo>{ms.map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{m.message}</div>)}</div></div>}
...
export default function App() {
  ...
  async function handleLogin(email, pw) {
    ...
    catch(e) { const code=e?.code||""; setLoginErr(memberMode&&code==="auth/user-not-found" ? "회원 이메일은 저장되어 있지만 Firebase Authentication 계정이 없습니다. 대표에게 회원앱 계정 생성을 요청해주세요." : "로그인 실패: 이메일 또는 비밀번호를 확인해주세요."); }
    finally { setLoading(false); }
  }
