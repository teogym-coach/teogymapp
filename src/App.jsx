...
function CalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const weekly=a.weeklyChange; const pace=weekly===null?"기록 수집 중":`${weekly>0?"+":""}${weekly.toFixed(2)}kg/주`; return <MCard title={admin?"🔥 칼로리 분석":"AI 섭취 칼로리 분석"}><div className="grid2"><Metric t="유지 칼로리" v={formatKcal(a.maintenance)}/><Metric t={target.label} v={formatKcal(target.value)}/>{admin&&<><Metric t="기초대사량 (BMR)" v={formatKcal(a.bmr)}/><Metric t="최근 7일 평균" v={formatKcal(a.avg7)}/></>}</div>{admin&&<><Info l="최근 30일 평균 섭취칼로리" v={formatKcal(a.avg30)}/><Info l="최근 체중 변화 / 감량·증량 속도" v={pace}/></>}<Info l="신뢰도" v={`${a.confidence}%`}/><Info l="데이터 기준" v={a.basis}/><p className="notice soft">공식 기반 초기값은 Mifflin-St Jeor BMR과 활동계수 TDEE를 사용합니다. 기록이 충분하면 최근 14/28/56일 섭취칼로리와 체중 변화로 유지칼로리를 보정합니다.</p></MCard>}
