  ...
  return (
    <div style={{minHeight:"100vh",background:"#0B1120"}}>
      ...
      <div className="noprint" style={{
        ...
        {screen==="notices" && <NoticeAdminScreen members={members} onBack={()=>setScreen("home")} showToast={showToast}/>}
...
// HOME
// ════════════════════════════════════════════
function NoticeAdminScreen({members=[],onBack,showToast}){const empty={title:"",content:"",targetType:"all",targetMemberId:"",isImportant:false,isPublished:true}; const [rows,setRows]=useState([]),[form,setForm]=useState(empty),[editing,setEditing]=useState(null),[saving,setSaving]=useState(false); const refresh=useCallback(()=>getNotices().then(setRows).catch(e=>showToast?.(e.message||"공지 조회 실패","err")),[showToast]); useEffect(()=>{refresh();},[refresh]); const set=(k,v)=>setForm(f=>({...f,[k]:v})); const edit=n=>{setEditing(n.id);setForm({title:n.title||"",content:n.content||"",targetType:n.targetType||"all",targetMemberId:n.targetMemberId||"",isImportant:!!n.isImportant,isPublished:n.isPublished!==false});}; const save=async()=>{setSaving(true);try{await saveNotice(form,editing);showToast?.(editing?"공지를 수정했습니다.":"공지를 작성했습니다.");setForm(empty);setEditing(null);refresh();}catch(e){showToast?.(e.message||"공지 저장 실패","err");}finally{setSaving(false);}}; const remove=async(n)=>{if(!window.confirm("공지를 삭제할까요?"))return;try{await deleteNotice(n.id);showToast?.("공지를 삭제했습니다.");refresh();}catch(e){showToast?.(e.message||"삭제 실패","err");}}; return <div><SH title="📢 공지사항 관리" sub="회원앱 홈/프로필 공지 노출" right={<Btn ghost sm onClick={onBack}>← 홈</Btn>}/><Card title={editing?"공지 수정":"공지 작성"}><div style={{display:"grid",gap:10}}><Field label="제목" value={form.title} onChange={v=>set("title",v)}/><TextArea label="내용" value={form.content} onChange={v=>set("content",v)}/><SelectLine label="공지 대상" value={form.targetType} opts={["all","member"]} onChange={v=>set("targetType",v)}/>{form.targetType==="member"&&<SelectLine label="특정 회원" value={form.targetMemberId} opts={["",...members.map(m=>m.id)]} onChange={v=>set("targetMemberId",v)}/>}<SelectLine label="중요 공지 여부" value={form.isImportant?"중요":"일반"} opts={["일반","중요"]} onChange={v=>set("isImportant",v==="중요")}/><SelectLine label="게시 상태" value={form.isPublished?"게시":"숨김"} opts={["게시","숨김"]} onChange={v=>set("isPublished",v==="게시")}/><Btn disabled={saving} onClick={save}>{saving?"저장 중...":editing?"수정 저장":"공지 게시/저장"}</Btn>{editing&&<Btn ghost onClick={()=>{setEditing(null);setForm(empty);}}>취소</Btn>}</div></Card><Card title="공지 목록">{rows.length?rows.map(n=><div key={n.id} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8,padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,.08)"}}><div><Mo c="#fff" s={11}>{n.isImportant?"📌 ":""}{n.title}</Mo><Mo c="#64748b" s={9}>{n.targetType==="member"?`개별 · ${members.find(m=>m.id===n.targetMemberId)?.name||n.targetMemberId}`:"전체 회원"} · {n.isPublished?"게시":"숨김"} · {formatCompactDate(n.createdAt)}</Mo></div><div style={{display:"flex",gap:6}}><Btn ghost sm onClick={()=>edit(n)}>수정</Btn><Btn ghost sm onClick={()=>saveNotice({...n,isPublished:!n.isPublished},n.id).then(refresh)}>{n.isPublished?"숨김":"게시"}</Btn><Btn ghost sm onClick={()=>remove(n)} style={{color:"#f87171"}}>삭제</Btn></div></div>):<Mo c="#64748b" s={9}>등록된 공지가 없습니다.</Mo>}</Card></div>}
