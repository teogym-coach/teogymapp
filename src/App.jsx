  ...
  return (
    <div style={{minHeight:"100vh",background:"#09090c"}}>
      ...
      <div className="noprint" style={{
        ...
        {screen==="nutrition"  && member && <NutritionScreen member={member} onBack={() => setScreen("hub")} nutritionData={nutritionData} onSaveNutrition={async d => { await saveNutrition(member.id, d); setNutritionData(d); }} showToast={showToast} targetCal={bodyData?.targetCal||0} />}
        {screen==="bodycheck"  && member && <BodyCheckScreen member={member} onBack={() => setScreen("hub")} bodyData={bodyData} onSaveBodyData={async d => { await saveBodyCheck(member.id, d); setBodyData(d); }} showToast={showToast} />}
      </div>
