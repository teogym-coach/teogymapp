...
function getProfileAge(profile={},onboarding={}){const direct=Number(onboarding.age||profile.age); if(Number.isFinite(direct)&&direct>0)return direct; const birth=Number(onboarding.birthYear||profile.birthYear||profile.birth); return Number.isFinite(birth)?Math.max(10,new Date().getFullYear()-birth):35;}
...
function CalorieTrendCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const goal=onboarding?.goal||profile?.goal||nutrition?.goal; const target=getGoalCalorieRecommendation(a,goal); const rows=getKcalLogs(nutrition).slice(-30).map(x=>({date:x.date.slice(5),kcal:x.kcal,target:target.value})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; const m7=mean(avg7),m30=mean(avg30); const diff=Number.isFinite(Number(m7))&&Number.isFinite(Number(target.value))?m7-target.value:null; const chartRows=rows.map(r=>({...r,target:target.value})); const items=[{label:"최근 7일 평균",value:formatKcal(m7),color:admin?"#54a0ff":"#2F73F6"},{label:"최근 30일 평균",value:formatKcal(m30),color:admin?"#5EEAD4":"#14B8A6"},{label:"유지 칼로리",value:formatKcal(a.maintenance),color:admin?"#ffd166":"#20242A"},{label:"권장 섭취 칼로리",value:formatKcal(target.value),sub:target.shortLabel||target.label,color:admin?"#22c55e":"#16A34A"},{label:"권장 대비",value:formatSignedKcal(diff),color:diff>150?"#ff6b6b":diff<-150?"#ffd166":admin?"#5EEAD4":"#16A34A"}]; const content=<><CalorieSummaryCard avg7={m7} target={target.value} goal={goal} admin={admin}/><CalorieTrendChart rows={rows} chartRows={chartRows} admin={admin}/><CalorieKpiGrid items={items} admin={admin}/></>; return admin?<Card title="📈 섭취 칼로리 변화 그래프" style={{marginBottom:11}}>{content}</Card>:<MCard title="섭취 칼로리 변화 그래프"><CalorieTrendChart rows={rows} chartRows={chartRows} admin={admin}/></MCard>}
function MemberCalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const goal=onboarding?.goal||profile?.goal||nutrition?.goal; const target=getGoalCalorieRecommendation(a,goal); const logs=getKcalLogs(nutrition).slice(-30); const mean=arr=>arr.length?Math.round(arr.reduce((sum,x)=>sum+x.kcal,0)/arr.length):null; const m7=mean(logs.slice(-7)),m30=mean(logs); const diff=Number.isFinite(Number(m7))&&Number.isFinite(Number(target.value))?m7-target.value:null; const items=[{label:"최근 7일 평균",value:formatKcal(m7),color:"#2F73F6"},{label:"최근 30일 평균",value:formatKcal(m30),color:"#14B8A6"},{label:"유지 칼로리",value:formatKcal(a.maintenance),color:"#20242A"},{label:"권장 섭취 칼로리",value:formatKcal(target.value),sub:target.shortLabel||target.label,color:"#16A34A"},{label:"권장 대비",value:formatSignedKcal(diff),color:diff>150?"#EF4444":diff<-150?"#F59E0B":"#16A34A"}]; return <MCard title="섭취 칼로리 분석"><CalorieSummaryCard avg7={m7} target={target.value} goal={goal}/><CalorieKpiGrid items={items}/></MCard>}
...
function MemberApp({ onLogout }) {
  ...
  if(!onboardingReadFailed&&!onboarding?.completedAt) return <>{debugPanel}<MemberOnboarding profile={profile} body={body} existing={onboarding} onDone={load}/></>;
  ...
  const saveProfileInfo=async(data)=>{const result=await saveMemberProfileFields(profile.id,data); await load(); return result;};
  const common={profile,sessions,body:effectiveBody,nutrition:effectiveNutrition,checkins,onboarding:effectiveOnboarding,routineRecommendations,dailyConditioning,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,deleteHealthRecord,healthSaving,saveSoreness,saveFeedback,saveProfileInfo,onLogout,setTab:goMemberTab,resetMemberScroll,accessErrors};
...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">대표님이 관리하는 변화 리포트입니다 · 오늘 상태도 건강관리 탭에서 기록하세요</p><div className="hero-card pt-progress"><b>대표님과 함께한 수업</b><strong>{done}회</strong>{p.totalReg?<><em>PT 진행률 {pct}% · 남은 PT {p.remaining}회</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 수업 기록이 쌓이고 있어요</em>}</div><div className="grid2 top-metrics"><ChangeReportMetric startW={p.startW} curW={p.curW}/><GoalMetric goal={p.onboarding.goal||"꾸준한 변화"}/><GoalWeightForecastCard {...p}/><NextWorkoutMetric profile={p.profile}/></div><ReviewRoutine {...p}/><DailyConditioningCard items={p.dailyConditioning}/></>}
...
function GoalWeightForecastCard(p){const f=getWeightForecast(p); return <div className="metric goal-forecast"><span>🎯 목표 체중 도달 예상일</span><b>{f.target?f.estimatedDate:"목표 설정 필요"}</b><small>현재 {kgText(f.cur)} · 목표 {kgText(f.target)}</small><small>남은 감량 {f.remain?`-${f.remain.toFixed(1)}kg`:"0kg"} · 약 {f.weeks}주</small><em>{f.risk==="high"?"목표 기간 조정 권장":"현재 페이스 유지 시 가능"}</em></div>}
...
function ChangeReportMetric({startW,curW}){const start=Number(startW),cur=Number(curW),ok=Number.isFinite(start)&&Number.isFinite(cur); const diff=ok?cur-start:0; const label=!ok?"기록 수집 중":diff<0?`${diff.toFixed(1)}kg 감량`:diff>0?`+${diff.toFixed(1)}kg 증량`:"변화 유지"; return <div className="metric change-report"><span>변화 리포트</span><b>{label}</b><small>시작 {formatWeightValue(startW)}</small><small>현재 {formatWeightValue(curW)}</small></div>}
...
function buildReviewRoutine(sessions,onboarding,checkins,selectedPart){const lastCheck=checkins?.[0]||{}; const lower=['강함','매우 강함','심함'].includes(lastCheck.soreness)||String(lastCheck.condition).includes('피곤'); const cutoff=new Date(Date.now()-14*86400000).toISOString().slice(0,10); const map=new Map(); sessions.forEach(s=>(s.exercises||[]).forEach(e=>{const filled=getFilledSets(e); if(!e.name||!exerciseMatchesPart(e,selectedPart)||(!isTrainerMarkedExercise(e)&&filled.length<3))return; const prev=map.get(e.name)||{...e,count:0,recent:false,stim:0,painFree:0,latestDate:""}; const feedback=String(e.feedback||""); const stim=/자극|좋|잘|우수|높/.test(feedback)?1:0; const pain=/통증|불편|아픔|저림/.test(feedback)?1:0; map.set(e.name,{...prev,...e,count:prev.count+1,recent:prev.recent||(s.date||"")>=cutoff,stim:prev.stim+stim,painFree:prev.painFree+(pain?0:1),marked:prev.marked||isTrainerMarkedExercise(e),latestDate:String(s.date||prev.latestDate)});})); const list=[...map.values()]; const sorted=[...list].sort((a,b)=>(b.marked-a.marked)||(b.stim-a.stim)||(b.painFree-a.painFree)||((a.recent?1:0)-(b.recent?1:0))||(b.count-a.count)); const fallback={등:["스트레이트 암풀다운","랫풀다운","시티드 케이블로우","페이스풀"],가슴:["체스트프레스","인클라인 덤벨프레스","케이블 플라이","푸시업"],어깨:["숄더프레스","사이드 레터럴 레이즈","리어델트 플라이","페이스풀"],이두:["덤벨 컬","해머 컬","케이블 컬","프리처 컬"],삼두:["케이블 푸시다운","오버헤드 익스텐션","라잉 트라이셉스 익스텐션","딥스"],팔:["케이블 푸시다운","덤벨 컬","해머 컬","오버헤드 익스텐션"],하체:["레그프레스","레그컬","힙브릿지","카프레이즈"],코어:["플랭크","데드버그","케이블 크런치","팔로프 프레스"]}; const base=(fallback[selectedPart]||fallback.등).map(name=>({name,sets:[{label:"1세트",weight:"가볍게 시작",reps:"15회"},{label:"2세트",weight:"가능한 중량",reps:"12회"},{label:"3세트",weight:"가능한 중량",reps:"10회"}]})); const itemFor=e=>({name:e.name,...formatExerciseDose(e,lower)}); return {selectedPart,hasData:list.length>0,goodStim:sorted.filter(e=>e.stim>0).slice(0,2),painFree:sorted.filter(e=>e.painFree>0).slice(0,2),practice:sorted.filter(e=>e.count<3||!e.recent).slice(0,2),routine:(sorted.length?sorted.slice(0,4).map(itemFor):base),comment:lower?'오늘은 컨디션을 고려해 세트 수와 강도를 낮췄어요.':'무리하지 말고 가능한 중량으로 진행해주세요.'};}
function ReviewRoutine({profile,sessions,onboarding,checkins,routineRecommendations=[]}){const today=getKoreaDateString(); const visibleRoutines=(routineRecommendations||[]).filter(r=>isPublishedData(r)&&String(r.date||"")>=today).sort((a,b)=>String(a.date||"").localeCompare(String(b.date||""))); const publishedRoutine=visibleRoutines.find(r=>r.date===today)||visibleRoutines[0]; const upcomingRoutines=visibleRoutines.filter(r=>r.id!==publishedRoutine?.id&&r.date>today).slice(0,4); const info=getNextWorkoutInfo(profile); const recommended=getRecommendedPart(profile,sessions); const [selected,setSelected]=useState(""); const rec=selected?buildReviewRoutine(sessions,onboarding,checkins,selected):null; const daysText=info.daysUntil===null?"아직 정해지지 않았습니다":info.daysUntil===0?"오늘입니다":`${Math.abs(info.daysUntil)}일${info.daysUntil<0?" 지났습니다":"입니다"}`; return <MCard title="오늘의 운동 가이드"><div className="workout-guide"><p>다음 수업은 <b>{info.part}</b>입니다.</p><p>남은 기간은 <b>{daysText}</b></p><p>오늘 추천은 <b>{recommended} 운동</b>입니다.</p><p>{info.part} 수업 전 충분한 회복을 위해<br/>오늘 컨디션에 맞는 부위를 선택해주세요.</p></div><div className="part-pills">{["가슴","등","하체","어깨","이두","삼두","코어"].map(x=><button key={x} className={selected===x?"active":recommended===x?"recommended":""} onClick={()=>{setSelected(x); scrollMemberAppToTop();}}>{x}</button>)}</div><CoachRoutineCard routine={publishedRoutine}/>{upcomingRoutines.length>0&&<div className="rec-group"><b>앞으로의 추천</b>{upcomingRoutines.map(r=><p key={r.id}><span>{String(r.date).slice(5)} · {formatParts(r)}</span></p>)}</div>}{selected&&<><h3>{selected} 운동 추천</h3><div className="routine-list">{rec.routine.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><div className="routine-sets">{x.sets.map((st,j)=><span key={j}><strong>{st.label}</strong><i>{st.weight}</i><i>{st.reps}</i></span>)}</div></div>)}</div></>}</MCard>}
...
function getMemberExerciseSection(ex){const text=[ex.section,ex.category,ex.type,ex.exerciseType,ex.movementPurpose,ex.funcCategory,ex.muscleTop,ex.name].map(v=>String(v||"").toLowerCase()).join(" "); if(/warm|워밍|준비운동|오픈북|월 슬라이드|호흡 리셋/.test(text))return "워밍업"; if(/conditioning|컨디셔닝/.test(text))return "컨디셔닝"; if(/correct|교정|움직임교정|조직이완|가동성|호흡|밸런스/.test(text))return "교정운동"; if(isFuncEx(ex))return "기능운동"; return "웨이트 트레이닝";}
...
function ExerciseReportSection({title,items,weight=false}){if(!items.length)return null; return <section className={weight?"exercise-report-section weight":"exercise-report-section assist"}><h3>{title}</h3>{items.map((e,i)=><div key={`${title}-${i}`} className={weight?"exercise-report":"assist-exercise-report"}><b>{e.name||e.movementPurpose||"운동"}</b>{weight?<div className="set-table">{(e.sets||[]).map((x,j)=>{const w=toPositiveNumber(x.weight); const r=toPositiveNumber(x.reps); const rawD=x.durationSec??x.duration??x.time??x.sec??x.seconds; const d=toPositiveNumber(rawD); if(!w&&!r&&!d)return null; return <span key={j}><strong>{j+1}세트</strong>{w?<i>{w}kg</i>:null}{r?<i>{r}회</i>:null}{!r&&d?<i>{d>=60&&d%60===0?`${d/60}분`:`${d}초`}</i>:null}</span>})}</div>:<p>{formatAssistExerciseDose(e)}</p>}{e.feedback&&<em>대표 코멘트: {e.feedback}</em>}{e.memo&&<em>{e.memo}</em>}</div>)}</section>}
function SessionMini({s}){const groups={"워밍업":[],"기능운동":[],"교정운동":[],"컨디셔닝":[],"웨이트 트레이닝":[]}; (s.exercises||[]).forEach(e=>groups[getMemberExerciseSection(e)]?.push(e)); return <div className="session-mini"><ExerciseReportSection title="워밍업" items={groups["워밍업"]}/><ExerciseReportSection title="기능운동" items={groups["기능운동"]}/><ExerciseReportSection title="교정운동" items={groups["교정운동"]}/><ExerciseReportSection title="컨디셔닝" items={groups["컨디셔닝"]}/><ExerciseReportSection title="웨이트 트레이닝" items={groups["웨이트 트레이닝"]} weight/>{s.sorenessReport&&<p className="soreness-summary">근육통 기록: {s.sorenessReport.part||'-'} · {s.sorenessReport.level||'-'} · {s.sorenessReport.timing||'-'}</p>}{s.trainerComment&&<p className="trainer-comment">대표 코멘트: {s.trainerComment}</p>}</div>}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  const memberId=member?.id;
  ...
  const onboardingBaseLine=ob?.completedAt?`${ob.gender||"-"} · ${formatAgeFromBirthYear(ob.birthYear)} · ${ob.heightCm||member?.height||"-"}cm`:"아직 완료 전";
...
function HubScreen({ member, allMembers, sessions, bodyData, nutritionData, loading, setScreen, onEdit, onMemberPatch }) {
  ...
  const menus = [
    ...
    {icon:"📨",label:"루틴 추천",   desc:"회원앱으로 대표 루틴 전송", sc:"routine_recommend", c:"#5EEAD4"},
    {icon:"🧘",label:"오늘의 컨디셔닝", desc:"매일 기능 운동 관리", sc:"daily_conditioning", c:"#22c55e"},
  ...
  return (
    <div>
      <div style={{background:"#111827",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"14px",marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            ...
            <div>
              ...
              <Mo c="#54546a" s={9}>등록일: {member.startDate || "-"}</Mo>
              <Mo c="#cbd5e1" s={10} style={{display:"block",marginTop:2}}>이메일: {member.email||"-"}</Mo>
              <Mo c={member.memberUid?"#86efac":"#fbbf24"} s={10} style={{display:"block",marginTop:2}}>회원앱: {getMemberAppStatus(member)}</Mo>
            </div>
          </div>
        ...
        {member.memo && <div style={{marginTop:8,padding:"9px 12px",background:"rgba(255,209,102,.08)",borderRadius:8,fontSize:12,lineHeight:1.55,color:"#f8d98a",border:"1px solid rgba(255,209,102,.14)",borderLeft:"3px solid rgba(255,209,102,.55)"}}>📝 {member.memo}</div>}
        <AdminMemberAppPanel member={member} members={allMembers} onAccountCreated={onMemberPatch} showManagement={showMemberAppManagement} />
      </div>
      {!loading && (() => {
        // ── 데이터 추출 ───────────────────────────────────────
        // 1. 수업 진행
      ...
      {member.survey?.surveyDone && (
        <button onClick={()=>setScreen("consultReport")}
          style={{width:"100%",marginBottom:12,padding:"11px 14px",borderRadius:10,cursor:"pointer",
            background:"rgba(94,234,212,.07)",border:"1px solid rgba(94,234,212,.18)",
            display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:16}}>🤖</span>
