...
function CalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const kcalCount=getKcalLogs(nutrition).length; const weightCount=getBodyWeightRecords(body).length; const learningDays=Math.max(0,Math.min(56,kcalCount?Math.min(kcalCount,56):0)); return <MCard title={admin?"🔥 전문가 칼로리 분석":"섭취 칼로리 분석"}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginBottom:12}}><KpiTile large label="유지 칼로리" value={formatKcal(a.maintenance)} sub="현재 추정 TDEE" accent="#ffd166"/><KpiTile large label="현재 목표 권장" value={formatKcal(target.value)} sub={target.shortLabel||target.label} accent="#5EEAD4"/></div><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8,marginBottom:12}}><MiniInsightTile label="신뢰도" value={`${a.confidence}%`} accent={a.learned?"#5EEAD4":"#ffd166"}/><MiniInsightTile label="데이터 기준" value={a.basis}/><MiniInsightTile label="학습 기간" value={`${learningDays}일`}/><MiniInsightTile label="최근 7일 평균" value={formatKcal(a.avg7)} accent="#5EEAD4"/><MiniInsightTile label="최근 30일 평균" value={formatKcal(a.avg30)} accent="#7c6fff"/><MiniInsightTile label="기록 표본" value={`${kcalCount}식 · ${weightCount}체중`}/></div><div style={{border:"1px solid rgba(94,234,212,.16)",background:"linear-gradient(135deg,rgba(94,234,212,.08),rgba(124,111,255,.07))",borderRadius:16,padding:"15px 14px"}}><Mo c="#e2e8f0" s={12} style={{display:"block",fontWeight:800,lineHeight:1.65}}>현재 기록이 부족하여<br/>공식 기반 초기값을 사용 중입니다.</Mo><Mo c="#94a3b8" s={10} style={{display:"block",lineHeight:1.75,marginTop:8}}>체중과 섭취칼로리 기록이 누적되면 실제 데이터 기반 유지칼로리로 자동 보정됩니다.</Mo></div></MCard>}
...
function SessionMini({s}){return <div className="session-mini">{(s.exercises||[]).map((e,i)=><div key={i} className="exercise-report"><b>{e.name||e.movementPurpose}</b><div className="set-table">{(e.sets||[]).map((x,j)=><span key={j}><strong>{j+1}세트</strong><i>{x.weight||'-'}kg</i><i>{x.reps||'-'}회</i></span>)}</div>{e.feedback&&<em>대표 코멘트: {e.feedback}</em>}</div>)}{s.sorenessReport&&<p className="soreness-summary">근육통 기록: {s.sorenessReport.part||'-'} · {s.sorenessReport.level||'-'} · {s.sorenessReport.timing||'-'}</p>}{s.trainerComment&&<p className="trainer-comment">대표 코멘트: {s.trainerComment}</p>}</div>}
