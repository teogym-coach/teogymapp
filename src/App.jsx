...
function MemberFeedbackForm({s,onSave}){const existing=s.memberFeedback||{}; const sorenessBodyPartsKey=JSON.stringify(existing.sorenessBodyParts||[]); const makeState=()=>({sorenessLevel:existing.sorenessLevel||"없음",sorenessBodyParts:memberFeedbackParts(existing),rpe:Number.isFinite(Number(existing.rpe))?Number(existing.rpe):5,memo:existing.memo||""}); const [d,setD]=useState(makeState); const [open,setOpen]=useState(false); useEffect(()=>setD(makeState()),[s.id,existing.sorenessLevel,existing.sorenessBodyPart,sorenessBodyPartsKey,existing.rpe,existing.memo]); const togglePart=part=>setD(prev=>({ ...prev, sorenessBodyParts: prev.sorenessBodyParts.includes(part)?prev.sorenessBodyParts.filter(x=>x!==part):[...prev.sorenessBodyParts,part] })); const summary=`근육통: ${formatSorenessBodyParts(existing)} / ${existing.sorenessLevel||"미입력"} · RPE: ${existing.rpe??"-"}`; const save=()=>onSave?.(s.id,{...d,sorenessBodyPart:d.sorenessBodyParts[0]||""}); return <div className="member-feedback-form compact-feedback"><div className="feedback-summary"><div><b>수업 후 상태</b><span>{existing.updatedAt||existing.createdAt?summary:"수업 후 상태 기록하기"}</span></div><button type="button" className="ghost compact" onClick={()=>setOpen(v=>!v)}>{open?"닫기":(existing.updatedAt||existing.createdAt?"수정":"기록하기")}</button></div>{open&&<div className="feedback-edit"><div className="form-line"><label>근육통 부위 <small>복수 선택 후 강도를 선택해주세요.</small></label><div className="choice-buttons">{SORENESS_BODY_PARTS.map(part=><button type="button" key={part} className={d.sorenessBodyParts.includes(part)?"active":""} onClick={()=>togglePart(part)}>{part}</button>)}</div></div><SelectLine label="근육통 세기" value={d.sorenessLevel} opts={SORENESS_LEVELS} onChange={v=>setD({...d,sorenessLevel:v})}/><div className="form-line"><label>RPE <details><summary>RPE란?</summary><small>운동이 얼마나 힘들게 느껴졌는지 0~10점으로 표시합니다. 0은 매우 쉬움, 10은 더 반복하기 어려움입니다.</small></details></label><div className="vas-buttons">{Array.from({length:11},(_,i)=><button type="button" key={i} className={Number(d.rpe)===i?"active":""} onClick={()=>setD({...d,rpe:i})}>{i}</button>)}</div></div><InputLine label="메모" value={d.memo} onChange={v=>setD({...d,memo:v})}/><button className="primary compact" onClick={save}>저장</button></div>}</div>}
function MemberAnalysis(p){const weightData=getBodyWeightRecords(p.body).map(r=>({date:r.date.slice(5),weight:r.weight})); return <><h1>변화분석</h1><p className="sub">체중 변화와 최근 3주 부위별 운동량을 확인해요</p><MCard title="체중 변화 그래프"><ResponsiveContainer width="100%" height={190}><LineChart data={weightData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis domain={["dataMin-2","dataMax+2"]}/><Tooltip/><Line dataKey="weight" name="체중" stroke="#2F73F6" strokeWidth={4} dot={{r:4}}/></LineChart></ResponsiveContainer></MCard><PartVolumeCard sessions={p.sessions}/><MCard title="대표 코멘트"><p>{p.latest?.trainerComment||"최근 수행량이 이전보다 안정적으로 올라왔습니다."}</p></MCard></>}

function PartVolumeCard({sessions=[]}){const parts=["등","가슴","어깨","하체","팔","코어"]; const rows=buildPartVolumeChart(sessions); const data=parts.map(part=>({part,values:rows.map(r=>({label:r.name,value:Math.round(r[part]||0)}))})); const max=Math.max(1,...data.flatMap(d=>d.values.map(v=>v.value))); const [selected,setSelected]=useState(data[0]); const colors=["#8B95A7","#DCEBFF","#34F5C5"]; return <section className="part-volume-card"><h2>이번주 부위별 운동량은 지난주보다 늘었을까?</h2><p>최근 3주 동안의 부위별 운동량 변화를 보여드려요.</p><p>막대를 누르면 해당 부위의 운동량을 확인할 수 있어요.</p><div className="part-volume-chart">{data.map(row=><div className="part-volume-group" key={row.part}><div className="part-volume-bars">{row.values.map((v,i)=><button type="button" aria-label={`${row.part} ${v.label} ${v.value}kg`} key={v.label} onClick={()=>setSelected(row)} style={{height:`${Math.max(8,(v.value/max)*132)}px`,background:colors[i]}} />)}</div><b>{row.part}</b></div>)}</div><div className="part-volume-legend"><span><i/>2주 전</span><span><i/>1주 전</span><span><i/>이번 주</span></div>{selected&&<div className="part-volume-detail"><b>{selected.part} 운동량</b>{selected.values.map(v=><span key={v.label}>{v.label}: {v.value.toLocaleString()}kg</span>)}</div>}</section>}
...
function getRecommendedPart(profile,sessions=[]){const info=getNextWorkoutInfo(profile); const days=info.daysUntil; const isNear=days!==null&&days<=2; const frequency=getWorkoutFrequencyNumber(profile); const lowFreq=frequency<=3; const parts=lowFreq?["가슴","등","하체","어깨","코어"]:["가슴","등","하체","어깨","코어"]; const avoid=new Set(["이두","삼두"]); if(days!==null&&days<=1)avoid.add(info.part); if(isNear){ if(info.part==="하체")avoid.add("등"); if(info.part==="등")avoid.add("하체"); if(info.part==="가슴")avoid.add("어깨"); if(info.part==="어깨")avoid.add("가슴"); }
  const preferred={하체:lowFreq?["가슴","코어"]:["가슴","코어","어깨"],등:lowFreq?["코어","가슴"]:["코어","가슴","어깨"],가슴:lowFreq?["하체","코어"]:["하체","코어","등"],어깨:lowFreq?["하체","등"]:["하체","등","코어"]};
  const counts=getRecentPartCounts(sessions); const pool=[...(isNear&&preferred[info.part]?preferred[info.part]:[]),...parts].filter((p,i,a)=>a.indexOf(p)===i&&!avoid.has(p)); return pool.sort((a,b)=>(counts[a]||0)-(counts[b]||0))[0]||"코어";}
...
function buildPartVolumeChart(sessions=[]){const labels=["2주 전","지난주","이번주"]; const now=new Date(); const startOfWeek=d=>{const x=new Date(d); const day=(x.getDay()+6)%7; x.setHours(0,0,0,0); x.setDate(x.getDate()-day); return x;}; const thisStart=startOfWeek(now); const ranges=labels.map((name,i)=>{const start=new Date(thisStart); start.setDate(start.getDate()-(2-i)*7); const end=new Date(start); end.setDate(end.getDate()+7); return {name,start,end};}); return ranges.map(r=>{const row={name:r.name}; ["가슴","등","어깨","하체","팔","코어"].forEach(part=>{row[part]=sessions.filter(s=>{const d=new Date(s.date); return d>=r.start&&d<r.end;}).reduce((sum,s)=>sum+(s.exercises||[]).filter(e=>normalizeWorkoutPart(e.muscleTop)===part).reduce((x,e)=>x+(e.sets||[]).reduce((v,st)=>v+(Number(st.volume)||Number(st.weight||0)*Number(st.reps||0)),0),0),0);}); return row;});}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:8,alignItems:"stretch"}}>
      ...
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.length?ci.map(c=><div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{c.date||c.id} · 체중 {c.weight||"-"}kg · 컨디션 {c.condition||"-"} · 근육통 {c.soreness||"없음"}</div>):<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>아직 체크인 기록이 없습니다</div>}</div>
    </div>
    ...
    <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo>{ms.map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{m.message}</div>)}</div>
  </div>
}
