...
function MemberApp({ onLogout }) {
  ...
  const weights=getBodyWeightRecords(body), curW=profile.currentWeight||weights.at(-1)?.weight||form.weight||profile.weight||"-", startW=profile.startWeight||weights[0]?.weight||curW; const totalReg=parseInt(String(profile.totalSessions||"").replace(/[^0-9]/g,""))||0; const remaining=totalReg?Math.max(0,totalReg-sessions.length):"-"; const latest=sessions.at(-1); const recentKcal=(nutrition?.logs||[]).find(l=>l.date===today)?.kcal||(nutrition?.logs||[]).at(-1)?.kcal||"-"; const steps=(checkins.find(c=>c.date===today||c.id===today)?.steps)||checkins.find(c=>c.steps)?.steps||"-";
...
function getMemberAppStatus(member){
  return member?.memberUid?"연결 완료":member?.memberAppAccountStatus==="auth-exists-missing-uid"||member?.memberAppAccountStatus==="invite-failed"?"확인 필요":member?.memberAppInviteSentAt?"초대 완료":"초대 전";
}
