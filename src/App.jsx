...
function initialMaintenanceCalories(profile={},onboarding={},body=null,checkins=[]){const weight=toPositiveNumber(onboarding.currentWeightKg)||toPositiveNumber(onboarding.startingWeightKg)||toPositiveNumber(profile.currentWeight)||getLatestBodyWeight(body)?.weight||toPositiveNumber(profile.weight)||70; const height=toPositiveNumber(onboarding.heightCm)||toPositiveNumber(profile.height)||170; const birth=Number(onboarding.birthYear||profile.birthYear||profile.birth); const age=Number.isFinite(birth)?Math.max(10,new Date().getFullYear()-birth):35; const gender=onboarding.gender||profile.gender||"남성"; const bmr=10*weight+6.25*height-5*age+(gender==="여성"?-161:5); return Math.round(bmr*activityFactorFromProfile(onboarding,checkins));}
...
function estimateMaintenance(profile={},onboarding={},body=null,nutrition=null,checkins=[],sessions=[]){const formula=initialMaintenanceCalories(profile,onboarding,body,checkins); const kcalLogs=getKcalLogs(nutrition); const weights=getBodyWeightRecords(body); const today=new Date(); const windows=[14,28,56]; const estimates=[]; windows.forEach(days=>{const since=new Date(today.getTime()-days*86400000).toISOString().slice(0,10); const kcals=kcalLogs.filter(x=>x.date>=since); const ws=weights.filter(x=>x.date>=since); if(kcals.length>=4&&ws.length>=2){const avg=kcals.reduce((a,b)=>a+b.kcal,0)/kcals.length; const first=ws[0],last=ws.at(-1); const span=Math.max(1,(new Date(last.date)-new Date(first.date))/86400000); const weekly=(last.weight-first.weight)/span*7; const maintenance=avg-(weekly*7700/7); estimates.push({days,maintenance,avg,weekly,quality:Math.min(1,kcals.length/days)*0.65+Math.min(1,ws.length/Math.max(2,days/7))*0.35});}}); const weighted=estimates.reduce((a,e)=>a+e.maintenance*e.quality*(e.days===28?1.15:e.days===56?0.9:1),0); const wsum=estimates.reduce((a,e)=>a+e.quality*(e.days===28?1.15:e.days===56?0.9:1),0); const learned=wsum?Math.round(weighted/wsum):null; const maintenance=learned||formula; const confidence=Math.round(Math.min(96,Math.max(42,(learned?58:35)+Math.min(30,kcalLogs.length*2)+Math.min(18,weights.length*3)+Math.min(8,sessions.length)))); const diet=Math.max(1200,Math.round(maintenance-500)); const bulk=Math.round(maintenance+300); const recent7=kcalLogs.slice(-7); const avg7=recent7.length?Math.round(recent7.reduce((a,b)=>a+b.kcal,0)/recent7.length):null; const recent28=kcalLogs.slice(-28); const avg28=recent28.length?Math.round(recent28.reduce((a,b)=>a+b.kcal,0)/recent28.length):null; const lastEstimate=estimates.find(e=>e.days===28)||estimates[0]; return {formula,maintenance,diet,maintain:maintenance,bulk,confidence,learned:!!learned,period:learned?`최근 ${lastEstimate?.days||28}일`:"공식 기반 초기값",avg7,avg28,weeklyChange:lastEstimate?.weekly??null,avgIntake:lastEstimate?.avg?Math.round(lastEstimate.avg):avg28};}
...
function CalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const weekly=a.weeklyChange; const pace=weekly===null?"기록 수집 중":`${weekly>0?"+":""}${weekly.toFixed(2)}kg/주`; return <MCard title={admin?"AI 칼로리 분석":"AI 섭취 칼로리 분석"}><div className="grid2"><Metric t="유지 칼로리" v={formatKcal(a.maintenance)}/><Metric t={target.label} v={formatKcal(target.value)}/></div><Info l="신뢰도" v={`${a.confidence}%`}/><Info l="데이터 학습 기간" v={a.period}/>{admin&&<><Info l="최근 7일 평균" v={formatKcal(a.avg7)}/><Info l="최근 28일 평균" v={formatKcal(a.avg28)}/><Info l="회원에게 표시 중인 값" v={`${target.label} · ${formatKcal(target.value)}`}/></>}<p className="notice soft">처음에는 공식으로 추정하고, 기록이 쌓이면 실제 체중 변화와 섭취 기록을 기준으로 자동 보정됩니다. {a.avgIntake?`최근 평균 ${formatKcal(a.avgIntake)} 섭취 기준 `:""}{pace!=="기록 수집 중"?`현재 체중 변화는 ${pace}입니다.`:""}</p></MCard>}
...
function CalorieTrendCard({profile,onboarding,body,nutrition,checkins,sessions}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const rows=getKcalLogs(nutrition).slice(-30).map(x=>({date:x.date.slice(5),kcal:x.kcal,target:target.value})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; const m7=mean(avg7),m30=mean(avg30); const chartRows=rows.map(r=>({...r,avg7:m7,avg30:m30})); return <MCard title="섭취 칼로리 변화 그래프"><ResponsiveContainer width="100%" height={180}><ComposedChart data={chartRows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis/><Tooltip/><Bar dataKey="kcal" name="일별 섭취 kcal" fill="#2F73F6" radius={[6,6,0,0]}/><Line type="monotone" dataKey="avg7" name="7일 평균" stroke="#5EEAD4" dot={false}/><Line type="monotone" dataKey="avg30" name="30일 평균" stroke="#8B5CF6" dot={false}/><Line type="monotone" dataKey="target" name="추천 기준" stroke="#FFB020" strokeDasharray="4 4" dot={false}/></ComposedChart></ResponsiveContainer><Info l="최근 7일 평균 섭취 칼로리" v={formatKcal(m7)}/><Info l="최근 30일 평균 섭취 칼로리" v={formatKcal(m30)}/><Info l="현재 감량 속도" v={a.weeklyChange===null?"기록 수집 중":`${a.weeklyChange>0?"+":""}${a.weeklyChange.toFixed(2)}kg/주`}/><Info l="현재 유지 추정 칼로리" v={formatKcal(a.maintenance)}/><Info l="현재 추천 칼로리" v={`${target.label} · ${formatKcal(target.value)}`}/></MCard>}
function AdminCalorieSummaryCard({member,bodyData,nutritionData,sessions=[],onOpen}){const a=estimateMaintenance(member,bodyData?.goal||{},bodyData,nutritionData,[],sessions); const target=getGoalCalorieRecommendation(a,bodyData?.goal?.goal||member?.goal||nutritionData?.goal); return <Card title="🤖 AI 칼로리 분석" style={{marginBottom:11}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}><div style={{background:"#0B1120",borderRadius:8,padding:"10px 12px"}}><Mo c="#54546a" s={8}>유지</Mo><div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:18,color:"#ffd166",marginTop:3}}>{a.maintenance.toLocaleString()} kcal</div></div><div style={{background:"#0B1120",borderRadius:8,padding:"10px 12px"}}><Mo c="#54546a" s={8}>{target.label.replace(" 칼로리","")}</Mo><div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:18,color:"#5EEAD4",marginTop:3}}>{target.value.toLocaleString()} kcal</div></div></div><button onClick={onOpen} style={{width:"100%",border:"1px solid rgba(94,234,212,.25)",background:"rgba(94,234,212,.08)",color:"#5EEAD4",borderRadius:8,padding:"9px 10px",fontWeight:800,cursor:"pointer"}}>자세히 보기 →</button></Card>}
...
function HubScreen({ member, allMembers, sessions, bodyData, nutritionData, loading, setScreen, onEdit, onMemberPatch }) {
  ...
  return (
    <div>
      ...
      <AdminCalorieSummaryCard member={member} bodyData={bodyData} nutritionData={nutritionData} sessions={sessions} onOpen={() => setScreen("healthhub")} />
        ...
        <div>
          ...
          {goal.currentWeight && (
            <div>
              ...
              <AdminCalorieSummaryCard member={member} bodyData={bodyData} nutritionData={nutritionData} sessions={sessions} onOpen={()=>setTab("AI 칼로리 분석")} />
