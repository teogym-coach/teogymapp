...
function AdminMemberAppPanel({member,members=[],onAccountCreated}){
  ...
  const memberAppStatus=member?.memberUid?"연결 완료":member?.memberAppAccountStatus==="auth-exists-missing-uid"||member?.memberAppAccountStatus==="invite-failed"?"확인 필요":member?.memberAppInviteSentAt?"초대 완료":"초대 전";
  const connectionStatus=member?.memberUid?"정상 연결":hasEmail?"연결 필요":"이메일 등록 필요";
  const statusColor=member?.memberUid?"#86efac":memberAppStatus==="확인 필요"?"#fbbf24":"#93c5fd";
  return <div style={{marginTop:10,display:"grid",gap:8}}><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#60a5fa" s={9}>회원앱 관리</Mo><div style={{display:"grid",gap:5,marginTop:8}}><div style={{fontSize:11,color:statusColor}}>회원앱 상태: {memberAppStatus}</div><div style={{fontSize:11,color:"#cbd5e1"}}>회원 이메일: {hasEmail?currentEmail:"없음"}</div><div style={{fontSize:11,color:member?.memberUid?"#86efac":"#fbbf24"}}>연결 상태: {connectionStatus}</div></div>{!hasEmail&&<div style={{fontSize:11,color:"#fbbf24",marginTop:6}}>회원 이메일을 먼저 등록해주세요.</div>}<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}><Btn sm onClick={sendInvite} disabled={busy||!hasEmail}>{busy?"처리 중...":"회원앱 초대 보내기"}</Btn><Btn ghost sm onClick={cleanupTeoGym12} disabled={busy} style={{color:"#5EEAD4",borderColor:"#5EEAD444"}}>회원앱 이메일 분리</Btn></div><div style={{marginTop:8,padding:8,borderRadius:8,background:"rgba(94,234,212,.06)",border:"1px solid rgba(94,234,212,.18)"}}><Mo c="#5EEAD4" s={9}>memberUid 저장</Mo><div style={{display:"flex",gap:6,marginTop:6}}><input value={manualUid} onChange={e=>setManualUid(e.target.value)} placeholder="auth.uid 입력" style={{flex:1,minWidth:180,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:6,color:"#e5e7eb",padding:"7px 8px",fontSize:11}}/><Btn ghost sm onClick={saveManualMemberUid} disabled={busy||!manualUid.trim()} style={{color:"#5EEAD4",borderColor:"#5EEAD444"}}>memberUid 저장</Btn></div></div>{msg&&<div style={{fontSize:11,color:msg.includes("완료")||msg.includes("발송했습니다")?"#86efac":"#ff9f43",marginTop:6}}>{msg}</div>}<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}><Btn ghost sm onClick={()=>setShowDiagnostics(v=>!v)} style={{color:"#94a3b8",borderColor:"#334155"}}>{showDiagnostics?"상세 관리 숨기기":"상세 관리 보기"}</Btn></div>{showDiagnostics&&<div style={{marginTop:8,padding:8,borderRadius:8,background:"rgba(255,255,255,.04)",display:"grid",gap:6,fontSize:10,color:"#94a3b8"}}><div><b style={{color:"#cbd5e1"}}>문제 해결 정보</b></div>{emailChangedWithUid&&<div>현재 이메일과 과거 초대 이메일이 다릅니다. 과거 이메일: {linkedEmail}</div>}<div>auth UID: {identityDiagnostics.current?.authUid||"-"}</div><div>memberUid 상세 값: {identityDiagnostics.current?.memberUid||"-"}</div><div>UID 점검: {identityDiagnostics.current?.memberUidMatchesAuthUid?"memberUid와 Auth UID가 일치합니다.":"불일치 또는 관리자 Auth로 확인 중입니다."}</div><div>연결 흐름: 회원 이메일 확인 → Firebase Auth 사용자 생성 확인 → 비밀번호 설정/재설정 메일 발송 → members.memberUid 저장</div><div>중복 email 그룹: {identityDiagnostics.duplicateEmails.length?identityDiagnostics.duplicateEmails.map(g=>`${g.value} (${g.members.map(m=>m.name||m.id).join(" · ")})`).join(" / "):"없음"}</div><div>중복 memberUid 그룹: {identityDiagnostics.duplicateMemberUids.length?identityDiagnostics.duplicateMemberUids.map(g=>`${g.value} (${g.members.map(m=>m.name||m.id).join(" · ")})`).join(" / "):"없음"}</div><div>Firestore 경로: members/{memberId||"memberId"}</div>{duplicateEmailMembers.length>0&&<div style={{color:"#ff6b6b"}}>현재 이메일 중복 회원: {duplicateEmailMembers.map(m=>m.name||m.id).join(" · ")}</div>}{duplicateUidMembers.length>0&&<div style={{color:"#ff6b6b"}}>현재 memberUid 중복 회원: {duplicateUidMembers.map(m=>m.name||m.id).join(" · ")}</div>}{inviteLog.length>0&&<div style={{display:"grid",gap:4}}>디버그 로그: {inviteLog.map((l,i)=><div key={i} style={{color:l.ok?"#86efac":"#ff6b6b"}}>{l.ok?"✓":"!"} {l.at} · {l.text}</div>)}</div>}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#5EEAD4" s={9}>다음 PT 예정 부위</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원앱 개인운동 추천에서 우선 제외할 다음 수업 예정 부위입니다.</div><select value={member?.nextPtPart||"미정"} onChange={e=>saveNextPtPart(e.target.value)} disabled={busy} style={{width:"100%",marginTop:8,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:12}}>{NEXT_PT_PART_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#a78bfa" s={9}>회원앱 온보딩</Mo><div style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{ob?.completedAt?`${ob.gender||"-"} · ${ob.birthYear||"-"}년생 · ${ob.heightCm||"-"}cm · ${ob.startingWeightKg||"-"}kg · ${ob.goal||"-"} · ${ob.weeklyWorkoutCount||"-"}`:"아직 완료 전"}</div>{ob?.focusAreas?.length>0&&<div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>집중 부위: {ob.focusAreas.join(" · ")}</div>}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#5EEAD4" s={9}>회원앱 최근 체크인</Mo>{ci.map(c=><div key={c.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{c.date||c.id} · {c.weight||"-"}kg · {c.condition||"-"} · 근육통 {c.soreness||"-"}</div>)}</div><div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)"}}><Mo c="#ffd166" s={9}>회원앱 소통</Mo>{ms.map(m=><div key={m.id} style={{fontSize:11,color:"#cbd5e1",marginTop:5}}>{m.message}</div>)}</div></div>
}
