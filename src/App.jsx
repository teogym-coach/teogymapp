...
function getKcalLogs(nutrition={}){const byDate=new Map(); (nutrition?.logs||[]).forEach(l=>{const kcal=toPositiveNumber(l.kcal||l.totalKcal||l.memberInputKcal); if(l.date&&kcal)byDate.set(l.date,{date:l.date,kcal});}); Object.entries(nutrition?.dates||{}).forEach(([date,d])=>{const kcal=toPositiveNumber(d.totalKcal||d.memberInputKcal||d.kcal); if(kcal&&!byDate.has(date))byDate.set(date,{date,kcal});}); return [...byDate.values()].sort((a,b)=>String(a.date).localeCompare(String(b.date)));}
...
function CalorieAnalysisCard({profile,onboarding,body,nutrition,checkins,sessions,admin=false}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const weekly=a.weeklyChange; const pace=weekly===null?"기록 수집 중":`${weekly>0?"+":""}${weekly.toFixed(2)}kg/주`; return <MCard title={admin?"AI 칼로리 분석":"AI 섭취 칼로리 분석"}><div className="grid2"><Metric t="유지 칼로리" v={formatKcal(a.maintenance)}/><Metric t="다이어트 권장" v={formatKcal(a.diet)}/><Metric t="체중 유지 권장" v={formatKcal(a.maintain)}/><Metric t="벌크업 권장" v={formatKcal(a.bulk)}/></div><Info l="신뢰도" v={`${a.confidence}%`}/><Info l="데이터 학습 기간" v={a.period}/>{admin&&<><Info l="최근 7일 평균" v={formatKcal(a.avg7)}/><Info l="최근 28일 평균" v={formatKcal(a.avg28)}/><Info l="회원에게 표시 중인 값" v={formatKcal(a.maintenance)}/></>}<p className="notice soft">현재 유지칼로리는 약 <b>{formatKcal(a.maintenance)}</b>로 추정됩니다. {a.avgIntake?`최근 평균 ${formatKcal(a.avgIntake)} 섭취 기준 `:""}{pace!=="기록 수집 중"?`체중 변화는 ${pace}입니다.`:"체중·섭취 기록이 쌓이면 실제 데이터 우선으로 자동 보정됩니다."}</p></MCard>}
function CalorieTrendCard({nutrition}){const rows=getKcalLogs(nutrition).slice(-31).map(x=>({date:x.date.slice(5),kcal:x.kcal})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; return <MCard title="섭취 칼로리 변화"><ResponsiveContainer width="100%" height={180}><BarChart data={rows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis/><Tooltip/><Bar dataKey="kcal" name="섭취 kcal" fill="#2F73F6" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer><Info l="주간 평균" v={formatKcal(mean(avg7))}/><Info l="월간 평균" v={formatKcal(mean(avg30))}/></MCard>}
...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">대표님이 관리하는 변화 리포트입니다 · 오늘 상태도 건강관리 탭에서 기록하세요</p><CalorieAnalysisCard {...p}/><div className="hero-card pt-progress"><b>대표님과 함께한 수업</b><strong>{done}회</strong>{p.totalReg?<><em>PT 진행률 {pct}% · 남은 PT {p.remaining}회</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 수업 기록이 쌓이고 있어요</em>}</div><div className="grid2 top-metrics"><ChangeReportMetric startW={p.startW} curW={p.curW}/><GoalMetric goal={p.onboarding.goal||"꾸준한 변화"}/><NextWorkoutMetric profile={p.profile}/></div><ReviewRoutine {...p}/><DailyConditioningCard items={p.dailyConditioning}/></>}
...
function HubScreen({ member, allMembers, sessions, bodyData, nutritionData, loading, setScreen, onEdit, onMemberPatch }) {
  ...
  return (
    <div>
      ...
      <CalorieAnalysisCard profile={member} onboarding={bodyData?.goal||{}} body={bodyData} nutrition={nutritionData} checkins={[]} sessions={sessions} admin />
