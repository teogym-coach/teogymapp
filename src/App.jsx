...
function MemberApp({ onLogout }) {
  ...
  const saveCheck=async()=>{const weightValue=String(form.weight??"").trim(); const normalizedWeight=weightValue.replace(",","."); const parsedWeight=normalizedWeight===""?null:Number(normalizedWeight); if(weightValue!==""&&(!Number.isFinite(parsedWeight)||parsedWeight<=0)){alert("체중은 0보다 큰 숫자로 입력해주세요.");return;} try{const savedBody=await saveMemberHealthInputs(profile.id,today,{weight:normalizedWeight,kcal:form.kcal,steps:form.steps}); if(savedBody){setBody(savedBody); setForm(f=>({...f,weight:String(parsedWeight)}));}else if(parsedWeight){setBody(prev=>({...(prev||{}),records:upsertBodyRecord(prev?.records||[],{id:`member_${today}`,date:today,weight:parsedWeight,note:"회원앱 직접 입력"})})); setForm(f=>({...f,weight:String(parsedWeight)}));} const painRecord={part:form.painPart,side:form.painSide,vas:Number(form.painVas)||0,memo:form.painMemo||""}; await saveMemberCheckin(profile.id,today,{...form,weight:normalizedWeight,painRecord}); await load(); alert("건강관리 기록이 저장됐어요");}catch(e){console.error("[MemberApp] health save failed",e); alert(`건강관리 저장 실패: ${e?.message||"알 수 없는 오류"}`);}};
...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; const nextPart=getNextPtPart(p.profile); return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">대표님이 관리하는 변화 리포트입니다 · 오늘 상태도 건강관리 탭에서 기록하세요</p><div className="hero-card pt-progress"><b>대표님과 함께한 수업</b><strong>{done}회</strong>{p.totalReg?<><em>PT 진행률 {pct}% · 남은 PT {p.remaining}회</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 수업 기록이 쌓이고 있어요</em>}</div><div className="grid2 top-metrics"><Metric t="체중 변화" v={`${p.startW} → ${p.curW}kg`}/><Metric t="오늘 목표" v={p.onboarding.goal||"꾸준한 변화"}/><Metric t="다음 PT" v={nextPart}/></div><ReviewRoutine {...p}/><MCard title="대표 피드백"><p>{p.latest?.trainerComment||"공개 수업일지의 대표 코멘트가 여기에 표시됩니다."}</p></MCard></>}
...
function getNextPtPart(profile){return profile?.nextPtPart&&profile.nextPtPart!=="미정"?normalizeWorkoutPart(profile.nextPtPart):"하체";}
...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const saveNextPtPart=async(value)=>{setBusy(true); setMsg(""); try{const patch={nextPtPart:value,nextPtPartUpdatedAt:new Date().toISOString()}; await updateMember(memberId,patch); onAccountCreated?.(patch); setMsg(`다음 PT 예정 부위 저장 완료 · ${value}`);}catch(e){setMsg(`다음 PT 예정 부위 저장 실패 · ${e?.message||String(e)}`);}finally{setBusy(false);}};
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:8,alignItems:"stretch"}}>
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column"}}><Mo c="#5EEAD4" s={9}>다음 PT 예정 부위</Mo><div style={{fontSize:10,color:"#94a3b8",marginTop:4}}>회원앱 개인운동 추천에서 우선 제외할 다음 수업 예정 부위입니다.</div><select value={member?.nextPtPart||"미정"} onChange={e=>saveNextPtPart(e.target.value)} disabled={busy} style={{width:"100%",marginTop:8,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:12}}>{NEXT_PT_PART_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></div>
