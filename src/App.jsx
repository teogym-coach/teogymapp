...
import {
  ...
  getMemberOnboarding, saveMemberOnboarding, saveSessionSoreness, saveSessionMemberFeedback, saveMemberHealthInputs, saveMemberProfileFields, prepareMemberAppEmailRelink, buildMemberIdentityDiagnostics, getRoutineRecommendations, saveRoutineRecommendation, getDailyConditioning, saveDailyConditioning,
} from "./db";
...
function MemberApp({ onLogout }) {
  ...
  const common={profile,sessions,body,nutrition,checkins,onboarding:effectiveOnboarding,routineRecommendations,dailyConditioning,curW,startW,totalReg,remaining,latest,recentKcal,steps,form,setForm,saveCheck,saveSoreness,saveFeedback,saveProfileInfo,onLogout,setTab,accessErrors};
  return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><main className="member-page">{debugPanel}{tab==="home"&&<MemberHome {...common}/>} {tab==="workout"&&<MemberWorkout {...common}/>} {tab==="health"&&<MemberHealth {...common}/>} {tab==="analysis"&&<MemberAnalysis {...common}/>} {tab==="profile"&&<MemberProfile {...common}/>}</main><nav className="member-nav">{[["home","⌂","홈"],["workout","＋","수업일지"],["health","♥","건강관리"],["analysis","▥","변화분석"],["profile","●","프로필"]].map(([k,i,l])=><button key={k} onClick={()=>setTab(k)} className={tab===k?"active":""}><span>{i}</span>{l}</button>)}</nav></div>;
}
...
function PainInput({form,setForm}){return <div className="pain-input"><SelectLine label="통증 부위" value={form.painPart} opts={PAIN_PARTS} onChange={v=>setForm({...form,painPart:v})}/><div className="form-line"><label>좌/우</label><div className="choice-buttons">{PAIN_SIDES.map(x=><button type="button" key={x} className={form.painSide===x?"active":""} onClick={()=>setForm({...form,painSide:x})}>{x}</button>)}</div></div><div className="form-line"><label>VAS 0~10 <small>0 통증 없음 · 10 가장 심함</small></label><div className="vas-buttons">{Array.from({length:11},(_,i)=><button type="button" key={i} className={Number(form.painVas)===i?"active":""} onClick={()=>setForm({...form,painVas:i})}>{i}</button>)}</div></div><InputLine label="통증 메모" value={form.painMemo} onChange={v=>setForm({...form,painMemo:v})}/></div>}
