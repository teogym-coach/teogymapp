...
export default function App() {
  ...
  async function handleMigrate() {
    if (!window.confirm("기존 회원 전체에 trainerUid를 추가합니다.\n이미 있는 회원은 건드리지 않습니다.\n계속하시겠습니까?")) return;
    setMigrating(true);
    try {
      const count = await migrateAddTrainerUid();
      if (count === 0) {
        showToast("모든 회원이 이미 정상 상태입니다 ✓");
      } else {
        showToast(`${count}명 업데이트 완료 ✓ (콘솔 확인)`);
      }
    } catch(e) {
      showToast("마이그레이션 실패: " + e.message, "err");
      ...
      <nav className="noprint" style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={handleMigrate} disabled={migrating}
            ...
            style={{background:"none",border:"1px solid #1a1a2e",borderRadius:6,
              color:migrating?"#3a3a4a":"#3a3a5a",fontSize:9,fontWeight:700,
              padding:"5px 9px",cursor:migrating?"not-allowed":"pointer"}}
            id="migrate-btn">
            {migrating ? "⏳" : "🔧"}
          </button>
...
function HomeScreen({ setScreen, loadMembers }) {
  return (
    <div style={{textAlign:"center",paddingTop:52}}>
      ...
      <div style={{maxWidth:320,margin:"16px auto 0",padding:"10px 14px",
        background:"#0d0d14",borderRadius:8,border:"1px dashed #1a1a24",
        fontSize:10,color:"#2a2a3a",lineHeight:1.8,textAlign:"left"}}>
        <span style={{color:"#3a3a5a",fontWeight:700}}>🔧 우측 상단 렌치 버튼</span>
        <br/>
        <span style={{color:"#2a2a3a"}}>기존 데이터 마이그레이션 (최초 1회만 실행)</span>
      </div>
    </div>
  );
}
...
function MembersScreen({ members, sessionsMap, loading, onSelect, onAdd, onRefresh, onDelete }) {
  ...
  function getMemberMeta(m) {
    ...
    const lastMuscle = last ? (last.exercises||[]).map(e=>e.muscleTop).filter(Boolean)
      .filter((v,i,a)=>a.indexOf(v)===i).slice(0,2).join("·") : null;
        ...
        {exercises.map((ex, ei) => (
          <div key={ei}
            ...
            {ex.name && (() => {
              ...
              // 불러오기 핸들러 (단일 setExercises 호출로 원자적 업데이트)
              function loadRecord() {
                if (!rec) return;
                ...
                const newSets = rec.sets.map(s => {
                  ...
                  const r = String(s.reps   ?? "");
                  const vol = calcVol(w, r, exType, mbw);
                  return { weight: w, reps: r, volume: vol };
                });
                setExercises(prev => prev.map((e, i) => {
                  if (i !== ei) return e;
                  const newTop = rec.muscleTop || e.muscleTop;
                  return {
                    ...e,
                    sets:      newSets,
                    rpe:       rec.rpe       || e.rpe,
                    feedback:  rec.feedback  || e.feedback,
            ...
            <div key={s.id||i} style={{background:"#111116",border:"1px solid #1a1a24",borderRadius:10,padding:"11px 13px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:14}}>{formatTypes(s.selectedTypes || s.type) || "웨이트"}</div>
