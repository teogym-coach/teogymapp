...
function MemberHome(p){const done=p.sessions.length; const pct=p.totalReg?Math.min(100,Math.round(done/p.totalReg*100)):0; return <><h1>안녕하세요 {p.profile.name}님</h1><p className="sub">오늘 필요한 것만 바로 확인하세요.</p><NoticeCard notices={p.notices} onOpen={p.openNotice} onAll={()=>p.setTab("profile")}/><div className="hero-card pt-progress"><b>남은 PT</b><strong>{p.remaining}회</strong>{p.totalReg?<><em>진행 {done}/{p.totalReg}회 · {pct}%</em><i><small style={{width:`${pct}%`}}/></i></>:<em>누적 {done}회 진행</em>}</div><div className="grid2 top-metrics"><CurrentWeightMetric curW={p.curW}/><GoalWeightForecastCard {...p}/><NextWorkoutMetric profile={p.profile}/><GoalMetric goal={p.onboarding.goal||"꾸준한 변화"}/></div><ReviewRoutine {...p}/><DailyConditioningCard items={p.dailyConditioning}/></>}
...
function ReviewRoutine({profile,sessions,onboarding,checkins,routineRecommendations=[]}){const today=getKoreaDateString(); const visibleRoutines=(routineRecommendations||[]).filter(r=>isPublishedData(r)&&String(r.date||"")>=today).sort((a,b)=>String(a.date||"").localeCompare(String(b.date||""))); const publishedRoutine=visibleRoutines.find(r=>r.date===today)||visibleRoutines[0]; const upcomingRoutines=visibleRoutines.filter(r=>r.id!==publishedRoutine?.id&&r.date>today).slice(0,4); const info=getNextWorkoutInfo(profile); const recommended=getRecommendedPart(profile,sessions); const [selected,setSelected]=useState(recommended); const rec=buildReviewRoutine(sessions,onboarding,checkins,selected||recommended); const daysText=info.daysUntil===null?"아직 정해지지 않았습니다":info.daysUntil===0?"오늘입니다":`${Math.abs(info.daysUntil)}일${info.daysUntil<0?" 지났습니다":"입니다"}`; return <MCard title="오늘의 추천 루틴"><div className="workout-guide compact"><p>다음 수업 <b>{info.part}</b> · <b>{daysText}</b></p></div><div className="part-pills">{["가슴","등","하체","어깨","이두","삼두","코어"].map(x=><button key={x} className={selected===x?"active":recommended===x?"recommended":""} onClick={()=>{setSelected(x); scrollMemberAppToTop();}}>{x}</button>)}</div><CoachRoutineCard routine={publishedRoutine}/>{upcomingRoutines.length>0&&<div className="rec-group"><b>앞으로의 추천</b>{upcomingRoutines.map(r=><p key={r.id}><span>{String(r.date).slice(5)} · {formatParts(r)}</span></p>)}</div>}<h3>{selected} 추천</h3>{!rec.hasClassSessions?<p className="notice soft">추천 루틴를 만들 데이터가 부족합니다.</p>:!rec.hasData?<p className="notice soft">추천 루틴를 만들 데이터가 부족합니다.</p>:<><div className="routine-list">{rec.routine.map((x,i)=><div className="routine-row" key={i}><b>{x.name}</b><div className="routine-sets">{x.sets.map((st,j)=><span key={j}><strong>{st.label}</strong><i>{st.weight}</i><i>{st.reps}</i></span>)}</div></div>)}</div><div className="rec-reasons"><b>추천 이유</b><span>최근 자극이 좋았던 운동입니다.</span><span>통증 기록이 없습니다.</span><span>다음 PT 전 회복을 고려했습니다.</span></div></>}<button type="button" className="ghost compact" onClick={()=>{const parts=["가슴","등","하체","어깨","이두","삼두","코어"]; const i=parts.indexOf(selected); setSelected(parts[(i+1)%parts.length]); scrollMemberAppToTop();}}>다른 추천 보기</button></MCard>}
...
function SessionMini({s}){const groups={"움직임 준비":[],"웨이트 트레이닝":[]}; (s.exercises||[]).forEach(e=>groups[getMemberExerciseSection(e)]?.push(e)); return <div className="session-mini"><ExerciseReportSection title="움직임 준비" items={groups["움직임 준비"]}/><ExerciseReportSection title="웨이트 트레이닝" items={groups["웨이트 트레이닝"]} weight/>{s.sorenessReport&&<p className="soreness-summary">근육통 기록: {s.sorenessReport.part||'-'} · {s.sorenessReport.level||'-'} · {s.sorenessReport.timing||'-'}</p>}{s.trainerComment&&<p className="trainer-comment">대표 코멘트: {s.trainerComment}</p>}</div>}
