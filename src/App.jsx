  ...
  return (
    <div style={{minHeight:"100vh",background:"#09090c"}}>
      ...
      <div className="noprint" style={{
        ...
        {screen==="bodycheck"  && member && <BodyCheckScreen member={member} onBack={() => setScreen("hub")} bodyData={bodyData} onSaveBodyData={async d => { try { await saveBodyCheck(member.id, d); setBodyData(d); } catch(e) { showToast(e.message || "저장 실패", "err"); } }} showToast={showToast} />}
      </div>
    ...
    setExercises(prev => prev.map((ex,i) => {
      ...
      const sets   = ex.sets.map((row,j) => {
        ...
        if (key==="weight"||key==="reps") {
          ...
          const mbw = bodyWeight || (sessions.length>0 ? sessions[sessions.length-1]?.bodyWeight : "") || "";
        ...
        {exercises.map((ex, ei) => (
          <div key={ei} style={{background:"#09090c",border:"1px solid #1a1a24",borderRadius:10,padding:11,marginBottom:9}}>
            ...
            {ex.sets.map((row, si) => {
              ...
              const mbwRow    = bodyWeight || (sessions.length>0 ? sessions[sessions.length-1]?.bodyWeight : "") || "";
            ...
            {(() => {
              ...
              const mbw3    = bodyWeight || (sessions.length>0 ? sessions[sessions.length-1]?.bodyWeight : "") || "";
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:7}}>
                    ...
                    {exType3==="assist" && (
                      <span style={{fontFamily:"'DM Mono',monospace",fontSize:7,padding:"2px 6px",borderRadius:4,
                        background:"rgba(124,111,255,.18)",color:"#7c6fff"}}>어시스트 보정</span>
                    )}
                  </div>
                </div>
              );
