...
function CoachRoutineCard({routine}){if(!routine)return null; return <div className="coach-routine-card"><h3>대표 추천 루틴</h3><p>오늘은 이 루틴으로 진행해주세요.</p>{(routine.exercises||[]).map((ex,i)=><div className="routine-row coach" key={i}><b>{ex.name||"운동"}</b><div className="routine-sets">{(ex.sets||[]).map((st,j)=><span key={j}>{formatRoutineSet(st,j)}</span>)}</div></div>)}{routine.coachComment&&<p className="coach-comment"><b>대표 코멘트:</b><br/>{routine.coachComment}</p>}</div>}
function DailyConditioningCard({items=[]}){const today=getKoreaDateString(); const item=(items||[]).find(x=>(x.date||x.id)===today&&isPublishedData(x)); if(!item)return null; return <MCard title="오늘의 컨디셔닝"><div className="conditioning-card"><b>{item.exerciseName||item.title||"기능 운동"}</b><p>{[item.sets&&`${item.sets}세트`,item.reps&&`${item.reps}회`,item.duration].filter(Boolean).join(" × ")}</p>{item.description&&<p>{item.description}</p>}{item.caution&&<em>주의사항: {item.caution}</em>}</div></MCard>}
...
function RoutineRecommendScreen({member,sessions,onBack,showToast}){const today=getKoreaDateString(); const [date,setDate]=useState(today); const [targetPart,setTargetPart]=useState("가슴"); const [exercises,setExercises]=useState([emptyRoutineExercise()]); const [coachComment,setCoachComment]=useState(""); const [saving,setSaving]=useState(false); const setName=(i,name)=>{setExercises(prev=>prev.map((ex,idx)=>idx===i?{...ex,name,sets:findLastExerciseSets(sessions,name)||ex.sets||emptyRoutineExercise().sets}:ex));}; const setSet=(ei,si,k,v)=>setExercises(prev=>prev.map((ex,i)=>i===ei?{...ex,sets:(ex.sets||[]).map((st,j)=>j===si?{...st,[k]:v}:st)}:ex)); const save=async(publish)=>{setSaving(true); try{const saved=await saveRoutineRecommendation(member.id,null,{date,targetPart,exercises,coachComment,status:publish?"published":"draft"},publish); if(publish&&!isPublishedData(saved)) throw new Error("전송 실패: 저장 경로 또는 권한을 확인해주세요."); showToast(publish?"회원앱으로 루틴 추천을 전송했습니다.":"루틴 추천을 임시 저장했습니다.");}catch(e){console.error(e); showToast(publish?"전송 실패: 저장 경로 또는 권한을 확인해주세요.":"임시 저장에 실패했습니다.","err");}finally{setSaving(false);}}; return <div><SH title="📨 루틴 추천" sub={`${member.name} 회원앱 전송`} right={<Btn ghost sm onClick={onBack}>← 뒤로</Btn>}/><Card title="대표 추천 루틴 작성"><div style={{display:"grid",gap:10}}><Field label="추천 날짜" type="date" value={date} onChange={setDate}/><SelectLine label="추천 부위" value={targetPart} opts={["가슴","등","하체","어깨","이두","삼두","코어","전신"]} onChange={setTargetPart}/>{exercises.map((ex,ei)=><div key={ei} style={{background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,padding:12}}><Field label="운동 종목" value={ex.name} onChange={v=>setName(ei,v)} placeholder="덤벨 인클라인 벤치프레스"/><div style={{display:"grid",gap:6}}>{(ex.sets||[]).map((st,si)=><div key={si} style={{display:"grid",gridTemplateColumns:"50px 1fr 1fr",gap:6,alignItems:"end"}}><Mo c="#94a3b8" s={10}>{si+1}세트</Mo><Field label="중량" value={st.weight} onChange={v=>setSet(ei,si,"weight",v)}/><Field label="횟수" value={st.reps} onChange={v=>setSet(ei,si,"reps",v)}/></div>)}</div><Btn ghost sm onClick={()=>setExercises(prev=>prev.filter((_,i)=>i!==ei))}>운동 삭제</Btn></div>)}<Btn ghost onClick={()=>setExercises(prev=>[...prev,emptyRoutineExercise()])}>+ 운동 추가</Btn><TextArea label="대표 코멘트" value={coachComment} onChange={setCoachComment}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Btn ghost disabled={saving} onClick={()=>save(false)}>임시 저장</Btn><Btn disabled={saving} onClick={()=>save(true)}>회원앱 전송</Btn></div></div></Card></div>}
function DailyConditioningAdminScreen({member,onBack,showToast}){const today=getKoreaDateString(); const [form,setForm]=useState({date:today,exerciseName:"",description:"",sets:"2",reps:"10",duration:"",caution:""}); const [scope,setScope]=useState("global"); const [saving,setSaving]=useState(false); const set=(k,v)=>setForm(f=>({...f,[k]:v})); const save=async(publish)=>{setSaving(true); try{const saved=await saveDailyConditioning(form,{memberId:scope==="member"?member.id:null,publish}); if(publish&&!isPublishedData(saved)) throw new Error("전송 실패: 저장 경로 또는 권한을 확인해주세요."); showToast(publish?"오늘의 컨디셔닝을 회원앱으로 전송했습니다.":"오늘의 컨디셔닝을 임시 저장했습니다.");}catch(e){console.error(e); showToast(publish?"전송 실패: 저장 경로 또는 권한을 확인해주세요.":"오늘의 컨디셔닝 저장에 실패했습니다.","err");}finally{setSaving(false);}}; return <div><SH title="🧘 오늘의 컨디셔닝" sub="매일 기능 운동 관리" right={<Btn ghost sm onClick={onBack}>← 뒤로</Btn>}/><Card title="컨디셔닝 입력"><div style={{display:"grid",gap:10}}><SelectLine label="전송 대상" value={scope} opts={["global","member"]} onChange={setScope}/><Field label="날짜" type="date" value={form.date} onChange={v=>set("date",v)}/><Field label="운동명" value={form.exerciseName} onChange={v=>set("exerciseName",v)} placeholder="데드버그 밴드 Y레이즈"/><TextArea label="설명" value={form.description} onChange={v=>set("description",v)}/><Field label="세트" value={form.sets} onChange={v=>set("sets",v)}/><Field label="횟수" value={form.reps} onChange={v=>set("reps",v)}/><Field label="시간" value={form.duration} onChange={v=>set("duration",v)}/><TextArea label="주의사항" value={form.caution} onChange={v=>set("caution",v)}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Btn ghost disabled={saving} onClick={()=>save(false)}>임시 저장</Btn><Btn disabled={saving} onClick={()=>save(true)}>회원앱 전송</Btn></div></div></Card></div>}
