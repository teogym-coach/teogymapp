...
function MetabolismScreen({ member, sessions=[], onBack }) {
  ...
  const cutoff = new Date(today); cutoff.setDate(today.getDate() - range);
  const cutStr = cutoff.toISOString().split("T")[0];

  const recent = sessions.filter(s => (s.date||"") >= cutStr);
  const older  = sessions.filter(s => (s.date||"") <  cutStr);

  // 체중 데이터 (전체 세션 기준 날짜별)
  const weightData = sessions
    .filter(s => parseFloat(s.bodyWeight) > 0)
    .map(s => ({ date: s.date, w: parseFloat(s.bodyWeight) }))
    .sort((a,b) => a.date.localeCompare(b.date));

  // 유산소 데이터
  const cardioData = sessions
    .filter(s => s.cardio && (s.cardio.minutes > 0))
    .map(s => ({ date: s.date, min: s.cardio.minutes||0, kcal: s.cardio.calories||0 }))
    .sort((a,b) => a.date.localeCompare(b.date));

  // 최근/이전 기간 평균 체중
  const avgW = (arr) => {
    const ws = arr.filter(s=>parseFloat(s.bodyWeight)>0).map(s=>parseFloat(s.bodyWeight));
    return ws.length ? (ws.reduce((a,b)=>a+b,0)/ws.length) : null;
  };
  const wRecent = avgW(recent), wOlder = avgW(older);
  const wDiff = (wRecent && wOlder) ? (wRecent - wOlder) : null;

  // 유산소 합계
  const cardioSum = (arr) => arr.reduce((s,ss)=>s+(ss.cardio?.minutes||0),0);
  const cardioKcalSum = (arr) => arr.reduce((s,ss)=>s+(ss.cardio?.calories||0),0);
  const cardioRecent = cardioSum(recent);
  const cardioOlder  = cardioSum(older);
  const cardioKcalRecent = cardioKcalSum(recent);

  // 웨이트 볼륨
  const volSum = (arr) => arr.reduce((s,ss)=>s+(ss.totalVolume||0),0);
  const volRecent = volSum(recent), volOlder = volSum(older);

  // 주당 평균 유산소
  const weeks = Math.max(1, range / 7);
  const weeklyCardioAvg = Math.round(cardioRecent / weeks);

  // 감량 반응성 분석
  const fatigueComment = (() => {
    if (!wDiff && !cardioRecent) return null;
    const cardioUp = cardioRecent > cardioOlder;
    const volUp    = volRecent > volOlder;
    if (wDiff !== null) {
      if (wDiff < -1.0 && cardioUp) return { label:"반응 양호", color:"#22c55e", text:`유산소량 증가와 함께 체중이 ${Math.abs(wDiff).toFixed(1)}kg 감소했습니다. 현재 방향은 유지 가능성이 높습니다.` };
      if (wDiff < -2.5) return { label:"회복 체크 필요", color:"#f97316", text:`체중이 빠르게 감소(${Math.abs(wDiff).toFixed(1)}kg)하고 있습니다. 근손실 방지를 위해 단백질 섭취와 근력 운동 강도를 확인하세요.` };
      if (Math.abs(wDiff) < 0.5 && cardioUp) return { label:"섭취/피로 요인 가능", color:"#ffd166", text:`유산소량은 증가했지만 체중 변화가 거의 없습니다. 식단 섭취량, 수면, 염분, 스트레스 요인을 함께 확인하는 것이 좋습니다.` };
      if (wDiff > 0.5 && !volUp) return { label:"활동량 감소 영향", color:"#ef4444", text:`운동량이 줄고 체중이 증가(+${wDiff.toFixed(1)}kg)했습니다. 활동량 감소 영향 가능성이 있습니다.` };
    }
    return { label:"데이터 누적 중", color:"#5EEAD4", text:`${range}일간 기록이 쌓이면 더 정확한 분석이 가능합니다.` };
  })();

  // 그래프 데이터 (날짜 라벨 짧게)
  const fmtDate = d => d ? d.slice(5) : "";
  const wChartData = weightData.slice(-20).map(d => ({ name: fmtDate(d.date), 체중: d.w }));
  const cChartData = cardioData.slice(-20).map(d => ({ name: fmtDate(d.date), 유산소: d.min, kcal: d.kcal }));
  // 복합 그래프 (체중+유산소)
  const comboMap = {};
  weightData.forEach(d => { comboMap[d.date] = { name: fmtDate(d.date), 체중: d.w }; });
  cardioData.forEach(d => {
    if (!comboMap[d.date]) comboMap[d.date] = { name: fmtDate(d.date) };
    comboMap[d.date].유산소분 = d.min;
  });
  const comboData = Object.values(comboMap).sort((a,b)=>a.name.localeCompare(b.name)).slice(-20);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[7,30].map(d=>(
          <button key={d} onClick={()=>setRange(d)}
            style={{padding:"5px 14px",borderRadius:7,border:"1px solid",fontSize:11,fontWeight:700,cursor:"pointer",
              borderColor:range===d?"#f97316":"rgba(255,255,255,0.1)",
              background:range===d?"rgba(249,115,22,.12)":"transparent",
              color:range===d?"#f97316":"#54546a"}}>
            최근 {d}일
        ...
        <Mo c="#3a4a5a" s={8} style={{alignSelf:"center"}}>* 대사 직접 측정이 아닌 기록 기반 추정 지표입니다</Mo>
      </div>

      {/* 요약 통계 */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
        {[
          { label:"유산소 총시간", val: cardioRecent > 0 ? `${cardioRecent}분` : "—", sub: cardioKcalRecent > 0 ? `${cardioKcalRecent}kcal` : null, color:"#f97316" },
          { label:"주당 평균 유산소", val: cardioRecent > 0 ? `${weeklyCardioAvg}분` : "—", sub: `${range}일 기준`, color:"#fdba74" },
          { label:"체중 변화", val: wDiff !== null ? `${wDiff>0?"+":""}${wDiff.toFixed(1)}kg` : "—",
            sub: (wRecent && wOlder) ? `${wOlder.toFixed(1)} → ${wRecent.toFixed(1)}kg` : null,
            color: wDiff === null ? "#54546a" : wDiff < 0 ? "#5EEAD4" : "#f97316" },
          { label:"웨이트 볼륨", val: volRecent > 0 ? `${(volRecent/1000).toFixed(1)}t` : "—",
            sub: volOlder > 0 ? `이전 ${(volOlder/1000).toFixed(1)}t` : null, color:"#818cf8" },
        ].map(({label,val,sub,color})=>(
          <div key={label} style={{background:"#0c1523",borderRadius:10,padding:"10px 13px",
            border:"1px solid rgba(255,255,255,0.07)"}}>
            <Mo c="#3a4a5a" s={8} style={{display:"block",marginBottom:3}}>{label}</Mo>
            <span style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:18,color,display:"block",lineHeight:1.2}}>{val}</span>
            {sub && <Mo c="#3a3a5a" s={8} style={{marginTop:2}}>{sub}</Mo>}
          </div>
        ))}
      </div>
      ...
      {fatigueComment && (
        <div style={{marginBottom:12,padding:"12px 14px",borderRadius:10,
          background:`${fatigueComment.color}0f`,border:`1px solid ${fatigueComment.color}33`}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <Mo c={fatigueComment.color} s={10} style={{fontWeight:800}}>감량 반응성 추정</Mo>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"2px 8px",borderRadius:4,
              background:fatigueComment.color+"22",color:fatigueComment.color,fontWeight:700}}>
              {fatigueComment.label}
            </span>
          </div>
          <Mo c="#94a3b8" s={10} style={{lineHeight:1.7}}>{fatigueComment.text}</Mo>
          <Mo c="#3a4a5a" s={8} style={{marginTop:4}}>⚠ 식단, 수면, 호르몬, 수분 등 개인차가 있으며 의료적 진단이 아닙니다.</Mo>
        </div>
      )}

      {/* 그래프 1: 체중 변화 */}
      {wChartData.length > 1 && (
        <Card title="📉 체중 변화" style={{marginBottom:11}}>
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={wChartData} margin={{top:6,right:6,left:-28,bottom:0}}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
              <XAxis dataKey="name" tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}}/>
              <YAxis tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}} domain={['auto','auto']}/>
              <Tooltip contentStyle={tt}/>
      ...
      {/* 그래프 2: 유산소 시간 + kcal */}
      {cChartData.length > 0 && (
        <Card title="🔥 유산소 시간 · 소모 칼로리" style={{marginBottom:11}}>
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={cChartData} margin={{top:6,right:6,left:-28,bottom:0}}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
              <XAxis dataKey="name" tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}}/>
              <YAxis tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}}/>
              <Tooltip contentStyle={tt}/>
              <Bar dataKey="유산소" fill="#f97316" name="유산소(분)" radius={[3,3,0,0]}/>
              <Bar dataKey="kcal" fill="#fbbf24" name="kcal" radius={[3,3,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </Card>
      )}
      ...
      {/* 그래프 3: 체중 + 유산소 복합 */}
      {comboData.length > 1 && (
        <Card title="📊 체중 변화 + 유산소량 비교" style={{marginBottom:11}}>
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={comboData} margin={{top:6,right:6,left:-28,bottom:0}}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
              <XAxis dataKey="name" tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}}/>
              <YAxis yAxisId="w" orientation="left"  tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#5EEAD4"}} domain={['auto','auto']}/>
              <YAxis yAxisId="c" orientation="right" tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#f97316"}}/>
              <Tooltip contentStyle={tt}/>
              <Line yAxisId="w" type="monotone" dataKey="체중"   stroke="#5EEAD4" strokeWidth={2} dot={false} name="체중(kg)"/>
              <Line yAxisId="c" type="monotone" dataKey="유산소분" stroke="#f97316" strokeWidth={2} dot={false} name="유산소(분)"/>
            </LineChart>
          </ResponsiveContainer>
          <Mo c="#3a4a5a" s={8} style={{marginTop:4}}>좌축: 체중(kg) · 우축: 유산소(분)</Mo>
        </Card>
      )}

      {/* 데이터 없을 때 */}
      {wChartData.length <= 1 && cChartData.length === 0 && (
        <Emp msg="체중 또는 유산소 기록이 부족합니다. 수업 기록 시 체중과 유산소를 입력하면 분석이 시작됩니다." />
      )}
    </div>
  );
}
