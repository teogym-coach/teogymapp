...
function generateAutoSummary(bodyMap, posture, mobility) {
  ...
  const painPts = Object.entries(bodyMap).filter(([,v])=>v==="pain").map(([k])=>getName(k));
  const tightPts= Object.entries(bodyMap).filter(([,v])=>v==="tight").map(([k])=>getName(k));
  const weakPts = Object.entries(bodyMap).filter(([,v])=>v==="weak").map(([k])=>getName(k));
  const issues  = [];
  ...
  const recs = [];
  if (bodyMap["lumbar"]==="pain"||bodyMap["lumbar"]==="tight") recs.push("요추 안정화");
  if (bodyMap["glute_l"]==="weak"||bodyMap["glute_r"]==="weak"||mobility["glute"]==="약함") recs.push("둔근 활성화");
...
function BodySVG({ parts, bodyMap, onClickPart, view }) {
  return (
    <svg viewBox="0 0 350 470" style={{width:"100%",maxWidth:250,display:"block",margin:"0 auto"}}>
      <ellipse cx="175" cy="42" rx="24" ry="28" fill="none" stroke="#2a2a3a" strokeWidth="2"/>
      <rect x="163" y="68" width="24" height="18" rx="4" fill="none" stroke="#2a2a3a" strokeWidth="1.5"/>
      <path d="M120 86 Q100 96 98 172 Q98 228 114 240 L138 262 L212 262 L236 240 Q252 228 252 172 Q250 96 230 86 Z" fill="none" stroke="#2a2a3a" strokeWidth="2"/>
      <path d="M120 90 Q96 102 90 155 Q88 175 94 192" fill="none" stroke="#2a2a3a" strokeWidth="1.5"/>
      <path d="M230 90 Q254 102 260 155 Q262 175 256 192" fill="none" stroke="#2a2a3a" strokeWidth="1.5"/>
      <path d="M138 262 L134 358 L130 440 L154 440 L156 358 L158 262" fill="none" stroke="#2a2a3a" strokeWidth="1.5"/>
      <path d="M212 262 L216 358 L220 440 L196 440 L194 358 L192 262" fill="none" stroke="#2a2a3a" strokeWidth="1.5"/>
      {parts.map(p => {
        const status = bodyMap[p.id] || "normal";
        const cfg    = BODY_STATUS[status];
        const active = status !== "normal";
        return (
          <g key={p.id} onClick={() => onClickPart(p.id)} style={{cursor:"pointer"}}>
            <circle cx={p.x} cy={p.y} r={p.r}
              fill={active ? cfg.color+"55" : "#151520"}
              stroke={active ? cfg.color : "#252535"}
              strokeWidth={active ? 2 : 1}/>
            <text x={p.x} y={p.y+1} textAnchor="middle" dominantBaseline="middle"
              style={{fontSize:6,fill:active?cfg.color:"#3a3a52",fontFamily:"sans-serif",
                fontWeight:active?"700":"400",pointerEvents:"none",userSelect:"none"}}>
              {p.label.replace("L","").replace("R","").substring(0,4)}
            </text>
          </g>
        );
      })}
      <text x="175" y="462" textAnchor="middle" style={{fontSize:8,fill:"#2a2a3a",fontFamily:"monospace"}}>
        {view==="back"?"POSTERIOR":"ANTERIOR"}
      </text>
    </svg>
  );
}
...
function AssessmentScreen({ member, onBack, showToast }) {
  ...
  function setPartStatus(id, status) {
    setBodyMap(prev => {
  ...
  if (viewRec) {
    ...
    return (
      <div>
        ...
        {activeParts.length>0 && <Card title="🫀 인체도 소견" style={{marginBottom:11}}><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{activeParts.map(([id,status])=>{const cfg=BODY_STATUS[status];const label=ALL_PARTS.find(p=>p.id===id)?.label||id;return <span key={id} style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"3px 8px",borderRadius:4,background:cfg.color+"22",color:cfg.color}}>{label} {cfg.label}</span>;})}</div></Card>}
      ...
      {tab==="인체도" && (
        <div>
          ...
          {selPart && (()=>{
            ...
            const cur=bodyMap[selPart]||"normal";
            return (
              <Card style={{marginBottom:10,border:"1px solid "+ac+"44",background:ac+"08"}}>
                <Mo c={ac} s={9} style={{display:"block",marginBottom:8}}>📍 {part?.label} — 상태 선택</Mo>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {STATUS_KEYS.map(sk=>{const cfg=BODY_STATUS[sk];return(
                    <button key={sk} onClick={()=>setPartStatus(selPart,sk)}
                      style={{padding:"9px",borderRadius:7,border:"1px solid",cursor:"pointer",
                        borderColor:cur===sk?cfg.color:"#1a1a24",background:cur===sk?cfg.color+"22":"transparent",
                        color:cur===sk?cfg.color:"#54546a",fontSize:12,fontWeight:700}}>
                      {cfg.label}
                    </button>
                  );})}
                </div>
          ...
          {Object.keys(bodyMap).length>0 && (
            <Card title="선택된 부위" style={{marginBottom:10}}>
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
                {Object.entries(bodyMap).map(([id,status])=>{const cfg=BODY_STATUS[status];const label=ALL_PARTS.find(p=>p.id===id)?.label||id;return(
                  <button key={id} onClick={()=>setPartStatus(id,"normal")}
