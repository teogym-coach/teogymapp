...
function CalorieTrendCard({profile,onboarding,body,nutrition,checkins,sessions}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const rows=getKcalLogs(nutrition).slice(-30).map(x=>({date:x.date.slice(5),kcal:x.kcal,target:target.value})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; const m7=mean(avg7),m30=mean(avg30); const chartRows=rows.map(r=>({...r,avg7:m7,avg30:m30})); const tt={ background:"#111827", border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, fontFamily:"'DM Mono',monospace", fontSize:11 }; return <Card title="📈 섭취 칼로리 변화 그래프" style={{marginBottom:11}}>{rows.length?<ResponsiveContainer width="100%" height={180}><ComposedChart data={chartRows} margin={{top:6,right:14,left:-18,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)"/><XAxis dataKey="date" tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}}/><YAxis tick={{fontFamily:"'DM Mono',monospace",fontSize:8,fill:"#54546a"}} unit="kcal"/><Tooltip contentStyle={tt} formatter={(v,n)=>[formatKcal(v),n]}/><Legend wrapperStyle={{fontFamily:"'DM Mono',monospace",fontSize:8}}/><Bar dataKey="kcal" name="일별 섭취" fill="#5EEAD4" radius={[5,5,0,0]} opacity={0.35}/><Line type="monotone" dataKey="avg7" name="7일 평균" stroke="#5EEAD4" strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="avg30" name="30일 평균" stroke="#7c6fff" strokeWidth={2} dot={false}/><Line type="monotone" dataKey="target" name="추천 기준" stroke="#ff6b6b" strokeWidth={1.5} strokeDasharray="4 4" dot={false}/></ComposedChart></ResponsiveContainer>:<div style={{height:180,display:"grid",placeItems:"center",border:"1px dashed rgba(255,255,255,0.16)",borderRadius:10}}><Mo c="#54546a" s={10}>섭취 칼로리 기록 수집 중</Mo></div>}<div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8,marginTop:12}}><KpiTile label="최근 7일 평균" value={formatKcal(m7)} accent="#5EEAD4"/><KpiTile label="최근 30일 평균" value={formatKcal(m30)} accent="#7c6fff"/><KpiTile label="유지칼로리" value={formatKcal(a.maintenance)} accent="#ffd166"/><KpiTile label="권장섭취" value={formatKcal(target.value)} sub={target.shortLabel||target.label} accent="#5EEAD4"/></div></Card>}
        ...
        <div>
          ...
          {goal.currentWeight && (
            <div>
              ...
              <CalorieTrendCard profile={member} onboarding={bodyData?.goal||{}} body={bodyData} nutrition={nutritionData} checkins={[]} sessions={sessions} />
