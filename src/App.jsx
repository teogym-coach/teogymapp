...
const SORENESS_BODY_PARTS=["가슴","등","하체","어깨","팔","코어","기타"];
...
function MemberFeedbackForm({s,onSave}){const existing=s.memberFeedback||{}; const sorenessBodyPartsKey=JSON.stringify(existing.sorenessBodyParts||[]); const makeState=()=>({sorenessLevel:existing.sorenessLevel||"없음",sorenessBodyParts:memberFeedbackParts(existing),rpe:Number.isFinite(Number(existing.rpe))?Number(existing.rpe):5,memo:existing.memo||""}); const [d,setD]=useState(makeState); useEffect(()=>setD(makeState()),[s.id,existing.sorenessLevel,existing.sorenessBodyPart,sorenessBodyPartsKey,existing.rpe,existing.memo]); const togglePart=part=>setD(prev=>({ ...prev, sorenessBodyParts: prev.sorenessBodyParts.includes(part)?prev.sorenessBodyParts.filter(x=>x!==part):[...prev.sorenessBodyParts,part] })); return <div className="member-feedback-form"><h3>수업 후 상태 기록</h3><SelectLine label="근육통" value={d.sorenessLevel} opts={SORENESS_LEVELS} onChange={v=>setD({...d,sorenessLevel:v})}/><div className="form-line"><label>근육통 부위 <small>여러 부위를 선택할 수 있고, 선택하지 않아도 저장됩니다.</small></label><div className="choice-buttons">{SORENESS_BODY_PARTS.map(part=><button type="button" key={part} className={d.sorenessBodyParts.includes(part)?"active":""} onClick={()=>togglePart(part)}>{part}</button>)}</div></div><div className="form-line"><label>RPE <small>RPE = Rate of Perceived Exertion</small><small>운동이 얼마나 힘들게 느껴졌는지 0~10점으로 표시해주세요.</small><small>0점 = 전혀 힘들지 않음 · 5점 = 보통 · 10점 = 더 이상 반복하기 어려움</small></label><div className="vas-buttons">{Array.from({length:11},(_,i)=><button type="button" key={i} className={Number(d.rpe)===i?"active":""} onClick={()=>setD({...d,rpe:i})}>{i}</button>)}</div></div><InputLine label="메모" value={d.memo} onChange={v=>setD({...d,memo:v})}/><button className="primary compact" onClick={()=>onSave?.(s.id,{...d,sorenessBodyPart:d.sorenessBodyParts[0]||""})}>수업 후 상태 저장</button></div>}
...
function getRecommendedPart(profile,sessions=[]){const info=getNextWorkoutInfo(profile); const parts=["가슴","등","하체","어깨","이두","삼두","코어"]; const avoid=new Set(); if(info.daysUntil!==null&&info.daysUntil<=1)avoid.add(info.part); if(info.part==="하체"&&info.daysUntil!==null&&info.daysUntil<=3)return "가슴"; if(["가슴","등","어깨","이두","삼두"].includes(info.part)&&info.daysUntil!==null&&info.daysUntil<=1)return "하체"; const counts=getRecentPartCounts(sessions); return parts.filter(p=>!avoid.has(p)).sort((a,b)=>(counts[a]||0)-(counts[b]||0))[0]||"가슴";}
