...
function MemberDebugPanel({profile,logs,errors}){const authUid=auth.currentUser?.uid||"-"; const authEmail=(auth.currentUser?.email||"").trim().toLowerCase()||"-"; const memberUid=profile?.memberUid||"-"; const memberEmail=(profile?.email||"").trim().toLowerCase()||"-"; const uidMatch=authUid!=="-"&&memberUid!=="-"&&authUid===memberUid; const emailMatch=authEmail!=="-"&&memberEmail!=="-"&&authEmail===memberEmail; const failed=Object.values(errors||{}); return <MCard title="회원앱 디버그"><div className="notice"><b>auth.uid:</b> {authUid}<br/><b>auth.email:</b> {authEmail}<br/><b>members.email:</b> {memberEmail}<br/><b>members.memberUid:</b> {memberUid}<br/><b>UID 불일치:</b> {uidMatch?"아니오":"예"}<br/><b>이메일 일치:</b> {emailMatch?"예":"아니오"}{profile?.memberUidRelinked&&<><br/><b>자동 재연결:</b> 완료</>}</div>{failed.length>0&&<div className="danger">권한/조회 실패 경로: {failed.map(e=>`${e.path} (${e.code})`).join(" · ")}</div>}<div className="notice"><b>Firestore 읽기 순서</b>{(logs||[]).map((l,i)=><div key={i}>{l.step}) {l.path} — {l.status}{l.code?` / ${l.code}`:""}</div>)}</div></MCard>}

function MemberAppError({message,details,logs,onRetry,onLogout}){const isAuthMissing=details?.code==="auth/user-not-found"; const authUid=details?.authUid||auth.currentUser?.uid||"-"; const authEmail=(details?.authEmail||auth.currentUser?.email||"").trim().toLowerCase()||"-"; const memberUid=details?.memberUid||"-"; const memberEmail=(details?.memberEmail||"").trim().toLowerCase()||"-"; const uidMismatch=memberUid!=="-"&&authUid!=="-"&&memberUid!==authUid; const emailMismatch=memberEmail!=="-"&&authEmail!=="-"&&memberEmail!==authEmail; return <div className="member-shell"><style>{CSS+MEMBER_CSS}</style><div className="member-page"><MCard title="회원앱을 열 수 없습니다"><p className="danger">{message}</p>{details&&<div className="notice"><b>오류 코드:</b> {details.code||"unknown"}{details.path&&<><br/><b>실패 경로:</b> {details.path}</>}<br/><b>auth.uid:</b> {authUid}<br/><b>auth.email:</b> {authEmail}<br/><b>members.email:</b> {memberEmail}<br/><b>members.memberUid:</b> {memberUid}<br/><b>UID 불일치:</b> {uidMismatch?"예":"확인 필요"}<br/><b>이메일 불일치:</b> {emailMismatch?"예":"아니오 또는 확인 필요"}</div>}{logs?.length>0&&<div className="notice"><b>Firestore 읽기 순서</b>{logs.map((l,i)=><div key={i}>{l.step}) {l.path} — {l.status}{l.code?` / ${l.code}`:""}</div>)}</div>}<p className="notice">{isAuthMissing?"Firebase Authentication 사용자 계정을 찾을 수 없습니다. 회원앱 초대/비밀번호 설정 상태를 확인해주세요.":"Firebase Authentication 로그인은 완료됐지만 Firestore 권한 또는 데이터 조회 단계에서 실패했습니다. 위 오류 코드와 실패 경로를 확인해주세요."}</p><button className="primary" onClick={onRetry}>다시 시도</button><button className="ghost" onClick={onLogout}>로그아웃</button></MCard></div></div>}
