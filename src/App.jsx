...
function AdminMemberAppPanel({member,members=[],onAccountCreated,showManagement=false}){
  ...
  const onboardingFocus=ob?.focusAreas?.length?ob.focusAreas.join(" · "):"-";
  ...
  return <div style={{marginTop:10,display:"grid",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))",gap:8,alignItems:"stretch"}}>
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column",justifyContent:"space-between",minHeight:118}}><Mo c="#5EEAD4" s={9}>다음 PT 예정 부위</Mo><select value={member?.nextWorkoutPart||member?.nextPtPart||"미정"} onChange={e=>saveNextPtPart(e.target.value)} disabled={busy} style={{width:"100%",marginTop:10,background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:20,fontWeight:900}}>{NEXT_PT_PART_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></div>
      <div style={{padding:10,borderRadius:8,background:"#0B1120",border:"1px solid rgba(255,255,255,.08)",display:"flex",flexDirection:"column",justifyContent:"space-between",minHeight:118}}><Mo c="#5EEAD4" s={9}>다음 PT 날짜</Mo><div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8,alignItems:"center",marginTop:8}}><input type="date" value={member?.nextWorkoutDate||""} onChange={e=>saveNextWorkoutDate(e.target.value)} disabled={busy} style={{width:"100%",background:"#020617",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"#e5e7eb",padding:"9px 10px",fontSize:12}}/><b style={{fontSize:18,color:"#5EEAD4",whiteSpace:"nowrap"}}>{nextPtInfo.dDay}</b></div><div style={{fontSize:18,color:"#e5e7eb",fontWeight:900,textAlign:"center",marginTop:6}}>{member?.nextWorkoutDate?formatCompactDate(member.nextWorkoutDate):"날짜 미정"}</div></div>
