...
function CalorieTrendCard({profile,onboarding,body,nutrition,checkins,sessions}){const a=estimateMaintenance(profile,onboarding,body,nutrition,checkins,sessions); const target=getGoalCalorieRecommendation(a,onboarding?.goal||profile?.goal); const rows=getKcalLogs(nutrition).slice(-30).map(x=>({date:x.date.slice(5),kcal:x.kcal})); const avg7=rows.slice(-7); const avg30=rows; const mean=a=>a.length?Math.round(a.reduce((s,x)=>s+x.kcal,0)/a.length):null; return <MCard title="섭취 칼로리 변화 그래프"><ResponsiveContainer width="100%" height={180}><BarChart data={rows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis/><Tooltip/><Bar dataKey="kcal" name="섭취 kcal" fill="#2F73F6" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer><Info l="최근 7일 평균 섭취 칼로리" v={formatKcal(mean(avg7))}/><Info l="최근 30일 평균 섭취 칼로리" v={formatKcal(mean(avg30))}/><Info l="현재 감량 속도" v={a.weeklyChange===null?"기록 수집 중":`${a.weeklyChange>0?"+":""}${a.weeklyChange.toFixed(2)}kg/주`}/><Info l="현재 유지 추정 칼로리" v={formatKcal(a.maintenance)}/><Info l="현재 추천 칼로리" v={`${target.label} · ${formatKcal(target.value)}`}/></MCard>}
