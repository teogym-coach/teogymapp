...
// ════════════════════════════════════════════
const BODY_STATUS = {
  normal: { label:"정상",     color:"#3a3a5a", dot:"#54546a" },
  tight:  { label:"타이트함", color:"#ff6b6b", dot:"#ff6b6b" },
  weak:   { label:"기능저하", color:"#54a0ff", dot:"#54a0ff" },
  pain:   { label:"통증",     color:"#ff9f43", dot:"#ff9f43" },
};
const STATUS_KEYS = Object.keys(BODY_STATUS);

const FRONT_PARTS = [
  {id:"neck_f",     label:"목",        x:175, y:68,  r:18},
  {id:"chest",      label:"흉곽",      x:175, y:118, r:26},
  {id:"sh_fr_l",    label:"어깨전L",   x:120, y:105, r:15},
  {id:"sh_fr_r",    label:"어깨전R",   x:230, y:105, r:15},
  {id:"abdomen",    label:"복부",      x:175, y:172, r:22},
  {id:"pelvis_f",   label:"골반",      x:175, y:215, r:20},
  {id:"hip_l",      label:"고관절L",   x:142, y:238, r:14},
  {id:"hip_r",      label:"고관절R",   x:208, y:238, r:14},
  {id:"quad_l",     label:"대퇴사두L", x:142, y:295, r:18},
  {id:"quad_r",     label:"대퇴사두R", x:208, y:295, r:18},
  {id:"inner_l",    label:"내전근L",   x:157, y:282, r:11},
  {id:"inner_r",    label:"내전근R",   x:193, y:282, r:11},
  {id:"knee_l",     label:"무릎L",     x:142, y:345, r:13},
  {id:"knee_r",     label:"무릎R",     x:208, y:345, r:13},
  {id:"ankle_l",    label:"발목L",     x:142, y:430, r:14},
  {id:"ankle_r",    label:"발목R",     x:208, y:430, r:14},
];

const BACK_PARTS = [
  {id:"neck_b",     label:"목(후)",    x:175, y:68,  r:18},
  {id:"trap",       label:"등상부",    x:175, y:112, r:26},
  {id:"sh_bk_l",    label:"어깨후L",   x:120, y:105, r:15},
  {id:"sh_bk_r",    label:"어깨후R",   x:230, y:105, r:15},
  {id:"lumbar",     label:"요추",      x:175, y:163, r:19},
  {id:"pelvis_b",   label:"골반(후)",  x:175, y:208, r:20},
  {id:"glute_l",    label:"둔근L",     x:148, y:228, r:17},
  {id:"glute_r",    label:"둔근R",     x:202, y:228, r:17},
  {id:"ham_l",      label:"햄스트링L", x:142, y:293, r:18},
  {id:"ham_r",      label:"햄스트링R", x:208, y:293, r:18},
  {id:"calf_l",     label:"종아리L",   x:142, y:375, r:15},
  {id:"calf_r",     label:"종아리R",   x:208, y:375, r:15},
  {id:"foot_l",     label:"발목L",     x:142, y:430, r:14},
  {id:"foot_r",     label:"발목R",     x:208, y:430, r:14},
];
...
const MOBILITY_ITEMS = [
  ...
  {key:"glute",    label:"둔근 기능",             opts:["약함","보통","강함"]},
...
// bodyMap: { id: ["tight","pain"] } — 배열 구조 (다중 선택)
function getStatuses(bodyMap, id) { const v=bodyMap[id]; if(!v) return []; return Array.isArray(v)?v:[v]; }
function hasStatus(bodyMap, id, st) { return getStatuses(bodyMap,id).includes(st); }
function anyHasStatus(bodyMap, ids, st) { return ids.some(id=>hasStatus(bodyMap,id,st)); }

// 부위 표시색: 여러 상태 중 우선순위 (통증>기능저하>타이트함)
function getPrimaryStatus(statuses) {
  if (!statuses||!statuses.length) return "normal";
...
function generateAutoSummary(bodyMap, posture, mobility) {
  const getName = id => ALL_PARTS.find(p=>p.id===id)?.label || id;
  const entries = Object.entries(bodyMap);
  const hasS = (id,st) => hasStatus(bodyMap,id,st);

  const painPts  = entries.filter(([,v])=>(Array.isArray(v)?v:[v]).includes("pain")).map(([k])=>getName(k));
  const tightPts = entries.filter(([,v])=>(Array.isArray(v)?v:[v]).includes("tight")).map(([k])=>getName(k));
  const weakPts  = entries.filter(([,v])=>(Array.isArray(v)?v:[v]).includes("weak")).map(([k])=>getName(k));
  const issues = [];
  if (painPts.length)  issues.push(painPts.join(", ")+" 통증");
  if (tightPts.length) issues.push(tightPts.join(", ")+" 타이트함");
  if (weakPts.length)  issues.push(weakPts.join(", ")+" 기능 저하");
  ...
  const mobIssues = MOBILITY_ITEMS.filter(item=>{const v=mobility[item.key];return v&&(v==="제한"||v==="제한 있음"||v==="약함");}).map(item=>item.label+" "+mobility[item.key]);
  if (mobIssues.length) issues.push(...mobIssues);
  if (!issues.length) return "";
  const recs = [];
  if (hasS("lumbar","pain")||hasS("lumbar","tight")) recs.push("요추 안정화");
  if (hasS("glute_l","weak")||hasS("glute_r","weak")||mobility["glute"]==="약함") recs.push("둔근 활성화");
  if (mobility["df"]==="제한 있음") recs.push("발목 가동성 회복");
  if (mobility["hip_flex"]==="제한"||mobility["hip_ir"]==="제한") recs.push("고관절 가동성 개선");
  if (mobility["core"]==="약함") recs.push("코어 안정성 강화");
  return issues.join(", ")+"이(가) 확인됩니다."+(recs.length?" 초기 루틴은 "+recs.join(", ")+" 중심으로 구성하는 것이 좋습니다.":"");
}

function BodySVG({ parts, bodyMap, onClickPart, view }) {
  const isBack = view === "back";
  return (
    <svg viewBox="0 0 350 520" style={{width:"100%",maxWidth:280,display:"block",margin:"0 auto"}}>
      <defs>
      ...
      {!isBack && <>
        {/* 머리 */}
        <ellipse cx="175" cy="38" rx="26" ry="30" fill="#1a1a28" stroke="#2e2e42" strokeWidth="1.5"/>
        {/* 목 */}
        <path d="M163 66 Q163 78 162 82 L188 82 Q187 78 187 66 Z" fill="#1a1a28" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 승모근/어깨 라인 */}
        <path d="M162 82 Q145 82 128 92 Q108 104 106 118 L106 148 Q108 145 114 142 L114 108 Q130 96 148 90 L162 88 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1.2"/>
        <path d="M188 82 Q205 82 222 92 Q242 104 244 118 L244 148 Q242 145 236 142 L236 108 Q220 96 202 90 L188 88 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 대흉근 (L) */}
        <path d="M114 108 Q128 104 148 108 L162 116 L162 145 Q145 150 128 146 Q114 140 112 128 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 대흉근 (R) */}
        <path d="M236 108 Q222 104 202 108 L188 116 L188 145 Q205 150 222 146 Q236 140 238 128 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 흉골/복근 중앙선 */}
        <line x1="175" y1="84" x2="175" y2="220" stroke="#252538" strokeWidth="1"/>
        {/* 복직근 */}
        <path d="M162 148 L162 222 Q168 226 175 226 Q182 226 188 222 L188 148" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 복근 분절선 */}
        {[162,176,190].map((y,i)=><line key={i} x1="162" y1={y} x2="188" y2={y} stroke="#252538" strokeWidth="0.8"/>)}
        {/* 전거근 */}
        <path d="M114 142 Q110 168 112 195 L128 192 Q126 168 128 148 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.8"/>
        <path d="M236 142 Q240 168 238 195 L222 192 Q224 168 222 148 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.8"/>
        {/* 삼각근 (L) */}
        <path d="M106 118 Q96 128 94 148 Q96 162 106 168 L114 158 L114 128 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 삼각근 (R) */}
        <path d="M244 118 Q254 128 256 148 Q254 162 244 168 L236 158 L236 128 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 이두근 (L) */}
        <path d="M106 168 Q98 185 98 205 Q100 218 108 222 L116 218 Q112 205 112 188 L114 168 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 이두근 (R) */}
        <path d="M244 168 Q252 185 252 205 Q250 218 242 222 L234 218 Q238 205 238 188 L236 168 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 전완 (L) */}
        <path d="M108 222 Q102 240 104 265 Q108 275 114 276 L118 272 Q116 250 114 228 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 전완 (R) */}
        <path d="M242 222 Q248 240 246 265 Q242 275 236 276 L232 272 Q234 250 236 228 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 골반 */}
        <path d="M128 222 Q128 234 135 242 L215 242 Q222 234 222 222 L188 225 L162 225 Z" fill="#1c1c2c" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 대퇴사두 (L) */}
        <path d="M135 242 Q128 260 126 300 Q126 335 132 355 L148 355 Q150 330 152 295 Q154 260 162 245 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 대퇴사두 (R) */}
        <path d="M215 242 Q222 260 224 300 Q224 335 218 355 L202 355 Q200 330 198 295 Q196 260 188 245 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 내전근 (L) */}
        <path d="M162 245 Q155 265 154 300 Q154 330 156 355 L162 355 Q160 330 160 295 Q162 265 165 248 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.8"/>
        {/* 내전근 (R) */}
        <path d="M188 245 Q195 265 196 300 Q196 330 194 355 L188 355 Q190 330 190 295 Q188 265 185 248 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.8"/>
        {/* 무릎 (L) */}
        <ellipse cx="140" cy="362" rx="14" ry="10" fill="#1a1a28" stroke="#2e2e42" strokeWidth="1"/>
        {/* 무릎 (R) */}
        <ellipse cx="210" cy="362" rx="14" ry="10" fill="#1a1a28" stroke="#2e2e42" strokeWidth="1"/>
        {/* 정강이 (L) */}
        <path d="M132 370 Q128 400 130 430 L150 432 Q150 400 148 372 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 정강이 (R) */}
        <path d="M218 370 Q222 400 220 430 L200 432 Q200 400 202 372 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 발 (L) */}
        <path d="M130 430 Q126 445 128 452 L152 452 Q154 445 150 432 Z" fill="#161622" stroke="#252538" strokeWidth="1"/>
        {/* 발 (R) */}
        <path d="M220 430 Q224 445 222 452 L198 452 Q196 445 200 432 Z" fill="#161622" stroke="#252538" strokeWidth="1"/>
        <text x="175" y="470" textAnchor="middle" style={{fontSize:8,fill:"#2a2a3a",fontFamily:"monospace",letterSpacing:".1em"}}>ANTERIOR</text>
      </>}
      ...
      {isBack && <>
        ...
        <path d="M163 66 Q163 78 162 82 L188 82 Q187 78 187 66 Z" fill="#1a1a28" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 승모근 (상부) */}
        <path d="M162 82 Q148 84 132 94 Q115 106 112 124 L128 124 Q132 110 148 100 L162 94 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        <path d="M188 82 Q202 84 218 94 Q235 106 238 124 L222 124 Q218 110 202 100 L188 94 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 삼각근 후면 (L) */}
        <path d="M112 124 Q104 136 102 155 Q104 170 112 176 L118 170 L118 138 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 삼각근 후면 (R) */}
        <path d="M238 124 Q246 136 248 155 Q246 170 238 176 L232 170 L232 138 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 광배근 (L) */}
        <path d="M118 138 Q112 165 114 200 L138 210 Q138 175 130 145 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 광배근 (R) */}
        <path d="M232 138 Q238 165 236 200 L212 210 Q212 175 220 145 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 척추 기립근 */}
        <path d="M162 90 L162 222 Q168 225 175 225 Q182 225 188 222 L188 90" fill="#1c1c2c" stroke="#252538" strokeWidth="0.8"/>
        {/* 척추선 */}
        <line x1="175" y1="86" x2="175" y2="225" stroke="#252538" strokeWidth="0.8" strokeDasharray="3,2"/>
        {/* 능형근 */}
        <path d="M138 102 L138 140 L162 148 L162 104 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.7"/>
        <path d="M212 102 L212 140 L188 148 L188 104 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="0.7"/>
        {/* 삼두근 (L) */}
        <path d="M112 176 Q104 196 106 215 Q108 226 116 228 L120 224 Q116 210 116 192 L118 176 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 삼두근 (R) */}
        <path d="M238 176 Q246 196 244 215 Q242 226 234 228 L230 224 Q234 210 234 192 L232 176 Z" fill="#1c1c2c" stroke="#252538" strokeWidth="1"/>
        {/* 전완 후면 (L) */}
        <path d="M116 228 Q110 248 112 268 Q116 278 122 278 L126 274 Q122 254 122 234 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 전완 후면 (R) */}
        <path d="M234 228 Q240 248 238 268 Q234 278 228 278 L224 274 Q228 254 228 234 Z" fill="#1a1a28" stroke="#252538" strokeWidth="1"/>
        {/* 골반 후면 */}
        <path d="M128 222 Q128 235 138 244 L212 244 Q222 235 222 222 L188 226 L162 226 Z" fill="#1c1c2c" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 둔근 (L) */}
        <path d="M138 244 Q128 258 128 278 Q130 295 140 298 L162 295 Q162 272 158 254 Q155 244 148 244 Z" fill="#222232" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 둔근 (R) */}
        <path d="M212 244 Q222 258 222 278 Q220 295 210 298 L188 295 Q188 272 192 254 Q195 244 202 244 Z" fill="#222232" stroke="#2e2e42" strokeWidth="1.2"/>
        {/* 햄스트링 (L) */}
        <path d="M140 298 Q130 320 130 355 L148 358 Q150 325 152 300 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 햄스트링 (R) */}
        <path d="M210 298 Q220 320 220 355 L202 358 Q200 325 198 300 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 종아리 (L) */}
        <path d="M130 362 Q126 395 128 425 L148 427 Q150 395 152 362 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 종아리 (R) */}
        <path d="M220 362 Q224 395 222 425 L202 427 Q200 395 198 362 Z" fill="#1e1e2e" stroke="#2e2e42" strokeWidth="1"/>
        {/* 발 (L) */}
        <path d="M128 425 Q124 440 126 450 L150 450 Q152 440 148 427 Z" fill="#161622" stroke="#252538" strokeWidth="1"/>
        {/* 발 (R) */}
        <path d="M222 425 Q226 440 224 450 L200 450 Q198 440 202 427 Z" fill="#161622" stroke="#252538" strokeWidth="1"/>
        <text x="175" y="470" textAnchor="middle" style={{fontSize:8,fill:"#2a2a3a",fontFamily:"monospace",letterSpacing:".1em"}}>POSTERIOR</text>
      </>}

      {/* ── 클릭 가능 부위 오버레이 ── */}
      {parts.map(p => {
        const statuses  = getStatuses(bodyMap, p.id);
        const primary   = getPrimaryStatus(statuses);
        const cfg       = BODY_STATUS[primary];
        const active    = primary !== "normal";
        const multiCnt  = statuses.length;
        return (
          <g key={p.id} onClick={() => onClickPart(p.id)} style={{cursor:"pointer"}}>
            <circle cx={p.x} cy={p.y} r={p.r}
              fill={active ? cfg.color+"40" : "transparent"}
              stroke={active ? cfg.color : "#2a2a3c"}
              strokeWidth={active ? 2 : 1}
              filter={active ? "url(#glow)" : "none"}
              style={{transition:"all .2s"}}/>
            {active && multiCnt > 1 && (
              <circle cx={p.x+p.r-4} cy={p.y-p.r+4} r={5} fill="#09090c" stroke={cfg.color} strokeWidth={1}/>
            )}
            {active && multiCnt > 1 && (
              <text x={p.x+p.r-4} y={p.y-p.r+4} textAnchor="middle" dominantBaseline="middle"
                style={{fontSize:5,fill:cfg.color,fontWeight:"700",pointerEvents:"none",userSelect:"none"}}>
                {multiCnt}
              </text>
            )}
            <text x={p.x} y={p.y+1} textAnchor="middle" dominantBaseline="middle"
              style={{fontSize:active?6.5:6,fill:active?cfg.color:"#2e2e46",fontFamily:"sans-serif",
                fontWeight:active?"700":"400",pointerEvents:"none",userSelect:"none"}}>
              {p.label.length<=4?p.label:p.label.substring(0,4)}
            </text>
          </g>
        );
      })}
...
function AssessmentScreen({ member, onBack, showToast }) {
  ...
  const currentParts = bodyView==="front" ? FRONT_PARTS : BACK_PARTS;

  function togglePartStatus(id, status) {
    setBodyMap(prev => {
      const cur = getStatuses(prev, id);
      if (status === "normal") { const n={...prev}; delete n[id]; return n; }
      const next = cur.includes(status)
        ? cur.filter(s=>s!==status)
        : [...cur, status];
      if (!next.length) { const n={...prev}; delete n[id]; return n; }
      return {...prev, [id]: next};
    });
    // 선택 패널 닫지 않음 — 다중 선택 가능하게
  }
  function clearPartStatus(id) {
    setBodyMap(prev => { const n={...prev}; delete n[id]; return n; });
  }
  ...
  async function handleSave() {
    setSaving(true);
    const rec = {id:"a"+Date.now(),date:assDate,vasScore,vasTiming,vasMemo,bodyMap:{...bodyMap},posture:{...posture},mobility:{...mobility},summary};
  ...
  const TABS = [{key:"인체도",icon:"🫀"},{key:"통증",icon:"🩺"},{key:"자세",icon:"📐"},{key:"기능",icon:"⚡"},{key:"기록목록",icon:"📅"}];
  const ac = "#a29bfe";

  if (viewRec) {
    const vb = viewRec.bodyMap || {};
    const activeParts = Object.entries(vb).filter(([,v])=>v!=="normal");
    return (
      <div>
        <SH title={"📋 "+viewRec.date+" 평가"} sub={member.name} right={<Btn ghost sm onClick={()=>setViewRec(null)}>← 뒤로</Btn>}/>
        {viewRec.summary && <Card style={{marginBottom:11,border:"1px solid "+ac+"33",background:ac+"08"}}><Mo c={ac} s={9} style={{display:"block",marginBottom:5}}>📝 평가 요약</Mo><div style={{fontSize:12,color:"#ddddf0",lineHeight:1.8}}>{viewRec.summary}</div></Card>}
        {viewRec.vasScore>0 && <Card title="🩺 통증" style={{marginBottom:11}}><div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6}}><Mo c="#ff6b6b" s={9}>VAS</Mo><span style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:24,color:"#ff6b6b"}}>{viewRec.vasScore}</span><Mo c="#54546a" s={9}>/10</Mo>{(viewRec.vasTiming||[]).length>0&&<Mo c="#ffd166" s={9}>{viewRec.vasTiming.join(" · ")}</Mo>}</div>{viewRec.vasMemo&&<div style={{fontSize:11,color:"#54546a"}}>{viewRec.vasMemo}</div>}</Card>}
        {activeParts.length>0 && <Card title="🫀 인체도 소견" style={{marginBottom:11}}><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{activeParts.map(([id,statuses])=>{const list=Array.isArray(statuses)?statuses:[statuses];const prim=getPrimaryStatus(list);const cfg=BODY_STATUS[prim];const label=ALL_PARTS.find(p=>p.id===id)?.label||id;return <span key={id} style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"3px 8px",borderRadius:4,background:cfg.color+"22",color:cfg.color}}>{label} {list.map(s=>BODY_STATUS[s]?.label||s).join("+")}</span>;})}</div></Card>}
        {POSTURE_ITEMS.some(item=>(viewRec.posture?.[item.key]||[]).length>0) && <Card title="📐 자세 평가" style={{marginBottom:11}}>{POSTURE_ITEMS.map(item=>{const vals=viewRec.posture?.[item.key]||[];if(!vals.length)return null;return <div key={item.key} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #1a1a24"}}><Mo c="#54546a" s={10}>{item.label}</Mo><Mo c="#ffd166" s={10}>{vals.join(", ")}</Mo></div>;})}</Card>}
        {MOBILITY_ITEMS.some(item=>viewRec.mobility?.[item.key]) && <Card title="⚡ 기능 검사" style={{marginBottom:11}}>{MOBILITY_ITEMS.map(item=>{const val=viewRec.mobility?.[item.key];if(!val)return null;const col=val==="제한"||val==="제한 있음"||val==="약함"?"#ff9f43":val==="강함"||val==="양호"?"#00e5a0":"#ffd166";return <div key={item.key} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #1a1a24"}}><Mo c="#54546a" s={9}>{item.label}</Mo><Mo c={col} s={10}>{val}</Mo></div>;})}</Card>}
      </div>
    );
  }
  ...
  return (
    <div>
      <SH title="📋 평가 기록" sub={member.name} right={<Btn ghost sm onClick={onBack}>← 뒤로</Btn>}/>
      ...
      {tab==="인체도" && (
        <div>
          <div style={{display:"flex",gap:6,marginBottom:10}}>
            {["front","back"].map(v=>(
              <button key={v} onClick={()=>{setBodyView(v);setSelPart(null);}}
                style={{flex:1,padding:"7px",borderRadius:7,border:"1px solid",cursor:"pointer",
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
            {Object.entries(BODY_STATUS).map(([k,v])=>(
              <div key={k} style={{display:"flex",alignItems:"center",gap:4}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:v.dot}}/>
                <Mo c={v.dot} s={8}>{v.label}</Mo>
              </div>
            ))}
          </div>
          <Card style={{marginBottom:10}}>
            <BodySVG parts={currentParts} bodyMap={bodyMap} onClickPart={id=>setSelPart(prev=>prev===id?null:id)} view={bodyView}/>
          </Card>
          {selPart && (()=>{
            const part      = currentParts.find(p=>p.id===selPart);
            const curList   = getStatuses(bodyMap, selPart);
            const primary   = getPrimaryStatus(curList);
            return (
              <Card style={{marginBottom:10,border:"1px solid "+ac+"44",background:ac+"08"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <Mo c={ac} s={9}>📍 {part?.label} — 상태 선택 (복수 선택 가능)</Mo>
                  <button onClick={()=>setSelPart(null)}
                    style={{background:"none",border:"none",color:"#54546a",fontSize:14,cursor:"pointer",padding:"0 4px"}}>✕</button>
                </div>
                {/* 정상 초기화 버튼 */}
                <button onClick={()=>{togglePartStatus(selPart,"normal"); setSelPart(null);}}
                  style={{width:"100%",padding:"8px",borderRadius:7,border:"1px solid",cursor:"pointer",marginBottom:8,
                    borderColor:curList.length===0?"#00e5a0":"#1a1a24",
                    background:curList.length===0?"rgba(0,229,160,.15)":"transparent",
                    color:curList.length===0?"#00e5a0":"#54546a",fontSize:12,fontWeight:700}}>
                  ✓ 정상 (선택 해제)
                </button>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                  {["tight","weak","pain"].map(sk=>{
                    const cfg    = BODY_STATUS[sk];
                    const active = curList.includes(sk);
                    return (
                      <button key={sk} onClick={()=>togglePartStatus(selPart,sk)}
                        style={{padding:"11px 6px",borderRadius:7,border:"2px solid",cursor:"pointer",
                          borderColor:active?cfg.color:"#1a1a24",
                          background:active?cfg.color+"33":"transparent",
                          color:active?cfg.color:"#54546a",fontSize:12,fontWeight:800,
                        ...
                        {cfg.label}
                        {active && <span style={{fontSize:8,color:cfg.color}}>✓ 선택됨</span>}
                      </button>
                    );
                  })}
                </div>
                {curList.length > 0 && (
                  <div style={{marginTop:8,padding:"6px 10px",background:"#09090c",borderRadius:6,
                    display:"flex",gap:5,flexWrap:"wrap"}}>
                    {curList.map(sk => {
                      const cfg = BODY_STATUS[sk];
                      return <span key={sk} style={{fontFamily:"'DM Mono',monospace",fontSize:9,
                        padding:"2px 7px",borderRadius:4,background:cfg.color+"22",color:cfg.color}}>
                        {cfg.label}
                      </span>;
                    })}
                  </div>
          ...
          {Object.keys(bodyMap).length>0 && (
            <Card title="선택된 부위" style={{marginBottom:10}}>
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
                {Object.entries(bodyMap).map(([id,statuses])=>{
                  const list  = Array.isArray(statuses)?statuses:[statuses];
                  const prim  = getPrimaryStatus(list);
                  const cfg   = BODY_STATUS[prim];
                  const label = ALL_PARTS.find(p=>p.id===id)?.label||id;
                  return (
                    <button key={id} onClick={()=>clearPartStatus(id)}
                      style={{fontFamily:"'DM Mono',monospace",fontSize:9,padding:"3px 8px",borderRadius:4,
                        background:cfg.color+"22",color:cfg.color,border:"1px solid "+cfg.color+"44",cursor:"pointer",
                        display:"flex",alignItems:"center",gap:4}}>
                      <span>{label}</span>
                      <span style={{opacity:.7}}>{list.map(s=>BODY_STATUS[s]?.label).join("+")} ✕</span>
                    </button>
                  );
                })}
              </div>
              <button onClick={()=>setBodyMap({})} style={{background:"none",border:"1px solid #1a1a24",borderRadius:5,color:"#54546a",fontSize:10,fontWeight:700,padding:"4px 10px",cursor:"pointer"}}>전체 초기화</button>
            </Card>
          )}
        </div>
      )}
        ...
        <Card title="🩺 통증 기록" style={{marginBottom:12}}>
          <div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <span style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:40,
                color:vasScore===0?"#3a3a4a":vasScore<=3?"#00e5a0":vasScore<=6?"#ffd166":"#ff6b6b"}}>{vasScore}</span>
              <Mo c="#54546a" s={9} style={{alignSelf:"flex-end",paddingBottom:6}}>/10</Mo>
            </div>
            <input type="range" min={0} max={10} value={vasScore} onChange={e=>setVasScore(parseInt(e.target.value))}
              style={{width:"100%",accentColor:"#ff6b6b",border:"none",padding:0,background:"transparent"}}/>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
              <Mo c="#3a3a4a" s={8}>없음</Mo><Mo c="#ff6b6b" s={8}>극심함</Mo>
            </div>
          </div>
          <div style={{marginBottom:12}}>
          ...
          <TextArea label="통증 부위 및 메모" value={vasMemo} onChange={setVasMemo} placeholder="예: 우측 무릎 내측, 계단 오를 때 심함"/>
        </Card>
      )}

      {tab==="자세" && (
        <Card title="📐 체형 자세 평가" style={{marginBottom:12}}>
          <Mo c="#54546a" s={9} style={{display:"block",marginBottom:12}}>해당하는 항목 모두 선택 (복수 선택 가능)</Mo>
      ...
      {tab==="기능" && (
        <Card title="⚡ 가동성 및 기능 검사" style={{marginBottom:12}}>
          <Mo c="#54546a" s={9} style={{display:"block",marginBottom:12}}>각 항목 결과를 선택하세요</Mo>
        ...
        <div>
          {records.length===0 ? (
            <div style={{textAlign:"center",padding:"40px",background:"#111116",borderRadius:12,border:"1px dashed #1a1a24"}}>
              <div style={{fontSize:32,marginBottom:8}}>📋</div>
              <Mo c="#2a2a3a" s={10}>평가 기록이 없습니다.</Mo>
            </div>
          ) : (
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {records.map(r=>(
                <div key={r.id} style={{background:"#111116",border:"1px solid #1a1a24",borderRadius:10,padding:"12px 14px",cursor:"pointer"}}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    ...
                    <div style={{display:"flex",gap:5}}>
                      {r.vasScore>0&&<span style={{fontFamily:"'DM Mono',monospace",fontSize:8,padding:"2px 6px",borderRadius:4,background:"rgba(255,107,107,.15)",color:"#ff9f43"}}>VAS {r.vasScore}</span>}
                      {Object.values(r.bodyMap||{}).filter(v=>v!=="normal").length>0&&<span style={{fontFamily:"'DM Mono',monospace",fontSize:8,padding:"2px 6px",borderRadius:4,background:"rgba(162,155,254,.15)",color:"#a29bfe"}}>부위 {Object.values(r.bodyMap||{}).filter(v=>v!=="normal").length}곳</span>}
                    </div>
                  </div>
      ...
      {tab!=="기록목록" && (
        <div style={{marginTop:4}}>
          {summary && (
            <Card title="📝 자동 평가 요약" style={{marginBottom:12,border:"1px solid "+ac+"44",background:ac+"08"}}>
              <div style={{fontSize:12,color:"#ddddf0",lineHeight:1.9}}>{summary}</div>
            </Card>
          )}
